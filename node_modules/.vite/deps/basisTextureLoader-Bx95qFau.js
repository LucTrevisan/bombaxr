...
function workerFunction() {
	...
	onmessage = (event) => {
		if (event.data.action === "init") {
			...
			if (!transcoderModulePromise) transcoderModulePromise = BASIS({ wasmBinary: event.data.wasmBinary });
			if (transcoderModulePromise !== null) transcoderModulePromise.then((m) => {
				BASIS = m;
				m.initializeBasis();
			...
			let success = true;
			if (!loadedFile.startTranscoding()) success = false;
			const buffers = [];
			for (let imageIndex = 0; imageIndex < fileInfo.images.length; imageIndex++) {
				...
				if (config.loadSingleImage === void 0 || config.loadSingleImage === imageIndex) {
					...
					if (config.loadMipmapLevels === false) mipCount = 1;
	...
	*/
	function GetSupportedTranscodeFormat(config, fileInfo) {
		let format = null;
		if (config.supportedCompressionFormats) if (config.supportedCompressionFormats.astc) format = _BASIS_FORMAT.cTFASTC_4x4;
		else if (config.supportedCompressionFormats.bc7) format = _BASIS_FORMAT.cTFBC7;
		else if (config.supportedCompressionFormats.s3tc) format = fileInfo.hasAlpha ? _BASIS_FORMAT.cTFBC3 : _BASIS_FORMAT.cTFBC1;
		else if (config.supportedCompressionFormats.pvrtc) format = fileInfo.hasAlpha ? _BASIS_FORMAT.cTFPVRTC1_4_RGBA : _BASIS_FORMAT.cTFPVRTC1_4_RGB;
		else if (config.supportedCompressionFormats.etc2) format = _BASIS_FORMAT.cTFETC2;
		else if (config.supportedCompressionFormats.etc1) format = _BASIS_FORMAT.cTFETC1;
		else format = _BASIS_FORMAT.cTFRGB565;
		return format;
	}
	/**
	...
	return new Promise((res, reject) => {
		const initHandler = (msg) => {
			if (msg.data.action === "init") {
				...
				res(worker);
			} else if (msg.data.action === "error") reject(msg.data.error || "error initializing worker");
		};
...
//#endregion
//#region node_modules/@babylonjs/core/Misc/basis.js
/**
...
var _CreateWorkerAsync = () => {
	if (!_WorkerPromise) _WorkerPromise = new Promise((res, reject) => {
		if (_Worker) res(_Worker);
		else Tools.LoadFileAsync(Tools.GetBabylonScriptURL(BasisToolsOptions.WasmModuleURL)).then((wasmBinary) => {
		...
		_CreateWorkerAsync().then(() => {
			...
			const messageHandler = (msg) => {
				if (msg.data.action === "transcode" && msg.data.id === actionId) {
					...
					if (!msg.data.success) rej("Transcode is not supported on this device");
					else res(msg.data);
				}
			};
...
var BindTexture = (texture, engine) => {
	...
	if (texture.isCube) target = engine._gl?.TEXTURE_CUBE_MAP;
...
//#endregion
//#region node_modules/@babylonjs/core/Materials/Textures/Loaders/basisTextureLoader.js
/**
...
//#endregion
export { BasisTranscodeConfiguration as a, SetBasisTranscoderWorker as c, BasisToolsOptions as i, TranscodeAsync as l, BasisFileInfo as n, GetInternalFormatFromBasisFormat as o, BasisTools as r, LoadTextureFromTranscodeResult as s, _BasisTextureLoader as t };

//# sourceMappingURL=basisTextureLoader-Bx95qFau.js.map