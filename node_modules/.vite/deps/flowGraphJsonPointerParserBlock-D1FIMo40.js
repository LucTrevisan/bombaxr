...
import { A as FlowGraphInteger, m as RichTypeFlowGraphInteger, u as RichTypeAny } from "./flowGraphBlock-Cu7z1xvN.js";
...
var pathHasTemplatesRegex = /* @__PURE__ */ new RegExp(/\/\{(\w+)\}\//g);
/**
* @experimental
	...
	constructor(path, ownerBlock) {
		...
		*/
		...
		while (match) {
			...
			if (templateSet.has(matchGroup)) throw new Error("Duplicate template variable detected.");
	...
	*/
	getAccessor(pathConverter, context) {
		...
		for (const templatedInput of this.templatedInputs) {
			...
			if (typeof valueToReplace !== "number" || valueToReplace < 0) throw new Error("Invalid value for templated input.");
...
//#endregion
//#region node_modules/@babylonjs/core/FlowGraph/Blocks/Data/Transformers/flowGraphJsonPointerParserBlock.js
/**
...
var FlowGraphJsonPointerParserBlock = class extends FlowGraphCachedOperationBlock {
	...
	_doOperation(context) {
		...
		if (!object) throw new Error("Object is undefined");
			...
			if (propertyName) this.propertyName.setValue(propertyName, context);
		}
		return value;
	}
	_setPropertyValue(_target, _propertyName, value, context) {
		...
		if (type.startsWith("Color")) value = ToColor(value, type);
	...
	_getInterpolationAnimationPropertyInfo(_target, _propertyName, context) {
		...
		return (keys, fps, animationType, easingFunction) => {
			const animations = [];
			const type = accessorContainer.info.type;
			if (type.startsWith("Color")) keys = keys.map((key) => {
				return {
					frame: key.frame,
			...
			accessorContainer.info.interpolation?.forEach((info, index) => {
				...
				let newKeys = keys;
				if (animationType !== info.type) newKeys = keys.map((key) => {
					return {
						frame: key.frame,
						value: info.getValue(void 0, key.value.asArray ? key.value.asArray() : [key.value], 0, 1)
					};
				});
				info.buildAnimations(accessorContainer.object, name, 60, newKeys).forEach((animation) => {
					if (easingFunction) animation.babylonAnimation.setEasingFunction(easingFunction);
...
function ToColor(value, expectedValue) {
	if (value.getClassName().startsWith("Color")) return value;
	if (expectedValue === "Color3") return new Color3(value.x, value.y, value.z);
	else if (expectedValue === "Color4") return new Color4(value.x, value.y, value.z, value.w);
	return value;
}
RegisterClass("FlowGraphJsonPointerParserBlock", FlowGraphJsonPointerParserBlock);
//#endregion
export { FlowGraphPathConverterComponent as n, FlowGraphJsonPointerParserBlock as t };

//# sourceMappingURL=flowGraphJsonPointerParserBlock-D1FIMo40.js.map