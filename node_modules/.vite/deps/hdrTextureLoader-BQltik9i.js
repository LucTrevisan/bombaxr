...
//#region node_modules/@babylonjs/core/Misc/HighDynamicRange/panoramaToCubemap.js
/**
...
var PanoramaToCubeMapTools = class {
	...
	*/
	static ConvertPanoramaToCubemap(float32Array, inputWidth, inputHeight, size, supersample = false) {
		if (!float32Array) throw "ConvertPanoramaToCubemap: input cannot be null";
		if (float32Array.length != inputWidth * inputHeight * 3) throw "ConvertPanoramaToCubemap: input size is wrong";
		return {
			front: this.CreateCubemapTexture(size, this.FACE_FRONT, float32Array, inputWidth, inputHeight, supersample),
			back: this.CreateCubemapTexture(size, this.FACE_BACK, float32Array, inputWidth, inputHeight, supersample),
			left: this.CreateCubemapTexture(size, this.FACE_LEFT, float32Array, inputWidth, inputHeight, supersample),
			right: this.CreateCubemapTexture(size, this.FACE_RIGHT, float32Array, inputWidth, inputHeight, supersample),
			up: this.CreateCubemapTexture(size, this.FACE_UP, float32Array, inputWidth, inputHeight, supersample),
			down: this.CreateCubemapTexture(size, this.FACE_DOWN, float32Array, inputWidth, inputHeight, supersample),
			size,
			type: 1,
			format: 4,
			gammaSpace: false
	...
	static CreateCubemapTexture(texSize, faceData, float32Array, inputWidth, inputHeight, supersample = false) {
		const buffer = /* @__PURE__ */ new ArrayBuffer(texSize * texSize * 4 * 3);
		...
		const dy = 1 / texSize;
		let fy = 0;
		for (let y = 0; y < texSize; y++) for (let sy = 0; sy < samples; sy++) {
			let xv1 = faceData[0];
			let xv2 = faceData[2];
			for (let x = 0; x < texSize; x++) for (let sx = 0; sx < samples; sx++) {
	...
	static CalcProjectionSpherical(vDir, float32Array, inputWidth, inputHeight) {
		...
		return {
			r: float32Array[inputY * inputWidth * 3 + px * 3 + 0],
			g: float32Array[inputY * inputWidth * 3 + px * 3 + 1],
			b: float32Array[inputY * inputWidth * 3 + px * 3 + 2]
		};
	}
};
...
function GetCubeMapTextureData(buffer, size, supersample = false) {
	...
	return PanoramaToCubeMapTools.ConvertPanoramaToCubemap(data, hdrInfo.width, hdrInfo.height, size, supersample);
}
/**
...
function readRGBEPixelsRLE(uint8array, hdrInfo) {
	...
	const scanLineArrayBuffer = /* @__PURE__ */ new ArrayBuffer(scanline_width * 4);
	...
	while (num_scanlines > 0) {
		...
		if ((c << 8 | d) != scanline_width) throw "HDR Bad header format, wrong scan line width";
		index = 0;
		for (i = 0; i < 4; i++) {
...
//#endregion
export { PanoramaToCubeMapTools as i, GetCubeMapTextureData as n, HDRTools as r, _HDRTextureLoader as t };

//# sourceMappingURL=hdrTextureLoader-BQltik9i.js.map