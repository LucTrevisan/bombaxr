...
import { d as RichTypeBoolean, u as RichTypeAny, v as RichTypeNumber } from "./flowGraphBlock-Cu7z1xvN.js";
...
var FlowGraphPlayAnimationBlock = class extends FlowGraphAsyncExecutionBlock {
	...
	*/
	_preparePendingTasks(context) {
		...
		if (!ag && !animation) return this._reportError(context, "No animation or animation group provided");
		else {
			const currentAnimationGroup = this.currentAnimationGroup.getValue(context);
			if (currentAnimationGroup && currentAnimationGroup !== ag) currentAnimationGroup.dispose();
			let animationGroupToUse = ag;
			if (animation && !animationGroupToUse) {
				const target = this.object.getValue(context);
				if (!target) return this._reportError(context, "No target object provided");
				...
				for (const anim of animationsArray) {
					...
					if (interpolationAnimations.indexOf(anim.uniqueId) !== -1) isInterpolation = true;
				}
				if (isInterpolation) this._checkInterpolationDuplications(context, animationsArray, target);
			}
			...
			if (currentlyRunningAnimationGroups.indexOf(animationGroupToUse.uniqueId) !== -1) animationGroupToUse.stop();
			try {
	...
	*/
	_checkInterpolationDuplications(context, animation, target) {
		...
		for (const uniqueId of currentlyRunningAnimationGroups) {
			...
			if (ag) {
				for (const anim of ag.targetedAnimations) for (const animToCheck of animation) if (anim.animation.targetProperty === animToCheck.targetProperty && anim.target === target) this._stopAnimationGroup(context, ag);
			}
		}
	}
	...
	*/
	_cancelPendingTasks(context) {
		...
		if (ag) this._stopAnimationGroup(context, ag);
	}
	/**
...
//#endregion
export { FlowGraphPlayAnimationBlock as t };

//# sourceMappingURL=flowGraphPlayAnimationBlock-BnvVjn7C.js.map