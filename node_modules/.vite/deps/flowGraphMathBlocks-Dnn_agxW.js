import { a as Vector3, i as Vector2, n as Quaternion, o as Vector4, t as Matrix } from "./math.vector-Clv1Vglo.js";
import { r as RegisterClass } from "./typeStore-DHv0TG1U.js";
import { A as FlowGraphInteger, E as getRichTypeByFlowGraphType, O as FlowGraphMatrix2D, d as RichTypeBoolean, k as FlowGraphMatrix3D, m as RichTypeFlowGraphInteger, u as RichTypeAny, v as RichTypeNumber } from "./flowGraphBlock-Cu7z1xvN.js";
import { i as _getClassNameOf, n as _areSameMatrixClass, o as getNumericValue, r as _areSameVectorClass, s as isNumeric, t as _areSameIntegerClass } from "./utils-IXO60KEn.js";
import { t as FlowGraphCachedOperationBlock } from "./flowGraphCachedOperationBlock-BR556jo7.js";
import { t as FlowGraphBinaryOperationBlock } from "./flowGraphBinaryOperationBlock-UiG1YLBo.js";
import { t as FlowGraphUnaryOperationBlock } from "./flowGraphUnaryOperationBlock-UGx6m5RG.js";
import { t as FlowGraphTernaryOperationBlock } from "./flowGraphTernaryOperationBlock-BdTFVMqe.js";
...
//#endregion
//#region node_modules/@babylonjs/core/FlowGraph/Blocks/Data/Math/flowGraphMathBlocks.js
/**
...
var FlowGraphAddBlock = class extends FlowGraphBinaryOperationBlock {
	...
	*/
	...
	_polymorphicAdd(a, b) {
		...
		if (_areSameVectorClass(aClassName, bClassName) || _areSameMatrixClass(aClassName, bClassName) || _areSameIntegerClass(aClassName, bClassName)) return a.add(b);
		else if (aClassName === "Quaternion" || bClassName === "Quaternion") return a.add(b);
		else return a + b;
	}
};
...
var FlowGraphSubtractBlock = class extends FlowGraphBinaryOperationBlock {
	...
	*/
	...
	_polymorphicSubtract(a, b) {
		const aClassName = _getClassNameOf(a);
		const bClassName = _getClassNameOf(b);
		if (_areSameVectorClass(aClassName, bClassName) || _areSameIntegerClass(aClassName, bClassName) || _areSameMatrixClass(aClassName, bClassName)) return a.subtract(b);
		else if (aClassName === "Quaternion" || bClassName === "Quaternion") return a.subtract(b);
		else return a - b;
	}
};
...
var FlowGraphMultiplyBlock = class extends FlowGraphBinaryOperationBlock {
	...
	_polymorphicMultiply(a, b) {
		const aClassName = _getClassNameOf(a);
		const bClassName = _getClassNameOf(b);
		if (_areSameVectorClass(aClassName, bClassName) || _areSameIntegerClass(aClassName, bClassName)) return a.multiply(b);
			...
			aClone.w *= b.w;
			return aClone;
		} else if (_areSameMatrixClass(aClassName, bClassName)) if (this.config?.useMatrixPerComponent) {
			const aM = a.m;
			for (let i = 0; i < aM.length; i++) aM[i] *= b.m[i];
			if (aClassName === "Matrix2D") return new FlowGraphMatrix2D(aM);
			else if (aClassName === "Matrix3D") return new FlowGraphMatrix3D(aM);
			else return Matrix.FromArray(aM);
		} else {
			a = a;
			b = b;
			return b.multiply(a);
...
var FlowGraphDivideBlock = class extends FlowGraphBinaryOperationBlock {
	...
	*/
	...
	_polymorphicDivide(a, b) {
		const aClassName = _getClassNameOf(a);
		const bClassName = _getClassNameOf(b);
		if (_areSameVectorClass(aClassName, bClassName) || _areSameIntegerClass(aClassName, bClassName)) return a.divide(b);
		else if (aClassName === "Quaternion" || bClassName === "Quaternion") {
			const aClone = a.clone();
			aClone.x /= b.x;
			aClone.y /= b.y;
...
var FlowGraphRandomBlock = class extends FlowGraphConstantOperationBlock {
	...
	*/
	constructor(config) {
		...
		if (config?.seed) this._seed = config.seed;
	}
...
var FlowGraphNaNBlock = class extends FlowGraphConstantOperationBlock {
	constructor(config) {
		super(RichTypeNumber, () => NaN, "FlowGraphNaNBlock", config);
	}
};
...
function _componentWiseTernaryOperation(a, b, c, op) {
	switch (_getClassNameOf(a)) {
		...
		case "Matrix": return Matrix.FromArray(a.m.map((v, i) => op(v, b.m[i], c.m[i])));
		case "Matrix2D": return new FlowGraphMatrix2D(a.m.map((v, i) => op(v, b.m[i], c.m[i])));
		case "Matrix3D": return new FlowGraphMatrix3D(a.m.map((v, i) => op(v, b.m[i], c.m[i])));
		default: return op(a, b, c);
	}
}
/**
* Clamp block.
...
var FlowGraphEqualityBlock = class extends FlowGraphBinaryOperationBlock {
	...
	_polymorphicEq(a, b) {
		const aClassName = _getClassNameOf(a);
		const bClassName = _getClassNameOf(b);
		if (_areSameVectorClass(aClassName, bClassName) || _areSameMatrixClass(aClassName, bClassName) || _areSameIntegerClass(aClassName, bClassName)) return a.equals(b);
		else return a === b;
	}
};
RegisterClass("FlowGraphEqualityBlock", FlowGraphEqualityBlock);
function _comparisonOperators(a, b, op) {
	if (isNumeric(a) && isNumeric(b)) return op(getNumericValue(a), getNumericValue(b));
	else throw new Error(`Cannot compare ${a} and ${b}`);
}
/**
* Less than block.
*/
...
var FlowGraphIsNanBlock = class extends FlowGraphUnaryOperationBlock {
	...
	_polymorphicIsNan(a) {
		if (isNumeric(a)) return isNaN(getNumericValue(a));
		else throw new Error(`Cannot get NaN of ${a}`);
	}
};
...
var FlowGraphIsInfinityBlock = class extends FlowGraphUnaryOperationBlock {
	...
	_polymorphicIsInf(a) {
		if (isNumeric(a)) return !isFinite(getNumericValue(a));
		else throw new Error(`Cannot get isInf of ${a}`);
	}
};
...
var FlowGraphBitwiseNotBlock = class extends FlowGraphUnaryOperationBlock {
	constructor(config) {
		super(getRichTypeByFlowGraphType(config?.valueType || "FlowGraphInteger"), getRichTypeByFlowGraphType(config?.valueType || "FlowGraphInteger"), (a) => {
...
var FlowGraphBitwiseAndBlock = class extends FlowGraphBinaryOperationBlock {
	constructor(config) {
		super(getRichTypeByFlowGraphType(config?.valueType || "FlowGraphInteger"), getRichTypeByFlowGraphType(config?.valueType || "FlowGraphInteger"), getRichTypeByFlowGraphType(config?.valueType || "FlowGraphInteger"), (a, b) => {
			if (typeof a === "boolean" && typeof b === "boolean") return a && b;
			else if (typeof a === "number" && typeof b === "number") return a & b;
			else if (typeof a === "object" && typeof b === "object") return new FlowGraphInteger(a.value & b.value);
			else throw new Error(`Cannot perform bitwise AND on ${a} and ${b}`);
...
var FlowGraphBitwiseOrBlock = class extends FlowGraphBinaryOperationBlock {
	constructor(config) {
		super(getRichTypeByFlowGraphType(config?.valueType || "FlowGraphInteger"), getRichTypeByFlowGraphType(config?.valueType || "FlowGraphInteger"), getRichTypeByFlowGraphType(config?.valueType || "FlowGraphInteger"), (a, b) => {
			...
			else throw new Error(`Cannot perform bitwise OR on ${a} and ${b}`);
...
var FlowGraphBitwiseXorBlock = class extends FlowGraphBinaryOperationBlock {
	constructor(config) {
		super(getRichTypeByFlowGraphType(config?.valueType || "FlowGraphInteger"), getRichTypeByFlowGraphType(config?.valueType || "FlowGraphInteger"), getRichTypeByFlowGraphType(config?.valueType || "FlowGraphInteger"), (a, b) => {
			...
			else throw new Error(`Cannot perform bitwise XOR on ${a} and ${b}`);
...
//#endregion
export { FlowGraphSignBlock as $, FlowGraphInfBlock as A, FlowGraphMaxBlock as B, FlowGraphEBlock as C, FlowGraphFractionBlock as D, FlowGraphFloorBlock as E, FlowGraphLessThanOrEqualBlock as F, FlowGraphNegationBlock as G, FlowGraphModuloBlock as H, FlowGraphLog10Block as I, FlowGraphPowerBlock as J, FlowGraphOneBitsCounterBlock as K, FlowGraphLog2Block as L, FlowGraphIsNanBlock as M, FlowGraphLeadingZerosBlock as N, FlowGraphGreaterThanBlock as O, FlowGraphLessThanBlock as P, FlowGraphSaturateBlock as Q, FlowGraphLogBlock as R, FlowGraphDivideBlock as S, FlowGraphExpBlock as T, FlowGraphMultiplyBlock as U, FlowGraphMinBlock as V, FlowGraphNaNBlock as W, FlowGraphRandomBlock as X, FlowGraphRadToDegBlock as Y, FlowGraphRoundBlock as Z, FlowGraphClampBlock as _, FlowGraphAsinBlock as a, FlowGraphTanhBlock as at, FlowGraphCubeRootBlock as b, FlowGraphAtanBlock as c, FlowGraphBitwiseLeftShiftBlock as d, FlowGraphSinBlock as et, FlowGraphBitwiseNotBlock as f, FlowGraphCeilBlock as g, FlowGraphBitwiseXorBlock as h, FlowGraphAddBlock as i, FlowGraphTanBlock as it, FlowGraphIsInfinityBlock as j, FlowGraphGreaterThanOrEqualBlock as k, FlowGraphAtanhBlock as l, FlowGraphBitwiseRightShiftBlock as m, FlowGraphAcosBlock as n, FlowGraphSquareRootBlock as nt, FlowGraphAsinhBlock as o, FlowGraphTrailingZerosBlock as ot, FlowGraphBitwiseOrBlock as p, FlowGraphPiBlock as q, FlowGraphAcoshBlock as r, FlowGraphSubtractBlock as rt, FlowGraphAtan2Block as s, FlowGraphTruncBlock as st, FlowGraphAbsBlock as t, FlowGraphSinhBlock as tt, FlowGraphBitwiseAndBlock as u, FlowGraphCosBlock as v, FlowGraphEqualityBlock as w, FlowGraphDegToRadBlock as x, FlowGraphCoshBlock as y, FlowGraphMathInterpolationBlock as z };

//# sourceMappingURL=flowGraphMathBlocks-Dnn_agxW.js.map