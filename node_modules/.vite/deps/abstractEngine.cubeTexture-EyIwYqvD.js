...
import { u as LoadImage } from "./instantiationTools-4RwH0ysD.js";
import { c as _GetCompatibleTextureLoader, d as InternalTexture, o as AbstractEngine } from "./tools.functions-Bl8BhFJU.js";
...
//#endregion
//#region node_modules/@babylonjs/core/Engines/AbstractEngine/abstractEngine.cubeTexture.js
AbstractEngine.prototype._partialLoadFile = function(url, index, loadedFiles, onfinish, onErrorCallBack = null) {
	const onload = (data) => {
		loadedFiles[index] = data;
		loadedFiles._internalCount++;
		if (loadedFiles._internalCount === 6) onfinish(loadedFiles);
	};
	const onerror = (request, exception) => {
		if (onErrorCallBack && request) onErrorCallBack(request.status + " " + request.statusText, exception);
	};
...
AbstractEngine.prototype._partialLoadImg = function(url, index, loadedImages, scene, texture, onfinish, onErrorCallBack = null, mimeType) {
	...
	const onload = (img) => {
		...
		if (scene) scene.removePendingData(tokenPendingData);
		if (loadedImages._internalCount === 6 && onfinish) onfinish(texture, loadedImages);
	};
	const onerror = (message, exception) => {
		if (scene) scene.removePendingData(tokenPendingData);
		if (onErrorCallBack) onErrorCallBack(message, exception);
	};
	LoadImage(url, onload, onerror, scene ? scene.offlineProvider : null, mimeType);
	if (scene) scene.addPendingData(tokenPendingData);
};
AbstractEngine.prototype.createCubeTextureBase = function(rootUrl, scene, files, noMipmap, onLoad = null, onError = null, format, forcedExtension = null, createPolynomials = false, lodScale = 0, lodOffset = 0, fallback = null, beforeLoadCubeDataCallback = null, imageHandler = null, useSRGBBuffer = false, buffer = null) {
	...
	if (texture !== fallback) texture.label = rootUrl.substring(0, 60);
	...
	if (this._transformTextureUrl && !fallback) rootUrl = this._transformTextureUrl(rootUrl);
	const loaderPromise = _GetCompatibleTextureLoader(forcedExtension ?? GetExtensionFromUrl(rootUrl));
	...
	if (loaderPromise) loaderPromise.then((loader) => {
		const onloaddata = (data) => {
			if (beforeLoadCubeDataCallback) beforeLoadCubeDataCallback(texture, data);
			loader.loadCubeData(data, texture, createPolynomials, onLoad, onError);
		};
		if (buffer) onloaddata(buffer);
		else if (files && files.length === 6) if (loader.supportCascades) this._cascadeLoadFiles(scene, (images) => onloaddata(images.map((image) => new Uint8Array(image))), files, onError);
		else if (onError) onError("Textures type does not support cascades.");
		else Logger.Warn("Texture loader does not support cascades.");
		else this._loadFile(rootUrl, (data) => onloaddata(new Uint8Array(data)), void 0, void 0, true, onInternalError);
	});
	else {
		...
		this._cascadeLoadImgs(scene, texture, (texture, imgs) => {
			if (imageHandler) imageHandler(texture, imgs);
		}, files, onError);
	}
...
//#endregion
export { GetExtensionFromUrl as t };

//# sourceMappingURL=abstractEngine.cubeTexture-EyIwYqvD.js.map