import { i as AbstractSound, n as _FileExtensionRegex, r as _AbstractSoundInstance, t as _CleanUrl } from "./audioUtils-DASASPG-.js";
import { a as _StereoAudio, r as _WebAudioBusAndSoundSubGraph, t as _SpatialWebAudio, u as _HasSpatialAudioOptions } from "./spatialWebAudio-DxwvNLKi.js";
//#region node_modules/@babylonjs/core/AudioV2/abstractAudio/staticSound.js
/**
...
var StaticSound = class extends AbstractSound {
	...
	*/
	...
	set pitch(value) {
		...
		for (let instance = it.next(); !instance.done; instance = it.next()) instance.value.pitch = value;
	}
	/**
	...
	*/
	...
	set playbackRate(value) {
		...
		const it = this._instances.values();
		for (let instance = it.next(); !instance.done; instance = it.next()) instance.value.playbackRate = value;
	}
	/**
	* Plays the sound.
	...
	*/
	stop(options = {}) {
		if (options.waitTime && 0 < options.waitTime) this._setState(0);
		else this._setState(1);
		if (!this._instances) return;
		for (const instance of Array.from(this._instances)) instance.stop(options);
	}
};
//#endregion
//#region node_modules/@babylonjs/core/AudioV2/abstractAudio/staticSoundBuffer.js
/**
...
//#endregion
//#region node_modules/@babylonjs/core/AudioV2/abstractAudio/staticSoundInstance.js
/** @internal */
var _StaticSoundInstance = class extends _AbstractSoundInstance {};
//#endregion
//#region node_modules/@babylonjs/core/AudioV2/webAudio/webAudioStaticSound.js
/** @internal */
var _WebAudioStaticSound = class _WebAudioStaticSound extends StaticSound {
	/** @internal */
	...
	async _init(source, options) {
		...
		if (source instanceof _WebAudioStaticSoundBuffer) this._buffer = source;
		else if (typeof source === "string" || Array.isArray(source) || source instanceof ArrayBuffer || source instanceof AudioBuffer) this._buffer = await this.engine.createSoundBufferAsync(source, options);
		if (options.outBus) this.outBus = options.outBus;
		else {
			await this.engine.isReadyPromise;
			this.outBus = this.engine.defaultMainBus;
		}
		await this._subGraph.init(options);
		if (_HasSpatialAudioOptions(options)) this._initSpatialProperty();
	...
	_connect(node) {
		...
		if (node._inNode) this._outNode?.connect(node._inNode);
		return true;
	}
	_disconnect(node) {
		if (!super._disconnect(node)) return false;
		if (node._inNode) this._outNode?.disconnect(node._inNode);
		return true;
	}
	_initSpatialProperty() {
		if (!this._spatial) this._spatial = new _SpatialWebAudio(this._subGraph, this._spatialAutoUpdate, this._spatialMinUpdateTime);
...
var _WebAudioStaticSoundBuffer = class extends StaticSoundBuffer {
	...
	async _init(source, options) {
		if (source instanceof AudioBuffer) this._audioBuffer = source;
		else if (typeof source === "string") await this._initFromUrl(source);
		else if (Array.isArray(source)) await this._initFromUrls(source, options.skipCodecCheck ?? false);
		else if (source instanceof ArrayBuffer) await this._initFromArrayBuffer(source);
	}
	/** @internal */
	get channelCount() {
	...
	async _initFromUrls(urls, skipCodecCheck) {
		for (const url of urls) {
			if (skipCodecCheck) await this._initFromUrl(url);
			else {
				const format = url.match(_FileExtensionRegex)?.at(1);
				if (format && this.engine.isFormatValid(format)) try {
					await this._initFromUrl(url);
				} catch (e) {
					if (format && 0 < format.length) this.engine.flagInvalidFormat(format);
				}
			}
			if (this._audioBuffer) break;
		}
	}
};
/** @internal */
var _WebAudioStaticSoundInstance = class extends _StaticSoundInstance {
	constructor(sound, options) {
		...
		this._onEngineStateChanged = () => {
			...
			if (this._options.loop && this.state === 2) this.play();
	...
	/** @internal */
	set pitch(value) {
		if (this._sourceNode) this.engine._setAudioParam(this._sourceNode.detune, value);
	}
	/** @internal */
	set playbackRate(value) {
		if (this._sourceNode) this.engine._setAudioParam(this._sourceNode.playbackRate, value);
	}
	/** @internal */
	get startTime() {
		if (this._state === 1) return 0;
	...
	play(options = {}) {
		...
		if (options.duration !== void 0) this._options.duration = options.duration;
		if (options.loop !== void 0) this._options.loop = options.loop;
		if (options.loopStart !== void 0) this._options.loopStart = options.loopStart;
		if (options.loopEnd !== void 0) this._options.loopEnd = options.loopEnd;
		if (options.startOffset !== void 0) this._options.startOffset = options.startOffset;
	...
	_connect(node) {
		if (!super._connect(node)) return false;
		if (node instanceof _WebAudioStaticSound && node._inNode) this._outNode?.connect(node._inNode);
		return true;
	}
	_disconnect(node) {
		if (!super._disconnect(node)) return false;
		if (node instanceof _WebAudioStaticSound && node._inNode) this._outNode?.disconnect(node._inNode);
		return true;
	}
	_deinitSourceNode() {
		if (!this._sourceNode) return;
		if (!this._disconnect(this._sound)) throw new Error("Disconnect failed");
	...
	_initSourceNode() {
		if (!this._sourceNode) {
			...
			if (!this._connect(this._sound)) throw new Error("Connect failed");
		}
...
//#endregion
export { StaticSound as i, _WebAudioStaticSoundBuffer as n, StaticSoundBuffer as r, _WebAudioStaticSound as t };

//# sourceMappingURL=webAudioStaticSound-CTq1IyZe.js.map