...
import { a as Vector3, i as Vector2, n as Quaternion, o as Vector4, r as TmpVectors, t as Matrix } from "./math.vector-Clv1Vglo.js";
...
import { t as Bone } from "./bone-DqhQ2Sme.js";
import { A as RuntimeError, k as ErrorCodes, l as LoadFileError, n as DecodeBase64UrlToBinary, o as IsBase64DataUrl } from "./instantiationTools-4RwH0ysD.js";
import { v as Effect } from "./tools.functions-Bl8BhFJU.js";
import { n as RandomGUID } from "./guid-C8GtW-rp.js";
import { l as GetTypedArrayConstructor, n as VertexBuffer, t as Buffer } from "./buffer-C0-cMmV_.js";
import { B as ParseFlowGraphAsync, G as addToBlockFactory, P as DirectionalLight, a as DracoDecoder, at as WeightedSound, bt as AnimationEvent, d as RawCubeTexture, f as MorphTargetManager, g as PBRMaterial, gt as AssetContainer, h as PointLight, i as MeshoptCompression, m as MorphTarget, n as Deferred, pt as Sound, q as FreeCamera, r as deepMerge, rt as Skeleton, t as DataReader } from "./dataReader-DDgCIptM.js";
import { H as Geometry, K as SubMesh, L as MultiMaterial, N as Mesh, R as Material, V as TransformNode, W as VertexData, Z as Camera, _ as StandardMaterial, g as ShaderMaterial, h as HemisphericLight, q as BoundingInfo, s as RegisterSceneLoaderPlugin, z as AbstractMesh } from "./sceneLoader-Ctf8smxJ.js";
import { t as Texture } from "./texture-BC46D7N_.js";
import { n as Light, t as Constants } from "./constants-DXxR-C23.js";
import { T as SphericalPolynomial, c as RenderTargetTexture, w as SphericalHarmonics } from "./cubemapToSphericalPolynomial-CYDqs8Y8.js";
import { w as getAnimationTypeByFlowGraphType } from "./flowGraphBlock-Cu7z1xvN.js";
import { t as FlowGraphCoordinator } from "./flowGraphCoordinator-C6XHDW8I.js";
import { t as SpotLight } from "./spotLight-B4p4OWrG.js";
import { a as getVector3, c as GetMappingForKey, i as getQuaternion, l as GetPathToObjectConverter, n as TransformNodeAnimationPropertyInfo, o as getWeights, r as WeightAnimationPropertyInfo, s as AddObjectAccessorToKey, t as AnimationPropertyInfo, u as SetInterpolationForKey } from "./glTFLoaderAnimation-WufOmVm2.js";
import { t as FlowGraphGLTFDataProvider } from "./flowGraphGLTFDataProvider-AEcQUfes.js";
//#region node_modules/@babylonjs/loaders/glTF/glTFValidation.js
function validateAsync(data, rootUrl, fileName, getExternalResource) {
	const options = { externalResourceFunction: getExternalResource };
	if (fileName) options.uri = rootUrl === "file:" ? fileName : rootUrl + fileName;
	...
	*/
	static ValidateAsync(data, rootUrl, fileName, getExternalResource) {
		if (typeof Worker === "function") return new Promise((resolve, reject) => {
			...
			if (!this._LoadScriptPromise) this._LoadScriptPromise = Tools.LoadBabylonScriptAsync(this.Configuration.url);
...
*/
GLTFValidation.Configuration = { url: `${Tools._DefaultCdnUrl}/gltf_validator.js` };
var GLTFFileLoaderMetadata = {
	...
	canDirectLoad(data) {
		return data.indexOf("asset") !== -1 && data.indexOf("version") !== -1 || data.startsWith("data:base64,Z2xURg") || data.startsWith("data:;base64,Z2xURg") || data.startsWith("data:application/octet-stream;base64,Z2xURg") || data.startsWith("data:model/gltf-binary;base64,Z2xURg");
	}
};
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/glTFFileLoader.js
...
function readViewAsync(arrayBufferView, byteOffset, byteLength) {
	try {
		if (byteOffset < 0 || byteOffset >= arrayBufferView.byteLength) throw new RangeError("Offset is out of range.");
		if (byteOffset + byteLength > arrayBufferView.byteLength) throw new RangeError("Length is out of range.");
...
*/
var GLTFFileLoader = class GLTFFileLoader extends GLTFLoaderOptions {
	/**
	...
	*/
	set onParsed(callback) {
		if (this._onParsedObserver) this.onParsedObservable.remove(this._onParsedObserver);
		if (callback) this._onParsedObserver = this.onParsedObservable.add(callback);
	}
	/**
	...
	*/
	set onMeshLoaded(callback) {
		if (this._onMeshLoadedObserver) this.onMeshLoadedObservable.remove(this._onMeshLoadedObserver);
		if (callback) this._onMeshLoadedObserver = this.onMeshLoadedObservable.add(callback);
	}
	/**
	...
	*/
	set onSkinLoaded(callback) {
		if (this._onSkinLoadedObserver) this.onSkinLoadedObservable.remove(this._onSkinLoadedObserver);
		if (callback) this._onSkinLoadedObserver = this.onSkinLoadedObservable.add((data) => callback(data.node, data.skinnedNode));
	}
	/**
	* Callback raised when the loader creates a texture after parsing the glTF properties of the texture.
	*/
	set onTextureLoaded(callback) {
		if (this._onTextureLoadedObserver) this.onTextureLoadedObservable.remove(this._onTextureLoadedObserver);
		if (callback) this._onTextureLoadedObserver = this.onTextureLoadedObservable.add(callback);
	}
	/**
	* Callback raised when the loader creates a material after parsing the glTF properties of the material.
	*/
	set onMaterialLoaded(callback) {
		if (this._onMaterialLoadedObserver) this.onMaterialLoadedObservable.remove(this._onMaterialLoadedObserver);
		if (callback) this._onMaterialLoadedObserver = this.onMaterialLoadedObservable.add(callback);
	}
	/**
	* Callback raised when the loader creates a camera after parsing the glTF properties of the camera.
	*/
	set onCameraLoaded(callback) {
		if (this._onCameraLoadedObserver) this.onCameraLoadedObservable.remove(this._onCameraLoadedObserver);
		if (callback) this._onCameraLoadedObserver = this.onCameraLoadedObservable.add(callback);
	}
	/**
	...
	*/
	set onComplete(callback) {
		if (this._onCompleteObserver) this.onCompleteObservable.remove(this._onCompleteObserver);
	...
	*/
	set onError(callback) {
		if (this._onErrorObserver) this.onErrorObservable.remove(this._onErrorObserver);
	...
	*/
	set onExtensionLoaded(callback) {
		if (this._onExtensionLoadedObserver) this.onExtensionLoadedObservable.remove(this._onExtensionLoadedObserver);
	...
	*/
	...
	set loggingEnabled(value) {
		if (this._loggingEnabled === value) return;
		this._loggingEnabled = value;
		if (this._loggingEnabled) this._log = this._logEnabled;
		else this._log = this._logDisabled;
	}
	/**
	...
	*/
	...
	set capturePerformanceCounters(value) {
		if (this._capturePerformanceCounters === value) return;
	...
	*/
	set onValidated(callback) {
		if (this._onValidatedObserver) this.onValidatedObservable.remove(this._onValidatedObserver);
	...
	loadFile(scene, fileOrUrl, rootUrl, onSuccess, onProgress, useArrayBuffer, onError, name) {
		...
		if (useArrayBuffer) {
			if (this.useRangeRequests) {
				if (this.validate) Logger.Warn("glTF validation is not supported when range requests are enabled");
				const fileRequest = {
					abort: () => {},
		...
		} else return this._loadFile(scene, fileOrUrl, (data) => {
			try {
	...
	loadAssetContainerAsync(scene, data, rootUrl, onProgress, fileName) {
		return Promise.resolve().then(() => {
			...
			this.onMeshLoadedObservable.add((mesh) => {
				if (mesh.morphTargetManager) morphTargetManagers.push(mesh.morphTargetManager);
			});
	...
	*/
	directLoad(scene, data) {
		if (data.startsWith("base64,Z2xURg") || data.startsWith(";base64,Z2xURg") || data.startsWith("application/octet-stream;base64,Z2xURg") || data.startsWith("model/gltf-binary;base64,Z2xURg")) {
	...
	*/
	...
	_onProgress(event, request) {
		...
		for (const request of this._requests) {
			if (request._lengthComputable === void 0 || request._loaded === void 0 || request._total === void 0) return;
	...
	_validate(scene, data, rootUrl = "", fileName = "") {
		...
		GLTFValidation.ValidateAsync(data, rootUrl, fileName, (uri) => {
			return this.preprocessUrlAsync(rootUrl + uri).then((url) => {
				return scene._loadFileAsync(url, void 0, true, true).then((data) => {
	...
	_getLoader(loaderData) {
		...
		if (!version) throw new Error("Invalid version: " + asset.version);
		if (asset.minVersion !== void 0) {
			const minVersion = GLTFFileLoader._parseVersion(asset.minVersion);
			if (!minVersion) throw new Error("Invalid minimum version: " + asset.minVersion);
			if (GLTFFileLoader._compareVersion(minVersion, {
				major: 2,
				minor: 0
			}) > 0) throw new Error("Incompatible minimum version: " + asset.minVersion);
		}
		const createLoader = {
			1: GLTFFileLoader._CreateGLTF1Loader,
			2: GLTFFileLoader._CreateGLTF2Loader
		}[version.major];
		if (!createLoader) throw new Error("Unsupported version: " + asset.version);
	...
	_unpackBinaryAsync(dataReader) {
		...
		return dataReader.loadAsync(20).then(() => {
			const Binary = { Magic: 1179937895 };
			const magic = dataReader.readUint32();
			if (magic !== Binary.Magic) throw new RuntimeError("Unexpected magic: " + magic, ErrorCodes.GLTFLoaderUnexpectedMagicError);
			const version = dataReader.readUint32();
			if (this.loggingEnabled) this._log(`Binary version: ${version}`);
			const length = dataReader.readUint32();
			if (!this.useRangeRequests && length !== dataReader.buffer.byteLength) Logger.Warn(`Length in header does not match actual data length: ${length} != ${dataReader.buffer.byteLength}`);
			let unpacked;
			switch (version) {
				case 1:
	...
	_unpackBinaryV1Async(dataReader, length) {
		const ContentFormat = { JSON: 0 };
		const contentLength = dataReader.readUint32();
		const contentFormat = dataReader.readUint32();
		if (contentFormat !== ContentFormat.JSON) throw new Error(`Unexpected content format: ${contentFormat}`);
	...
	_unpackBinaryV2Async(dataReader, length) {
		...
		if (dataReader.readUint32() !== ChunkFormat.JSON) throw new Error("First chunk format is not JSON");
		if (dataReader.byteOffset + chunkLength === length) return dataReader.loadAsync(chunkLength).then(() => {
			return {
				json: this._parseJson(dataReader.readString(chunkLength)),
				bin: null
			};
		});
		return dataReader.loadAsync(chunkLength + 8).then(() => {
			...
			const readAsync = () => {
				...
				switch (dataReader.readUint32()) {
					case ChunkFormat.JSON: throw new Error("Unexpected JSON chunk");
				...
				if (dataReader.byteOffset !== length) return dataReader.loadAsync(8).then(readAsync);
				return Promise.resolve(data);
			};
			return readAsync();
		});
	...
	static _parseVersion(version) {
		if (version === "1.0" || version === "1.0.1") return {
			major: 1,
			minor: 0
		};
...
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/1.0/glTFLoaderInterfaces.js
/**
* Enums
* @internal
*/
...
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/1.0/glTFLoaderUtils.js
/**
* Utils functions for GLTF
* @internal
* @deprecated
*/
var GLTFUtils = class GLTFUtils {
	/**
	...
	*/
	static SetMatrix(scene, source, parameter, uniformName, shaderMaterial) {
		let mat = null;
		if (parameter.semantic === "MODEL") mat = source.getWorldMatrix();
		else if (parameter.semantic === "PROJECTION") mat = scene.getProjectionMatrix();
		else if (parameter.semantic === "VIEW") mat = scene.getViewMatrix();
		else if (parameter.semantic === "MODELVIEWINVERSETRANSPOSE") mat = Matrix.Transpose(source.getWorldMatrix().multiply(scene.getViewMatrix()).invert());
		else if (parameter.semantic === "MODELVIEW") mat = source.getWorldMatrix().multiply(scene.getViewMatrix());
		else if (parameter.semantic === "MODELVIEWPROJECTION") mat = source.getWorldMatrix().multiply(scene.getTransformMatrix());
		else if (parameter.semantic === "MODELINVERSE") mat = source.getWorldMatrix().invert();
		else if (parameter.semantic === "VIEWINVERSE") mat = scene.getViewMatrix().invert();
		else if (parameter.semantic === "PROJECTIONINVERSE") mat = scene.getProjectionMatrix().invert();
		else if (parameter.semantic === "MODELVIEWINVERSE") mat = source.getWorldMatrix().multiply(scene.getViewMatrix()).invert();
		else if (parameter.semantic === "MODELVIEWPROJECTIONINVERSE") mat = source.getWorldMatrix().multiply(scene.getTransformMatrix()).invert();
		else if (parameter.semantic === "MODELINVERSETRANSPOSE") mat = Matrix.Transpose(source.getWorldMatrix().invert());
		if (mat) switch (parameter.type) {
	...
	*/
	...
	static GetBufferFromBufferView(gltfRuntime, bufferView, byteOffset, byteLength, componentType) {
		...
		if (byteOffset + byteLength > loadedBufferView.byteLength) throw new Error("Buffer access is out of range");
	...
	*/
	static DecodeBufferToText(view) {
		...
		for (let i = 0; i < length; ++i) result += String.fromCharCode(view[i]);
		return result;
	}
	/**
	...
	*/
	static GetDefaultMaterial(scene) {
		if (!GLTFUtils._DefaultMaterial) {
			...
			GLTFUtils._DefaultMaterial = new ShaderMaterial("GLTFDefaultMaterial", scene, {
			...
			GLTFUtils._DefaultMaterial.setColor4("u_emission", new Color4(.5, .5, .5, 1));
		}
...
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/1.0/glTFLoader.js
/**
...
var glTFAnimationPaths = [
	"translation",
	"rotation",
	"scale"
];
var babylonAnimationPaths = [
	"position",
	"rotationQuaternion",
	"scaling"
];
...
var normalizeUVs = (buffer) => {
	if (!buffer) return;
	for (let i = 0; i < buffer.length / 2; i++) buffer[i * 2 + 1] = 1 - buffer[i * 2 + 1];
};
var getAttribute = (attributeParameter) => {
	if (attributeParameter.semantic === "NORMAL") return "normal";
	else if (attributeParameter.semantic === "POSITION") return "position";
	else if (attributeParameter.semantic === "JOINT") return "matricesIndices";
	else if (attributeParameter.semantic === "WEIGHT") return "matricesWeights";
	else if (attributeParameter.semantic === "COLOR") return "color";
...
var loadAnimations = (gltfRuntime) => {
	for (const anim in gltfRuntime.animations) {
		...
		if (!animation.channels || !animation.samplers) continue;
		...
		for (let i = 0; i < animation.channels.length; i++) {
			...
			if (targetNode === null) targetNode = gltfRuntime.scene.getNodeByName(targetId);
			if (targetNode === null) {
			...
			if (targetPathIndex !== -1) targetPath = babylonAnimationPaths[targetPathIndex];
			let animationType = Animation.ANIMATIONTYPE_MATRIX;
			if (!isBone) if (targetPath === "rotationQuaternion") {
				animationType = Animation.ANIMATIONTYPE_QUATERNION;
				targetNode.rotationQuaternion = new Quaternion();
			} else animationType = Animation.ANIMATIONTYPE_VECTOR3;
			...
			for (let j = 0; j < bufferInput.length; j++) {
				...
				if (targetPath === "rotationQuaternion") {
					value = Quaternion.FromArray([
						bufferOutput[arrayOffset],
						bufferOutput[arrayOffset + 1],
						bufferOutput[arrayOffset + 2],
						bufferOutput[arrayOffset + 3]
					]);
					arrayOffset += 4;
				} else {
					value = Vector3.FromArray([
						bufferOutput[arrayOffset],
						bufferOutput[arrayOffset + 1],
						bufferOutput[arrayOffset + 2]
					]);
				...
				if (isBone) {
					...
					if (modifyKey && lastAnimation) mat = lastAnimation.getKeys()[j].value;
					mat.decompose(scaling, rotationQuaternion, translation);
					if (targetPath === "position") translation = value;
					else if (targetPath === "rotationQuaternion") rotationQuaternion = value;
					else scaling = value;
					value = Matrix.Compose(scaling, rotationQuaternion, translation);
				}
				if (!modifyKey) keys.push({
					frame: bufferInput[j],
					value
				});
				else if (lastAnimation) lastAnimation.getKeys()[j].value = value;
			}
...
var configureBoneTransformation = (node) => {
	...
	if (node.translation || node.rotation || node.scale) {
		const scale = Vector3.FromArray(node.scale || [
			1,
			1,
			1
		]);
		const rotation = Quaternion.FromArray(node.rotation || [
			0,
			0,
			0,
			1
		]);
		const position = Vector3.FromArray(node.translation || [
			0,
			0,
			0
		]);
		mat = Matrix.Compose(scale, rotation, position);
	} else mat = Matrix.FromArray(node.matrix);
	return mat;
};
/**
...
var getParentBone = (gltfRuntime, skins, jointName, newSkeleton) => {
	for (let i = 0; i < newSkeleton.bones.length; i++) if (newSkeleton.bones[i].name === jointName) return newSkeleton.bones[i];
	const nodes = gltfRuntime.nodes;
	for (const nde in nodes) {
		const node = nodes[nde];
		if (!node.jointName) continue;
		const children = node.children;
		for (let i = 0; i < children.length; i++) {
			const child = gltfRuntime.nodes[children[i]];
			if (!child.jointName) continue;
...
var getNodeToRoot = (nodesToRoot, id) => {
	for (let i = 0; i < nodesToRoot.length; i++) {
		...
		for (let j = 0; j < nodeToRoot.node.children.length; j++) if (nodeToRoot.node.children[j] === id) return nodeToRoot.bone;
	}
	return null;
};
/**
...
var getJointNode = (gltfRuntime, jointName) => {
	...
	for (const nde in nodes) {
		node = nodes[nde];
		if (node.jointName === jointName) return {
			node,
			id: nde
		};
	}
...
var nodeIsInJoints = (skins, id) => {
	for (let i = 0; i < skins.jointNames.length; i++) if (skins.jointNames[i] === id) return true;
	return false;
};
/**
...
var getNodesToRoot = (gltfRuntime, newSkeleton, skins, nodesToRoot) => {
	for (const nde in gltfRuntime.nodes) {
		...
		const id = nde;
		if (!node.jointName || nodeIsInJoints(skins, node.jointName)) continue;
		const mat = configureBoneTransformation(node);
	...
	for (let i = 0; i < nodesToRoot.length; i++) {
		...
		for (let j = 0; j < children.length; j++) {
			let child = null;
			for (let k = 0; k < nodesToRoot.length; k++) if (nodesToRoot[k].id === children[j]) {
				child = nodesToRoot[k];
				break;
			}
			if (child) {
...
var importSkeleton = (gltfRuntime, skins, mesh, newSkeleton) => {
	if (!newSkeleton) newSkeleton = new Skeleton(skins.name || "", "", gltfRuntime.scene);
	if (!skins.babylonSkeleton) return newSkeleton;
	const nodesToRoot = [];
	...
	for (let i = 0; i < skins.jointNames.length; i++) {
		...
		for (let j = 0; j < i; j++) {
			...
			foundBone = false;
			for (let k = 0; k < children.length; k++) if (children[k] === id) {
		...
		if (!parentBone && nodesToRoot.length > 0) {
			...
			if (parentBone) {
				if (nodesToRootToAdd.indexOf(parentBone) === -1) nodesToRootToAdd.push(parentBone);
			}
		}
	...
	for (let i = 0; i < skins.jointNames.length; i++) {
		const jointNode = getJointNode(gltfRuntime, skins.jointNames[i]);
		if (!jointNode) continue;
		for (let j = 0; j < bones.length; j++) if (bones[j].id === jointNode.id) {
			newSkeleton.bones.push(bones[j]);
			break;
		}
	}
	newSkeleton.prepare();
	for (let i = 0; i < nodesToRootToAdd.length; i++) newSkeleton.bones.push(nodesToRootToAdd[i]);
	return newSkeleton;
};
/**
...
var importMesh = (gltfRuntime, node, meshes, id, newMesh) => {
	...
	if (!node.babylonNode) return newMesh;
	const subMaterials = [];
	let vertexData = null;
	...
	for (let meshIndex = 0; meshIndex < meshes.length; meshIndex++) {
		...
		for (let i = 0; i < mesh.primitives.length; i++) {
			...
			if (accessor) {
				...
				const indices = [];
				for (let j = 0; j < tempVertexData.positions.length / 3; j++) indices.push(j);
				tempVertexData.indices = new Int32Array(indices);
				indexCounts.push(tempVertexData.indices.length);
			}
			if (!vertexData) vertexData = tempVertexData;
			else vertexData.merge(tempVertexData);
	...
	} else material = new StandardMaterial("multimat" + id, gltfRuntime.scene);
	if (subMaterials.length === 1) material = subMaterials[0];
	material._parentContainer = gltfRuntime.assetContainer;
	if (!newMesh.material) newMesh.material = material;
...
var configureNode = (newNode, position, rotation, scaling) => {
	if (newNode.position) newNode.position = position;
	if (newNode.rotationQuaternion || newNode.rotation) newNode.rotationQuaternion = rotation;
	if (newNode.scaling) newNode.scaling = scaling;
};
/**
...
var configureNodeFromMatrix = (newNode, node) => {
	...
	} else if (node.translation && node.rotation && node.scale) configureNode(newNode, Vector3.FromArray(node.translation), Quaternion.FromArray(node.rotation), Vector3.FromArray(node.scale));
...
var importNode = (gltfRuntime, node, id) => {
	...
	if (gltfRuntime.importOnlyMeshes && (node.skin || node.meshes)) {
		if (gltfRuntime.importMeshesNames && gltfRuntime.importMeshesNames.length > 0 && gltfRuntime.importMeshesNames.indexOf(node.name || "") === -1) return null;
	}
	if (node.skin) {
		if (node.meshes) {
			...
			if (newMesh.skeleton === null) {
				...
				if (!skin.babylonSkeleton) skin.babylonSkeleton = newMesh.skeleton;
			}
			lastNode = newMesh;
		}
	} else if (node.meshes) lastNode = importMesh(gltfRuntime, node, node.mesh ? [node.mesh] : node.meshes, id, node.babylonNode);
		...
		if (light) {
			if (light.type === "ambient") {
				...
				if (ambienLight.color) hemiLight.diffuse = Color3.FromArray(ambienLight.color);
				lastNode = hemiLight;
				...
				if (directionalLight.color) dirLight.diffuse = Color3.FromArray(directionalLight.color);
				lastNode = dirLight;
			} else if (light.type === "point") {
				const pointLight = light[light.type];
				const ptLight = new PointLight(node.light, Vector3.Zero(), gltfRuntime.scene);
				ptLight.name = node.name || "";
				if (pointLight.color) ptLight.diffuse = Color3.FromArray(pointLight.color);
				lastNode = ptLight;
				...
				if (spotLight.color) spLight.diffuse = Color3.FromArray(spotLight.color);
				if (spotLight.fallOfAngle) spLight.angle = spotLight.fallOfAngle;
				if (spotLight.fallOffExponent) spLight.exponent = spotLight.fallOffExponent;
				lastNode = spLight;
			}
		}
		...
		if (camera) {
			...
			if (camera.type === "orthographic") {
				...
				if (!perspectiveCamera.aspectRatio) perspectiveCamera.aspectRatio = gltfRuntime.scene.getEngine().getRenderWidth() / gltfRuntime.scene.getEngine().getRenderHeight();
	...
	if (!node.jointName) {
		if (node.babylonNode) return node.babylonNode;
	...
	if (lastNode !== null) {
		if (node.matrix && lastNode instanceof Mesh) configureNodeFromMatrix(lastNode, node);
		else {
...
var traverseNodes = (gltfRuntime, id, parent, meshIncluded = false) => {
	...
	let newNode = null;
	if (gltfRuntime.importOnlyMeshes && !meshIncluded && gltfRuntime.importMeshesNames) if (gltfRuntime.importMeshesNames.indexOf(node.name || "") !== -1 || gltfRuntime.importMeshesNames.length === 0) meshIncluded = true;
	else meshIncluded = false;
	else meshIncluded = true;
	...
	if (node.children) for (let i = 0; i < node.children.length; i++) traverseNodes(gltfRuntime, node.children[i], newNode, meshIncluded);
};
/**
...
var postLoad = (gltfRuntime) => {
	...
	if (currentScene) for (let i = 0; i < currentScene.nodes.length; i++) traverseNodes(gltfRuntime, currentScene.nodes[i], null);
	else for (const thing in gltfRuntime.scenes) {
		currentScene = gltfRuntime.scenes[thing];
		for (let i = 0; i < currentScene.nodes.length; i++) traverseNodes(gltfRuntime, currentScene.nodes[i], null);
	}
...
var onBindShaderMaterial = (mesh, gltfRuntime, unTreatedUniforms, shaderMaterial, technique, material, onSuccess) => {
	...
	for (const unif in unTreatedUniforms) {
		...
		if (type === EParameterType.FLOAT_MAT2 || type === EParameterType.FLOAT_MAT3 || type === EParameterType.FLOAT_MAT4) {
			if (uniform.semantic && !uniform.source && !uniform.node) GLTFUtils.SetMatrix(gltfRuntime.scene, mesh, uniform, unif, shaderMaterial.getEffect());
			else if (uniform.semantic && (uniform.source || uniform.node)) {
				let source = gltfRuntime.scene.getNodeByName(uniform.source || uniform.node || "");
				if (source === null) source = gltfRuntime.scene.getNodeById(uniform.source || uniform.node || "");
			...
			if (type === EParameterType.SAMPLER_2D) {
				...
				if (texture === null || texture === void 0) continue;
				shaderMaterial.getEffect().setTexture(unif, texture);
			} else GLTFUtils.SetUniform(shaderMaterial.getEffect(), unif, value, type);
		}
	}
...
var prepareShaderMaterialUniforms = (gltfRuntime, shaderMaterial, technique, material, unTreatedUniforms) => {
	...
	*/
	for (const unif in unTreatedUniforms) {
		...
		if (value === void 0) value = uniform.value;
		if (!value) continue;
		...
		if (type === EParameterType.SAMPLER_2D) GLTFLoaderExtension.LoadTextureAsync(gltfRuntime, material.values ? value : uniform.value, onLoadTexture(unif), () => onLoadTexture(null));
		else if (uniform.value && GLTFUtils.SetUniform(shaderMaterial, unif, material.values ? value : uniform.value, type)) delete unTreatedUniforms[unif];
	}
};
/**
...
var importMaterials = (gltfRuntime) => {
	for (const mat in gltfRuntime.materials) GLTFLoaderExtension.LoadMaterialAsync(gltfRuntime, mat, () => {}, () => {});
};
/**
...
var GLTFLoaderBase = class {
	static CreateRuntime(parsedData, scene, rootUrl) {
		...
		if (parsedData.extensions) parseObject(parsedData.extensions, "extensions", gltfRuntime);
		if (parsedData.extensionsUsed) parseObject(parsedData.extensionsUsed, "extensionsUsed", gltfRuntime);
		if (parsedData.buffers) parseBuffers(parsedData.buffers, gltfRuntime);
		if (parsedData.bufferViews) parseObject(parsedData.bufferViews, "bufferViews", gltfRuntime);
		if (parsedData.accessors) parseObject(parsedData.accessors, "accessors", gltfRuntime);
		if (parsedData.meshes) parseObject(parsedData.meshes, "meshes", gltfRuntime);
		if (parsedData.lights) parseObject(parsedData.lights, "lights", gltfRuntime);
		if (parsedData.cameras) parseObject(parsedData.cameras, "cameras", gltfRuntime);
		if (parsedData.nodes) parseObject(parsedData.nodes, "nodes", gltfRuntime);
		if (parsedData.images) parseObject(parsedData.images, "images", gltfRuntime);
		if (parsedData.textures) parseObject(parsedData.textures, "textures", gltfRuntime);
		if (parsedData.shaders) parseShaders(parsedData.shaders, gltfRuntime);
		if (parsedData.programs) parseObject(parsedData.programs, "programs", gltfRuntime);
		if (parsedData.samplers) parseObject(parsedData.samplers, "samplers", gltfRuntime);
		if (parsedData.techniques) parseObject(parsedData.techniques, "techniques", gltfRuntime);
		if (parsedData.materials) parseObject(parsedData.materials, "materials", gltfRuntime);
		if (parsedData.animations) parseObject(parsedData.animations, "animations", gltfRuntime);
		if (parsedData.skins) parseObject(parsedData.skins, "skins", gltfRuntime);
		if (parsedData.scenes) gltfRuntime.scenes = parsedData.scenes;
		if (parsedData.scene && parsedData.scenes) gltfRuntime.currentScene = parsedData.scenes[parsedData.scene];
		return gltfRuntime;
	}
	static LoadBufferAsync(gltfRuntime, id, onSuccess, onError, onProgress) {
		const buffer = gltfRuntime.buffers[id];
		if (Tools.IsBase64(buffer.uri)) setTimeout(() => onSuccess(new Uint8Array(Tools.DecodeBase64(buffer.uri))));
		else Tools.LoadFile(gltfRuntime.rootUrl + buffer.uri, (data) => onSuccess(new Uint8Array(data)), onProgress, void 0, true, (request) => {
			if (request) onError(request.status + " " + request.statusText);
		});
	}
	static LoadTextureBufferAsync(gltfRuntime, id, onSuccess, onError) {
		...
		if (Tools.IsBase64(source.uri)) setTimeout(() => onSuccess(new Uint8Array(Tools.DecodeBase64(source.uri))));
		else Tools.LoadFile(gltfRuntime.rootUrl + source.uri, (data) => onSuccess(new Uint8Array(data)), void 0, void 0, true, (request) => {
			if (request) onError(request.status + " " + request.statusText);
		});
	}
	static CreateTextureAsync(gltfRuntime, id, buffer, onSuccess) {
		...
		if (sampler.wrapS !== void 0) newTexture.wrapU = GLTFUtils.GetWrapMode(sampler.wrapS);
		if (sampler.wrapT !== void 0) newTexture.wrapV = GLTFUtils.GetWrapMode(sampler.wrapT);
		newTexture.name = id;
	...
	static LoadShaderStringAsync(gltfRuntime, id, onSuccess, onError) {
		...
		if (Tools.IsBase64(shader.uri)) {
			...
			if (onSuccess) onSuccess(shaderString);
		} else Tools.LoadFile(gltfRuntime.rootUrl + shader.uri, onSuccess, void 0, void 0, false, (request) => {
			if (request && onError) onError(request.status + " " + request.statusText);
		});
	}
	static LoadMaterialAsync(gltfRuntime, id, onSuccess, onError) {
		const material = gltfRuntime.materials[id];
		if (!material.technique) {
			if (onError) onError("No technique found.");
			return;
		}
		...
		if (!technique) {
			...
			gltfRuntime.scene._blockEntityCollection = false;
			defaultMaterial.diffuseColor = new Color3(.5, .5, .5);
		...
		for (const unif in technique.uniforms) {
			...
			if (uniformParameter.semantic && !uniformParameter.node && !uniformParameter.source) {
				...
				if (transformIndex !== -1) {
					...
					delete unTreatedUniforms[unif];
				} else uniforms.push(unif);
			} else if (uniformParameter.type === EParameterType.SAMPLER_2D) samplers.push(unif);
			else uniforms.push(unif);
		}
		...
		while (!vertexTokenizer.isEnd() && vertexTokenizer.getNextToken()) {
			if (vertexTokenizer.currentToken !== ETokenType.IDENTIFIER) {
		...
		while (!pixelTokenizer.isEnd() && pixelTokenizer.getNextToken()) {
			if (pixelTokenizer.currentToken !== ETokenType.IDENTIFIER) {
		...
		if (states && states.functions) {
			...
			if (functions.cullFace && functions.cullFace[0] !== ECullingType.BACK) shaderMaterial.backFaceCulling = false;
			const blendFunc = functions.blendFuncSeparate;
			if (blendFunc) {
				if (blendFunc[0] === EBlendingFunction.SRC_ALPHA && blendFunc[1] === EBlendingFunction.ONE_MINUS_SRC_ALPHA && blendFunc[2] === EBlendingFunction.ONE && blendFunc[3] === EBlendingFunction.ONE) shaderMaterial.alphaMode = Constants.ALPHA_COMBINE;
				else if (blendFunc[0] === EBlendingFunction.ONE && blendFunc[1] === EBlendingFunction.ONE && blendFunc[2] === EBlendingFunction.ZERO && blendFunc[3] === EBlendingFunction.ONE) shaderMaterial.alphaMode = Constants.ALPHA_ONEONE;
				else if (blendFunc[0] === EBlendingFunction.SRC_ALPHA && blendFunc[1] === EBlendingFunction.ONE && blendFunc[2] === EBlendingFunction.ZERO && blendFunc[3] === EBlendingFunction.ONE) shaderMaterial.alphaMode = Constants.ALPHA_ADD;
				else if (blendFunc[0] === EBlendingFunction.ZERO && blendFunc[1] === EBlendingFunction.ONE_MINUS_SRC_COLOR && blendFunc[2] === EBlendingFunction.ONE && blendFunc[3] === EBlendingFunction.ONE) shaderMaterial.alphaMode = Constants.ALPHA_SUBTRACT;
				else if (blendFunc[0] === EBlendingFunction.DST_COLOR && blendFunc[1] === EBlendingFunction.ZERO && blendFunc[2] === EBlendingFunction.ONE && blendFunc[3] === EBlendingFunction.ONE) shaderMaterial.alphaMode = Constants.ALPHA_MULTIPLY;
				else if (blendFunc[0] === EBlendingFunction.SRC_ALPHA && blendFunc[1] === EBlendingFunction.ONE_MINUS_SRC_COLOR && blendFunc[2] === EBlendingFunction.ONE && blendFunc[3] === EBlendingFunction.ONE) shaderMaterial.alphaMode = Constants.ALPHA_MAXIMIZED;
			}
		}
	}
};
...
* @deprecated
*/
var GLTFLoader$1 = class GLTFLoader$1 {
	static RegisterExtension(extension) {
		if (GLTFLoader$1.Extensions[extension.name]) {
			Tools.Error("Tool with the same name \"" + extension.name + "\" already exists");
			return;
		}
		GLTFLoader$1.Extensions[extension.name] = extension;
	}
	dispose() {}
	_importMeshAsync(meshesNames, scene, data, rootUrl, assetContainer, onSuccess, onProgress, onError) {
		...
		GLTFLoaderExtension.LoadRuntimeAsync(scene, data, rootUrl, (gltfRuntime) => {
			...
			if (meshesNames === "") gltfRuntime.importMeshesNames = [];
			else if (typeof meshesNames === "string") gltfRuntime.importMeshesNames = [meshesNames];
			else if (meshesNames && !(meshesNames instanceof Array)) gltfRuntime.importMeshesNames = [meshesNames];
			else {
			...
			for (const nde in gltfRuntime.nodes) {
				const node = gltfRuntime.nodes[nde];
				if (node.babylonNode instanceof AbstractMesh) meshes.push(node.babylonNode);
			}
	...
	_loadShadersAsync(gltfRuntime, onload) {
		...
		const processShader = (sha, shader) => {
			GLTFLoaderExtension.LoadShaderStringAsync(gltfRuntime, sha, (shaderString) => {
				if (shaderString instanceof ArrayBuffer) return;
				gltfRuntime.loadedShaderCount++;
				if (shaderString) Effect.ShadersStore[sha + (shader.type === EShaderType.VERTEX ? "VertexShader" : "PixelShader")] = shaderString;
				if (gltfRuntime.loadedShaderCount === gltfRuntime.shaderscount) onload();
			}, () => {
		...
		for (const sha in gltfRuntime.shaders) {
			...
			if (shader) processShader.bind(this, sha, shader)();
			else Tools.Error("No shader named: " + sha);
		}
		if (!hasShaders) onload();
	}
	_loadBuffersAsync(gltfRuntime, onLoad) {
		...
		const processBuffer = (buf, buffer) => {
			GLTFLoaderExtension.LoadBufferAsync(gltfRuntime, buf, (bufferView) => {
				...
				if (bufferView) {
					if (bufferView.byteLength != gltfRuntime.buffers[buf].byteLength) Tools.Error("Buffer named " + buf + " is length " + bufferView.byteLength + ". Expected: " + buffer.byteLength);
					gltfRuntime.loadedBufferViews[buf] = bufferView;
				}
				if (gltfRuntime.loadedBufferCount === gltfRuntime.buffersCount) onLoad();
			}, () => {
		...
		for (const buf in gltfRuntime.buffers) {
			...
			if (buffer) processBuffer.bind(this, buf, buffer)();
			else Tools.Error("No buffer named: " + buf);
		}
		if (!hasBuffers) onLoad();
	}
...
/** @internal */
var GLTFLoaderExtension = class GLTFLoaderExtension {
	constructor(name) {
		this._name = name;
	}
	get name() {
	...
	*/
	...
	static LoadTextureAsync(gltfRuntime, id, onSuccess, onError) {
		GLTFLoaderExtension._LoadTextureBufferAsync(gltfRuntime, id, (buffer) => {
			if (buffer) GLTFLoaderExtension._CreateTextureAsync(gltfRuntime, id, buffer, onSuccess, onError);
		}, onError);
	}
	static LoadShaderStringAsync(gltfRuntime, id, onSuccess, onError) {
		GLTFLoaderExtension._ApplyExtensions((loaderExtension) => {
	...
	static _ApplyExtensions(func, defaultFunc) {
		for (const extensionName in GLTFLoader$1.Extensions) {
			...
			if (func(loaderExtension)) return;
		}
		defaultFunc();
	}
};
...
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/1.0/glTFBinaryExtension.js
...
var GLTFBinaryExtension = class extends GLTFLoaderExtension {
	...
	loadRuntimeAsync(scene, data, rootUrl, onSuccess) {
		...
		if (!extensionsUsed || extensionsUsed.indexOf(this.name) === -1 || !data.bin) return false;
		this._bin = data.bin;
		onSuccess(GLTFLoaderBase.CreateRuntime(data.json, scene, rootUrl));
		return true;
	}
	loadBufferAsync(gltfRuntime, id, onSuccess, onError) {
		if (gltfRuntime.extensionsUsed.indexOf(this.name) === -1) return false;
		if (id !== BinaryExtensionBufferName) return false;
	...
	loadTextureBufferAsync(gltfRuntime, id, onSuccess) {
		const texture = gltfRuntime.textures[id];
		const source = gltfRuntime.images[texture.source];
		if (!source.extensions || !(this.name in source.extensions)) return false;
		const sourceExt = source.extensions[this.name];
		const bufferView = gltfRuntime.bufferViews[sourceExt.bufferView];
		onSuccess(GLTFUtils.GetBufferFromBufferView(gltfRuntime, bufferView, 0, bufferView.byteLength, EComponentType.UNSIGNED_BYTE));
		return true;
	}
	loadShaderStringAsync(gltfRuntime, id, onSuccess) {
		const shader = gltfRuntime.shaders[id];
		if (!shader.extensions || !(this.name in shader.extensions)) return false;
		...
		setTimeout(() => {
			onSuccess(GLTFUtils.DecodeBufferToText(shaderBytes));
		});
		return true;
	}
};
...
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/1.0/glTFMaterialsCommonExtension.js
/**
* @internal
* @deprecated
*/
var GLTFMaterialsCommonExtension = class extends GLTFLoaderExtension {
		...
		if (lights) for (const thing in lights) {
			...
			switch (light.type) {
				...
				case "point": {
					...
					if (point) pointLight.diffuse = Color3.FromArray(point.color || [
						1,
						1,
						1
					]);
				...
				case "directional": {
					...
					if (directional) dirLight.diffuse = Color3.FromArray(directional.color || [
						1,
						1,
						1
					]);
				...
				case "spot": {
					...
					if (spot) {
						...
						spotLight.diffuse = Color3.FromArray(spot.color || [
							1,
							1,
							1
						]);
	...
	loadMaterialAsync(gltfRuntime, id, onSuccess, onError) {
		...
		if (extension.technique === "CONSTANT") standardMaterial.disableLighting = true;
		...
		if (typeof extension.values.ambient === "string") this._loadTexture(gltfRuntime, extension.values.ambient, standardMaterial, "ambientTexture", onError);
		else standardMaterial.ambientColor = Color3.FromArray(extension.values.ambient || [
			0,
			0,
			0
		]);
		if (typeof extension.values.diffuse === "string") this._loadTexture(gltfRuntime, extension.values.diffuse, standardMaterial, "diffuseTexture", onError);
		else standardMaterial.diffuseColor = Color3.FromArray(extension.values.diffuse || [
			0,
			0,
			0
		]);
		if (typeof extension.values.emission === "string") this._loadTexture(gltfRuntime, extension.values.emission, standardMaterial, "emissiveTexture", onError);
		else standardMaterial.emissiveColor = Color3.FromArray(extension.values.emission || [
			0,
			0,
			0
		]);
		if (typeof extension.values.specular === "string") this._loadTexture(gltfRuntime, extension.values.specular, standardMaterial, "specularTexture", onError);
		else standardMaterial.specularColor = Color3.FromArray(extension.values.specular || [
			0,
			0,
			0
		]);
...
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/1.0/index.js
var _1_0_exports = /* @__PURE__ */ __exportAll({
	EBlendingFunction: () => EBlendingFunction,
	EComponentType: () => EComponentType,
	ECullingType: () => ECullingType,
	EParameterType: () => EParameterType,
...
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/glTFLoaderExtensionRegistry.js
...
function registerGLTFExtension(name, isGLTFExtension, factory) {
	if (unregisterGLTFExtension(name)) Logger.Warn(`Extension with the name '${name}' already exists`);
...
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/glTFLoader.js
/**
...
var ArrayItem = class {
	...
	*/
	static Get(context, array, index) {
		if (!array || index == void 0 || !array[index]) throw new Error(`${context}: Failed to find index (${index})`);
		return array[index];
	}
	/**
	...
	*/
	static TryGet(array, index) {
		if (!array || index == void 0 || !array[index]) return null;
		return array[index];
	}
	/**
	...
	*/
	static Assign(array) {
		if (array) for (let index = 0; index < array.length; index++) array[index].index = index;
	}
};
/** @internal */
...
var GLTFLoader = class GLTFLoader {
	...
	importMeshAsync(meshesNames, scene, container, data, rootUrl, onProgress, fileName = "") {
		return Promise.resolve().then(() => {
			...
			if (meshesNames) {
				const nodeMap = {};
				if (this._gltf.nodes) {
					for (const node of this._gltf.nodes) if (node.name) nodeMap[node.name] = node.index;
				}
				nodes = (meshesNames instanceof Array ? meshesNames : [meshesNames]).map((name) => {
					const node = nodeMap[name];
					if (node === void 0) throw new Error(`Failed to find node '${name}'`);
					return node;
				});
			}
	...
	_loadAsync(rootUrl, fileName, nodes, resultFunc) {
		return Promise.resolve().then(async () => {
			...
			if (!this.parent.loadOnlyMaterials) {
				if (nodes) promises.push(this.loadSceneAsync("/nodes", {
					nodes,
					index: -1
				}));
			...
			if (!this.parent.skipMaterials && this.parent.loadAllMaterials && this._gltf.materials) for (let m = 0; m < this._gltf.materials.length; ++m) {
				const material = this._gltf.materials[m];
				const context = "/materials/" + m;
				const babylonDrawMode = Material.TriangleFillMode;
				promises.push(this._loadMaterialAsync(context, material, null, babylonDrawMode, () => {}));
			}
			if (this._allMaterialsDirtyRequired) this._babylonScene.blockMaterialDirtyMechanism = oldBlockMaterialDirtyMechanism;
			else this._babylonScene._forceBlockMaterialDirtyMechanism(oldBlockMaterialDirtyMechanism);
			if (this._parent.compileMaterials) promises.push(this._compileMaterialsAsync());
			if (this._parent.compileShadowGenerators) promises.push(this._compileShadowGeneratorsAsync());
			return Promise.all(promises).then(() => {
				if (this._rootBabylonMesh && this._rootBabylonMesh !== this._parent.customRootNode) this._rootBabylonMesh.setEnabled(true);
				for (const material of this._babylonScene.materials) {
					const mat = material;
					if (mat.maxSimultaneousLights !== void 0) mat.maxSimultaneousLights = Math.max(mat.maxSimultaneousLights, this._babylonScene.lights.length);
				}
				this._extensionsOnReady();
				this._parent._setState(GLTFLoaderState.READY);
				if (!this._skipStartAnimationStep) this._startAnimations();
				return resultFunc();
			}).then((result) => {
				this._parent._endPerformanceCounter(loadingToReadyCounterName);
				Tools.SetImmediate(() => {
					if (!this._disposed) Promise.all(this._completePromises).then(() => {
	...
	_loadData(data) {
		...
		if (data.bin) {
			...
			if (buffers && buffers[0] && !buffers[0].uri) {
				...
				if (binaryBuffer.byteLength < data.bin.byteLength - 3 || binaryBuffer.byteLength > data.bin.byteLength) Logger.Warn(`Binary buffer length (${binaryBuffer.byteLength}) from JSON does not match chunk length (${data.bin.byteLength})`);
				this._bin = data.bin;
			} else Logger.Warn("Unexpected BIN chunk");
		}
	}
	_setupData() {
		...
		if (this._gltf.nodes) {
			const nodeParents = {};
			for (const node of this._gltf.nodes) if (node.children) for (const index of node.children) nodeParents[index] = node.index;
	...
	async _loadExtensionsAsync() {
		...
		registeredGLTFExtensions.forEach((registeredExtension, name) => {
			if (this.parent.extensionOptions[name]?.enabled === false) {
				if (registeredExtension.isGLTFExtension && this.isExtensionUsed(name)) Logger.Warn(`Extension ${name} is used but has been explicitly disabled.`);
			} else if (!registeredExtension.isGLTFExtension || this.isExtensionUsed(name)) extensionPromises.push((async () => {
				const extension = await registeredExtension.factory(this);
				if (extension.name !== name) Logger.Warn(`The name of the glTF loader extension instance does not match the registered name: ${extension.name} !== ${name}`);
		...
		if (this._gltf.extensionsRequired) {
			for (const name of this._gltf.extensionsRequired) if (!this._extensions.some((extension) => extension.name === name && extension.enabled)) {
				if (this.parent.extensionOptions[name]?.enabled === false) throw new Error(`Required extension ${name} is disabled`);
	...
	_createRootNode() {
		...
		switch (this._parent.coordinateSystemMode) {
			...
			case GLTFLoaderCoordinateSystemMode.FORCE_RIGHT_HANDED:
				...
				break;
			default: throw new Error(`Invalid coordinate system mode (${this._parent.coordinateSystemMode})`);
		}
	...
	*/
	loadSceneAsync(context, scene) {
		...
		if (scene.nodes) for (const index of scene.nodes) {
			const node = ArrayItem.Get(`${context}/nodes/${index}`, this._gltf.nodes, index);
			promises.push(this.loadNodeAsync(`/nodes/${node.index}`, node, (babylonMesh) => {
				babylonMesh.parent = this._rootBabylonMesh;
			}));
		}
		for (const action of this._postSceneLoadActions) action();
		promises.push(this._loadAnimationsAsync());
		this.logClose();
		return Promise.all(promises).then(() => {});
	}
	_forEachPrimitive(node, callback) {
		if (node._primitiveBabylonMeshes) for (const babylonMesh of node._primitiveBabylonMeshes) callback(babylonMesh);
	}
	_getGeometries() {
		const geometries = [];
		const nodes = this._gltf.nodes;
		if (nodes) for (const node of nodes) this._forEachPrimitive(node, (babylonMesh) => {
			const geometry = babylonMesh.geometry;
			if (geometry && geometries.indexOf(geometry) === -1) geometries.push(geometry);
		});
		return geometries;
	}
	_getMeshes() {
		const meshes = [];
		if (this._rootBabylonMesh instanceof AbstractMesh) meshes.push(this._rootBabylonMesh);
		const nodes = this._gltf.nodes;
		if (nodes) for (const node of nodes) this._forEachPrimitive(node, (babylonMesh) => {
	...
	_getTransformNodes() {
		...
		if (nodes) for (const node of nodes) {
			if (node._babylonTransformNode && node._babylonTransformNode.getClassName() === "TransformNode") transformNodes.push(node._babylonTransformNode);
			if (node._babylonTransformNodeForSkin) transformNodes.push(node._babylonTransformNodeForSkin);
		}
	...
	_getSkeletons() {
		...
		if (skins) {
			for (const skin of skins) if (skin._data) skeletons.push(skin._data.babylonSkeleton);
		}
		return skeletons;
	}
	_getAnimationGroups() {
		...
		if (animations) {
			for (const animation of animations) if (animation._babylonAnimationGroup) animationGroups.push(animation._babylonAnimationGroup);
		}
		return animationGroups;
	}
	_startAnimations() {
		switch (this._parent.animationStartMode) {
			case GLTFLoaderAnimationStartMode.NONE: break;
			case GLTFLoaderAnimationStartMode.FIRST: {
				const babylonAnimationGroups = this._getAnimationGroups();
				if (babylonAnimationGroups.length !== 0) babylonAnimationGroups[0].start(true);
				break;
			}
			case GLTFLoaderAnimationStartMode.ALL: {
				const babylonAnimationGroups = this._getAnimationGroups();
				for (const babylonAnimationGroup of babylonAnimationGroups) babylonAnimationGroup.start(true);
				break;
			}
			default:
	...
	*/
	loadNodeAsync(context, node, assign = () => {}) {
		...
		if (extensionPromise) return extensionPromise;
		if (node._babylonTransformNode) throw new Error(`${context}: Invalid recursive node hierarchy`);
		const promises = new Array();
		...
		const loadNode = (babylonTransformNode) => {
			...
			if (node.children) for (const index of node.children) {
		...
		if (!hasMesh || hasSkin) {
			...
			this._babylonScene._blockEntityCollection = false;
			if (node.mesh == void 0) node._babylonTransformNode = transformNode;
			else node._babylonTransformNodeForSkin = transformNode;
			loadNode(transformNode);
		}
		if (hasMesh) if (hasSkin) {
				...
				promises.push(this._loadSkinAsync(`/skins/${skin.index}`, node, skin, (babylonSkeleton) => {
					...
					this._postSceneLoadActions.push(() => {
						if (skin.skeleton != void 0) {
							...
							if (node.index === parentNode.index) babylonTransformNode.parent = babylonTransformNodeForSkin.parent;
							else babylonTransformNode.parent = parentNode._babylonTransformNode;
						} else babylonTransformNode.parent = this._rootBabylonMesh;
		...
		return Promise.all(promises).then(() => {
			this._forEachPrimitive(node, (babylonMesh) => {
				...
				if (!asMesh.isAnInstance && asMesh.geometry && asMesh.geometry.useBoundingInfoFromGeometry) babylonMesh._updateBoundingInfo();
				else babylonMesh.refreshBoundingInfo(true, true);
			});
	...
	_loadMeshAsync(context, node, mesh, assign) {
		...
		if (!primitives || !primitives.length) throw new Error(`${context}: Primitives are missing`);
		if (primitives[0].index == void 0) ArrayItem.Assign(primitives);
		const promises = new Array();
		...
		if (primitives.length === 1) {
			...
			for (const primitive of primitives) promises.push(this._loadMeshPrimitiveAsync(`${context}/primitives/${primitive.index}`, `${name}_primitive${primitive.index}`, node, mesh, primitive, (babylonMesh) => {
	...
	*/
	...
	_loadVertexDataAsync(context, primitive, babylonMesh) {
		...
		if (extensionPromise) return extensionPromise;
		const attributes = primitive.attributes;
		if (!attributes) throw new Error(`${context}: Attributes are missing`);
		const promises = new Array();
		const babylonGeometry = new Geometry(babylonMesh.name, this._babylonScene);
		if (primitive.indices == void 0) babylonMesh.isUnIndexed = true;
		else {
		...
		const loadAttribute = (name, kind, callback) => {
			if (attributes[name] == void 0) return;
			babylonMesh._delayInfo = babylonMesh._delayInfo || [];
			if (babylonMesh._delayInfo.indexOf(kind) === -1) babylonMesh._delayInfo.push(kind);
			...
			if (kind == VertexBuffer.MatricesIndicesExtraKind) babylonMesh.numBoneInfluencers = 8;
			if (callback) callback(accessor);
		};
		...
		loadAttribute("COLOR_0", VertexBuffer.ColorKind, (accessor) => {
			if (accessor.type === "VEC4") babylonMesh.hasVertexAlpha = true;
		});
		return Promise.all(promises).then(() => {
	...
	_createMorphTargets(context, node, mesh, primitive, babylonMesh) {
		if (!primitive.targets || !this._parent.loadMorphTargets) return;
		if (node._numMorphTargets == void 0) node._numMorphTargets = primitive.targets.length;
		else if (primitive.targets.length !== node._numMorphTargets) throw new Error(`${context}: Primitives do not have the same number of targets`);
	...
	_loadMorphTargetsAsync(context, primitive, babylonMesh, babylonGeometry) {
		if (!primitive.targets || !this._parent.loadMorphTargets) return Promise.resolve();
		const promises = new Array();
	...
	_loadMorphTargetVertexDataAsync(context, babylonGeometry, attributes, babylonMorphTarget) {
		...
		const loadAttribute = (attribute, kind, setData) => {
			if (attributes[attribute] == void 0) return;
			const babylonVertexBuffer = babylonGeometry.getVertexBuffer(kind);
			if (!babylonVertexBuffer) return;
		...
		loadAttribute("COLOR_0", VertexBuffer.ColorKind, (babylonVertexBuffer, data) => {
			...
			if (componentSize === 3) {
				...
				for (let i = 0; i < data.length / 3; ++i) colors[4 * i + 3] = 1;
			} else if (componentSize === 4) {
				colors = new Float32Array(data.length);
				babylonVertexBuffer.forEach(data.length, (value, index) => {
					colors[index] = data[index] + value;
				});
			} else throw new Error(`${context}: Invalid number of components (${componentSize}) for COLOR_0 attribute`);
			babylonMorphTarget.setColors(colors);
		});
		return Promise.all(promises).then(() => {});
	}
	static _LoadTransform(node, babylonNode) {
		if (node.skin != void 0) return;
		let position = Vector3.Zero();
		let rotation = Quaternion.Identity();
		let scaling = Vector3.One();
		if (node.matrix) Matrix.FromArray(node.matrix).decompose(scaling, rotation, position);
		else {
	...
	_loadBones(context, skin, babylonSkeleton) {
		if (skin.skeleton == void 0 || this._parent.alwaysComputeSkeletonRootNode) {
			...
			if (rootNode) if (skin.skeleton === void 0) skin.skeleton = rootNode.index;
			else {
				const isParent = (a, b) => {
					for (; b.parent; b = b.parent) if (b.parent === a) return true;
					return false;
				};
			...
			else Logger.Warn(`${context}: Failed to find common root`);
		}
		const babylonBones = {};
	...
	_findSkeletonRootNode(context, joints) {
		...
		for (let i = 0;; ++i) {
			...
			if (i >= path.length) return rootNode;
			const node = path[i];
			for (let j = 1; j < joints.length; ++j) {
				path = paths[joints[j]];
				if (i >= path.length || node !== path[i]) return rootNode;
			}
			rootNode = node;
		}
	}
	_loadBone(node, skin, babylonSkeleton, babylonBones) {
		...
		if (babylonBone) return babylonBone;
		let parentBabylonBone = null;
		if (node.index !== skin.skeleton) {
			if (node.parent && node.parent.index !== -1) parentBabylonBone = this._loadBone(node.parent, skin, babylonSkeleton, babylonBones);
			else if (skin.skeleton !== void 0) Logger.Warn(`/skins/${skin.index}/skeleton: Skeleton node is not a common root`);
		}
	...
	_loadSkinInverseBindMatricesDataAsync(context, skin) {
		if (skin.inverseBindMatrices == void 0) return Promise.resolve(null);
	...
	_updateBoneMatrices(babylonSkeleton, inverseBindMatricesData) {
		for (const babylonBone of babylonSkeleton.bones) {
			...
			if (babylonParentBone) baseMatrix.multiplyToRef(babylonParentBone.getAbsoluteInverseBindMatrix(), baseMatrix);
	...
	loadCameraAsync(context, camera, assign = () => {}) {
		...
		switch (camera.type) {
			case "perspective": {
				...
				if (!perspective) throw new Error(`${context}: Camera perspective properties are missing`);
			...
			case "orthographic":
				if (!camera.orthographic) throw new Error(`${context}: Camera orthographic properties are missing`);
				...
				break;
			default: throw new Error(`${context}: Invalid camera type (${camera.type})`);
		}
	...
	_loadAnimationsAsync() {
		...
		for (let index = 0; index < animations.length; index++) {
			...
			promises.push(this.loadAnimationAsync(`/animations/${animation.index}`, animation).then((animationGroup) => {
				if (animationGroup.targetedAnimations.length === 0) animationGroup.dispose();
			}));
		}
		return Promise.all(promises).then(() => {});
	}
	...
	*/
	loadAnimationAsync(context, animation) {
		...
		return import("./animationGroup-Cbsxi6rr.js").then(({ AnimationGroup }) => {
			this._babylonScene._blockEntityCollection = !!this._assetContainer;
			...
			for (const channel of animation.channels) promises.push(this._loadAnimationChannelAsync(`${context}/channels/${channel.index}`, context, animation, channel, (babylonTarget, babylonAnimation) => {
	...
	*/
	async _loadAnimationChannelAsync(context, animationContext, animation, channel, onLoad) {
		...
		if (promise) return promise;
		if (channel.target.node == void 0) return Promise.resolve();
		const targetNode = ArrayItem.Get(`${context}/target/node`, this._gltf.nodes, channel.target.node);
		const channelTargetPath = channel.target.path;
		const pathIsWeights = channelTargetPath === "weights";
		if (pathIsWeights && !targetNode._numMorphTargets || !pathIsWeights && !targetNode._babylonTransformNode) return Promise.resolve();
		if (!this._parent.loadNodeAnimations && !pathIsWeights && !targetNode._isJoint) return Promise.resolve();
		await import("./glTFLoaderAnimation-DvhtXvi8.js");
		let properties;
		switch (channelTargetPath) {
			...
			case "weights":
				...
				break;
			default: throw new Error(`${context}/target/path: Invalid value (${channel.target.path})`);
		}
		if (!properties) throw new Error(`${context}/target/path: Could not find interpolation properties for target path (${channel.target.path})`);
		const targetInfo = {
			object: targetNode,
			info: properties
		};
	...
	*/
	...
	_loadAnimationSamplerAsync(context, sampler) {
		if (sampler._data) return sampler._data;
		const interpolation = sampler.interpolation || "LINEAR";
		switch (interpolation) {
			case "STEP":
			case "LINEAR":
			case "CUBICSPLINE": break;
			default: throw new Error(`${context}/interpolation: Invalid value (${sampler.interpolation})`);
		}
	...
	*/
	loadBufferAsync(context, buffer, byteOffset, byteLength) {
		...
		if (extensionPromise) return extensionPromise;
		if (!buffer._data) if (buffer.uri) buffer._data = this.loadUriAsync(`${context}/uri`, buffer, buffer.uri);
		else {
			if (!this._bin) throw new Error(`${context}: Uri is missing or the binary glTF is missing its binary chunk`);
	...
	*/
	loadBufferViewAsync(context, bufferView) {
		...
		if (extensionPromise) return extensionPromise;
		if (bufferView._data) return bufferView._data;
		const buffer = ArrayItem.Get(`${context}/buffer`, this._gltf.buffers, bufferView.buffer);
		bufferView._data = this.loadBufferAsync(`/buffers/${buffer.index}`, buffer, bufferView.byteOffset || 0, bufferView.byteLength);
		return bufferView._data;
	}
	_loadAccessorAsync(context, accessor, constructor) {
		if (accessor._data) return accessor._data;
		const numComponents = GLTFLoader._GetNumComponents(context, accessor.type);
		const byteStride = numComponents * VertexBuffer.GetTypeByteLength(accessor.componentType);
		const length = numComponents * accessor.count;
		if (accessor.bufferView == void 0) accessor._data = Promise.resolve(new constructor(length));
		else {
			const bufferView = ArrayItem.Get(`${context}/bufferView`, this._gltf.bufferViews, accessor.bufferView);
			accessor._data = this.loadBufferViewAsync(`/bufferViews/${bufferView.index}`, bufferView).then((data) => {
				if (accessor.componentType === 5126 && !accessor.normalized && (!bufferView.byteStride || bufferView.byteStride === byteStride)) return GLTFLoader._GetTypedArray(context, accessor.componentType, data, accessor.byteOffset, length);
				else {
		...
		if (accessor.sparse) {
			...
			accessor._data = accessor._data.then((data) => {
				...
				return Promise.all([this.loadBufferViewAsync(`/bufferViews/${indicesBufferView.index}`, indicesBufferView), this.loadBufferViewAsync(`/bufferViews/${valuesBufferView.index}`, valuesBufferView)]).then(([indicesData, valuesData]) => {
					...
					let values;
					if (accessor.componentType === 5126 && !accessor.normalized) values = GLTFLoader._GetTypedArray(`${context}/sparse/values`, accessor.componentType, valuesData, sparse.values.byteOffset, sparseLength);
					else {
					...
					for (let indicesIndex = 0; indicesIndex < indices.length; indicesIndex++) {
						...
						for (let componentIndex = 0; componentIndex < numComponents; componentIndex++) typedArray[dataIndex++] = values[valuesIndex++];
					}
					return typedArray;
				});
			});
	...
	*/
	_loadIndicesAccessorAsync(context, accessor) {
		if (accessor.type !== "SCALAR") throw new Error(`${context}/type: Invalid value ${accessor.type}`);
		if (accessor.componentType !== 5121 && accessor.componentType !== 5123 && accessor.componentType !== 5125) throw new Error(`${context}/componentType: Invalid value ${accessor.componentType}`);
		if (accessor._data) return accessor._data;
		if (accessor.sparse) {
	...
	*/
	_loadVertexBufferViewAsync(bufferView) {
		if (bufferView._babylonBuffer) return bufferView._babylonBuffer;
	...
	*/
	_loadVertexAccessorAsync(context, accessor, kind) {
		if (accessor._babylonVertexBuffer?.[kind]) return accessor._babylonVertexBuffer[kind];
		if (!accessor._babylonVertexBuffer) accessor._babylonVertexBuffer = {};
		const engine = this._babylonScene.getEngine();
		if (accessor.sparse || accessor.bufferView == void 0) accessor._babylonVertexBuffer[kind] = this._loadFloatAccessorAsync(context, accessor).then((data) => {
	...
	_loadMaterialMetallicRoughnessPropertiesAsync(context, properties, babylonMaterial) {
		if (!(babylonMaterial instanceof PBRMaterial)) throw new Error(`${context}: Material type not supported`);
		const promises = new Array();
		if (properties) {
			if (properties.baseColorFactor) {
				babylonMaterial.albedoColor = Color3.FromArray(properties.baseColorFactor);
				babylonMaterial.alpha = properties.baseColorFactor[3];
			} else babylonMaterial.albedoColor = Color3.White();
			...
			if (properties.baseColorTexture) promises.push(this.loadTextureInfoAsync(`${context}/baseColorTexture`, properties.baseColorTexture, (texture) => {
	...
	_loadMaterialAsync(context, material, babylonMesh, babylonDrawMode, assign = () => {}) {
		...
		if (babylonMesh) {
			...
			babylonMesh.onDisposeObservable.addOnce(() => {
				...
				if (index !== -1) babylonData.babylonMeshes.splice(index, 1);
			});
		}
	...
	*/
	loadMaterialPropertiesAsync(context, material, babylonMaterial) {
		...
		if (material.pbrMetallicRoughness) promises.push(this._loadMaterialMetallicRoughnessPropertiesAsync(`${context}/pbrMetallicRoughness`, material.pbrMetallicRoughness, babylonMaterial));
	...
	*/
	loadMaterialBasePropertiesAsync(context, material, babylonMaterial) {
		...
		if (material.normalTexture) {
			...
			if (material.normalTexture.scale != void 0 && babylonMaterial.bumpTexture) babylonMaterial.bumpTexture.level = material.normalTexture.scale;
		...
		if (material.occlusionTexture) {
			...
			if (material.occlusionTexture.strength != void 0) babylonMaterial.ambientTextureStrength = material.occlusionTexture.strength;
		}
		if (material.emissiveTexture) promises.push(this.loadTextureInfoAsync(`${context}/emissiveTexture`, material.emissiveTexture, (texture) => {
	...
	loadMaterialAlphaProperties(context, material, babylonMaterial) {
		...
		switch (material.alphaMode || "OPAQUE") {
			...
			case "MASK":
				...
				babylonMaterial.alphaCutOff = material.alphaCutoff == void 0 ? .5 : material.alphaCutoff;
				if (babylonMaterial.albedoTexture) babylonMaterial.albedoTexture.hasAlpha = true;
				break;
			case "BLEND":
				...
				break;
			default: throw new Error(`${context}/alphaMode: Invalid value (${material.alphaMode})`);
		}
	}
	/**
	...
	*/
	loadTextureInfoAsync(context, textureInfo, assign = () => {}) {
		...
		if (extensionPromise) return extensionPromise;
		this.logOpen(`${context}`);
		if (textureInfo.texCoord >= 6) throw new Error(`${context}/texCoord: Invalid value (${textureInfo.texCoord})`);
	...
	*/
	_createTextureAsync(context, sampler, image, assign = () => {}, textureLoaderOptions, useSRGBBuffer) {
		...
		const textureCreationOptions = {
			...
			onLoad: () => {
				if (!this._disposed) deferred.resolve();
			},
			onError: (message, exception) => {
				if (!this._disposed) deferred.reject(/* @__PURE__ */ new Error(`${context}: ${exception && exception.message ? exception.message : message || "Failed to load texture"}`));
			},
		...
		promises.push(this.loadImageAsync(`/images/${image.index}`, image).then((data) => {
			...
			if (internalTexture) internalTexture.label = image.name;
		}));
		babylonTexture.wrapU = samplerData.wrapU;
		babylonTexture.wrapV = samplerData.wrapV;
		assign(babylonTexture);
		if (this._parent.useGltfTextureNames) babylonTexture.name = image.name || image.uri || `image${image.index}`;
		return Promise.all(promises).then(() => {
			return babylonTexture;
		});
	}
	_loadSampler(context, sampler) {
		if (!sampler._data) sampler._data = {
	...
	*/
	loadImageAsync(context, image) {
		if (!image._data) {
			...
			if (image.uri) image._data = this.loadUriAsync(`${context}/uri`, image, image.uri);
			else {
	...
	*/
	loadUriAsync(context, property, uri) {
		...
		if (extensionPromise) return extensionPromise;
		if (!GLTFLoader._ValidateUri(uri)) throw new Error(`${context}: '${uri}' is invalid`);
	...
	*/
	static AddPointerMetadata(babylonObject, pointer) {
		...
		(gltf.pointers = gltf.pointers || []).push(pointer);
	}
	static _GetTextureWrapMode(context, mode) {
		mode = mode == void 0 ? 10497 : mode;
		switch (mode) {
			case 33071: return Texture.CLAMP_ADDRESSMODE;
			case 33648: return Texture.MIRROR_ADDRESSMODE;
			case 10497: return Texture.WRAP_ADDRESSMODE;
			default:
	...
	static _GetTextureSamplingMode(context, sampler) {
		...
		if (magFilter === 9729) switch (minFilter) {
			case 9728: return Texture.LINEAR_NEAREST;
			case 9729: return Texture.LINEAR_LINEAR;
			case 9984: return Texture.LINEAR_NEAREST_MIPNEAREST;
			case 9985: return Texture.LINEAR_LINEAR_MIPNEAREST;
			case 9986: return Texture.LINEAR_NEAREST_MIPLINEAR;
			...
			if (magFilter !== 9728) Logger.Warn(`${context}/magFilter: Invalid value (${magFilter})`);
			switch (minFilter) {
				case 9728: return Texture.NEAREST_NEAREST;
				case 9729: return Texture.NEAREST_LINEAR;
				case 9984: return Texture.NEAREST_NEAREST_MIPNEAREST;
				case 9985: return Texture.NEAREST_LINEAR_MIPNEAREST;
				case 9986: return Texture.NEAREST_NEAREST_MIPLINEAR;
				case 9987: return Texture.NEAREST_LINEAR_MIPLINEAR;
				default:
					Logger.Warn(`${context}/minFilter: Invalid value (${minFilter})`);
	...
	*/
	static _GetDrawMode(context, mode) {
		if (mode == void 0) mode = 4;
		switch (mode) {
			case 0: return Material.PointListDrawMode;
			case 1: return Material.LineListDrawMode;
			case 2: return Material.LineLoopDrawMode;
			case 3: return Material.LineStripDrawMode;
			case 4: return Material.TriangleFillMode;
			case 5: return Material.TriangleStripDrawMode;
			case 6: return Material.TriangleFanDrawMode;
		}
	...
	_compileMaterialsAsync() {
		...
		if (this._gltf.materials) {
			for (const material of this._gltf.materials) if (material._data) for (const babylonDrawMode in material._data) {
	...
	_compileShadowGeneratorsAsync() {
		...
		for (const light of lights) {
			...
			if (generator) promises.push(generator.forceCompilationAsync());
		}
		return Promise.all(promises).then(() => {
	...
	_forEachExtensions(action) {
		for (const extension of this._extensions) if (extension.enabled) action(extension);
	}
	_applyExtensions(property, functionName, actionAsync) {
		for (const extension of this._extensions) if (extension.enabled) {
	...
	*/
	static LoadExtensionAsync(context, property, extensionName, actionAsync) {
		if (!property.extensions) return null;
		const extension = property.extensions[extensionName];
	...
	*/
	static LoadExtraAsync(context, property, extensionName, actionAsync) {
		if (!property.extras) return null;
		const extra = property.extras[extensionName];
...
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/EXT_lights_image_based.js
var NAME$35 = "EXT_lights_image_based";
/**
	...
	*/
	constructor(loader) {
		...
		*/
		this.name = NAME$35;
		this._loader = loader;
		this.enabled = this._loader.isExtensionUsed(NAME$35);
	}
	/** @internal */
	dispose() {
		this._loader = null;
	...
	onLoading() {
		...
		if (extensions && extensions[this.name]) this._lights = extensions[this.name].lights;
	}
	/**
	* @internal
	*/
	...
	_loadLightAsync(context, light) {
		if (!light._loaded) {
			...
			light._loaded = Promise.all(promises).then(() => {
				...
				if (light.intensity != void 0) babylonTexture.level = light.intensity;
				if (light.rotation) {
					let rotation = Quaternion.FromArray(light.rotation);
					if (!this._loader.babylonScene.useRightHandedSystem) rotation = Quaternion.Inverse(rotation);
					Matrix.FromQuaternionToRef(rotation, babylonTexture.getReflectionTextureMatrix());
				}
				if (!light.irradianceCoefficients) throw new Error(`${context}: Irradiance coefficients are missing`);
...
unregisterGLTFExtension(NAME$35);
registerGLTFExtension(NAME$35, true, (loader) => new EXT_lights_image_based(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/EXT_mesh_gpu_instancing.js
var NAME$34 = "EXT_mesh_gpu_instancing";
/**
	...
	*/
	constructor(loader) {
		...
		*/
		this.name = NAME$34;
		this._loader = loader;
		this.enabled = this._loader.isExtensionUsed(NAME$34);
	}
	/** @internal */
	dispose() {
		this._loader = null;
	...
	*/
	loadNodeAsync(context, node, assign) {
		return GLTFLoader.LoadExtensionAsync(context, node, this.name, (extensionContext, extension) => {
			...
			this._loader._disableInstancedMesh--;
			if (!node._primitiveBabylonMeshes) return promise;
			const promises = new Array();
			let instanceCount = 0;
			const loadAttribute = (attribute) => {
				...
				else if (instanceCount !== accessor.count) throw new Error(`${extensionContext}/attributes: Instance buffer accessors do not have the same count.`);
			};
			...
			return promise.then((babylonTransformNode) => {
				return Promise.all(promises).then(([translationBuffer, rotationBuffer, scaleBuffer]) => {
					...
					for (const babylonMesh of node._primitiveBabylonMeshes) babylonMesh.thinInstanceSetBuffer("matrix", matrices, 16, true);
...
unregisterGLTFExtension(NAME$34);
registerGLTFExtension(NAME$34, true, (loader) => new EXT_mesh_gpu_instancing(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/EXT_meshopt_compression.js
var NAME$33 = "EXT_meshopt_compression";
/**
...
var EXT_meshopt_compression = class {
	...
	*/
	constructor(loader) {
		...
		*/
		this.name = NAME$33;
		this.enabled = loader.isExtensionUsed(NAME$33);
		this._loader = loader;
	}
	/** @internal */
	dispose() {
	...
	*/
	loadBufferViewAsync(context, bufferView) {
		return GLTFLoader.LoadExtensionAsync(context, bufferView, this.name, (extensionContext, extension) => {
			...
			if (bufferViewMeshopt._meshOptData) return bufferViewMeshopt._meshOptData;
...
unregisterGLTFExtension(NAME$33);
registerGLTFExtension(NAME$33, true, (loader) => new EXT_meshopt_compression(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/EXT_texture_webp.js
var NAME$32 = "EXT_texture_webp";
/**
...
var EXT_texture_webp = class {
	...
	*/
	constructor(loader) {
		...
		this.name = NAME$32;
		this._loader = loader;
		this.enabled = loader.isExtensionUsed(NAME$32);
	}
	/** @internal */
	dispose() {
		this._loader = null;
...
unregisterGLTFExtension(NAME$32);
registerGLTFExtension(NAME$32, true, (loader) => new EXT_texture_webp(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/EXT_texture_avif.js
var NAME$31 = "EXT_texture_avif";
/**
...
var EXT_texture_avif = class {
	...
	*/
	constructor(loader) {
		...
		this.name = NAME$31;
		this._loader = loader;
		this.enabled = loader.isExtensionUsed(NAME$31);
	}
	/** @internal */
	dispose() {
		this._loader = null;
...
unregisterGLTFExtension(NAME$31);
registerGLTFExtension(NAME$31, true, (loader) => new EXT_texture_avif(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/EXT_lights_ies.js
var NAME$30 = "EXT_lights_ies";
/**
	...
	*/
	constructor(loader) {
		...
		*/
		this.name = NAME$30;
		this._loader = loader;
		this.enabled = this._loader.isExtensionUsed(NAME$30);
	}
	/** @internal */
	dispose() {
		this._loader = null;
	...
	onLoading() {
		...
		if (extensions && extensions[this.name]) {
			this._lights = extensions[this.name].lights;
	...
	*/
	loadNodeAsync(context, node, assign) {
		return GLTFLoader.LoadExtensionAsync(context, node, this.name, async (extensionContext, extension) => {
			...
			let bufferData;
			if (light.uri) bufferData = await this._loader.loadUriAsync(context, light, light.uri);
			else {
...
unregisterGLTFExtension(NAME$30);
registerGLTFExtension(NAME$30, true, (loader) => new EXT_lights_ies(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_draco_mesh_compression.js
var NAME$29 = "KHR_draco_mesh_compression";
/**
	...
	*/
	constructor(loader) {
		...
		*/
		...
		this._loader = loader;
		this.enabled = DracoDecoder.DefaultAvailable && this._loader.isExtensionUsed(NAME$29);
	}
	/** @internal */
	dispose() {
	...
	_loadVertexDataAsync(context, primitive, babylonMesh) {
		return GLTFLoader.LoadExtensionAsync(context, primitive, this.name, (extensionContext, extension) => {
			if (primitive.mode != void 0) {
				if (primitive.mode !== 4 && primitive.mode !== 5) throw new Error(`${context}: Unsupported mode ${primitive.mode}`);
			}
			const attributes = {};
			const normalized = {};
			const loadAttribute = (name, kind) => {
				const uniqueId = extension.attributes[name];
				if (uniqueId == void 0) return;
				babylonMesh._delayInfo = babylonMesh._delayInfo || [];
				if (babylonMesh._delayInfo.indexOf(kind) === -1) babylonMesh._delayInfo.push(kind);
				...
				if (this.useNormalizedFlagFromAccessor) {
					...
					if (accessor) normalized[kind] = accessor.normalized || false;
				}
			};
			loadAttribute("POSITION", VertexBuffer.PositionKind);
			loadAttribute("NORMAL", VertexBuffer.NormalKind);
			...
			if (!bufferView._dracoBabylonGeometry) bufferView._dracoBabylonGeometry = this._loader.loadBufferViewAsync(`/bufferViews/${bufferView.index}`, bufferView).then((data) => {
...
unregisterGLTFExtension(NAME$29);
registerGLTFExtension(NAME$29, true, (loader) => new KHR_draco_mesh_compression(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_lights_punctual.js
var NAME$28 = "KHR_lights_punctual";
/**
	...
	*/
	constructor(loader) {
		...
		*/
		this.name = NAME$28;
		this._loader = loader;
		this.enabled = this._loader.isExtensionUsed(NAME$28);
	}
	/** @internal */
	dispose() {
		this._loader = null;
...
unregisterGLTFExtension(NAME$28);
registerGLTFExtension(NAME$28, true, (loader) => new KHR_lights(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_materials_pbrSpecularGlossiness.js
var NAME$27 = "KHR_materials_pbrSpecularGlossiness";
/**
...
var KHR_materials_pbrSpecularGlossiness = class {
	...
	*/
	constructor(loader) {
		...
		*/
		this.order = 200;
		this._loader = loader;
		this.enabled = this._loader.isExtensionUsed(NAME$27);
	}
	/** @internal */
	dispose() {
		this._loader = null;
	...
	*/
	...
	_loadSpecularGlossinessPropertiesAsync(context, properties, babylonMaterial) {
		...
		if (properties.diffuseTexture) promises.push(this._loader.loadTextureInfoAsync(`${context}/diffuseTexture`, properties.diffuseTexture, (texture) => {
...
unregisterGLTFExtension(NAME$27);
registerGLTFExtension(NAME$27, true, (loader) => new KHR_materials_pbrSpecularGlossiness(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_materials_unlit.js
var NAME$26 = "KHR_materials_unlit";
/**
	...
	*/
	constructor(loader) {
		...
		*/
		this.order = 210;
		this._loader = loader;
		this.enabled = this._loader.isExtensionUsed(NAME$26);
	}
	/** @internal */
	dispose() {
		this._loader = null;
	...
	*/
	...
	_loadUnlitPropertiesAsync(context, material, babylonMaterial) {
		...
		if (properties) {
			...
			if (properties.baseColorTexture) promises.push(this._loader.loadTextureInfoAsync(`${context}/baseColorTexture`, properties.baseColorTexture, (texture) => {
				texture.name = `${babylonMaterial.name} (Base Color)`;
				babylonMaterial.albedoTexture = texture;
			}));
		}
...
unregisterGLTFExtension(NAME$26);
registerGLTFExtension(NAME$26, true, (loader) => new KHR_materials_unlit(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_materials_clearcoat.js
var NAME$25 = "KHR_materials_clearcoat";
/**
...
var KHR_materials_clearcoat = class {
	...
	*/
	constructor(loader) {
		...
		*/
		this.order = 190;
		this._loader = loader;
		this.enabled = this._loader.isExtensionUsed(NAME$25);
	}
	/** @internal */
	dispose() {
		this._loader = null;
	...
	*/
	...
	_loadClearCoatPropertiesAsync(context, properties, babylonMaterial) {
		...
		if (properties.clearcoatFactor != void 0) babylonMaterial.clearCoat.intensity = properties.clearcoatFactor;
		else babylonMaterial.clearCoat.intensity = 0;
		if (properties.clearcoatTexture) promises.push(this._loader.loadTextureInfoAsync(`${context}/clearcoatTexture`, properties.clearcoatTexture, (texture) => {
			texture.name = `${babylonMaterial.name} (ClearCoat)`;
			babylonMaterial.clearCoat.texture = texture;
		}));
		if (properties.clearcoatRoughnessFactor != void 0) babylonMaterial.clearCoat.roughness = properties.clearcoatRoughnessFactor;
		else babylonMaterial.clearCoat.roughness = 0;
		...
		if (properties.clearcoatNormalTexture) {
			...
			if (properties.clearcoatNormalTexture.scale != void 0) babylonMaterial.clearCoat.bumpTexture.level = properties.clearcoatNormalTexture.scale;
		}
		return Promise.all(promises).then(() => {});
	}
};
unregisterGLTFExtension(NAME$25);
registerGLTFExtension(NAME$25, true, (loader) => new KHR_materials_clearcoat(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_materials_iridescence.js
var NAME$24 = "KHR_materials_iridescence";
/**
...
var KHR_materials_iridescence = class {
	...
	*/
	constructor(loader) {
		...
		*/
		this.order = 195;
		this._loader = loader;
		this.enabled = this._loader.isExtensionUsed(NAME$24);
	}
	/** @internal */
	dispose() {
		this._loader = null;
	...
	*/
	...
	_loadIridescencePropertiesAsync(context, properties, babylonMaterial) {
		...
		if (properties.iridescenceTexture) promises.push(this._loader.loadTextureInfoAsync(`${context}/iridescenceTexture`, properties.iridescenceTexture, (texture) => {
			texture.name = `${babylonMaterial.name} (Iridescence)`;
			babylonMaterial.iridescence.texture = texture;
		}));
		if (properties.iridescenceThicknessTexture) promises.push(this._loader.loadTextureInfoAsync(`${context}/iridescenceThicknessTexture`, properties.iridescenceThicknessTexture, (texture) => {
...
unregisterGLTFExtension(NAME$24);
registerGLTFExtension(NAME$24, true, (loader) => new KHR_materials_iridescence(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_materials_anisotropy.js
var NAME$23 = "KHR_materials_anisotropy";
/**
	...
	*/
	constructor(loader) {
		...
		*/
		this.order = 195;
		this._loader = loader;
		this.enabled = this._loader.isExtensionUsed(NAME$23);
	}
	/** @internal */
	dispose() {
		this._loader = null;
...
unregisterGLTFExtension(NAME$23);
registerGLTFExtension(NAME$23, true, (loader) => new KHR_materials_anisotropy(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_materials_emissive_strength.js
var NAME$22 = "KHR_materials_emissive_strength";
/**
...
var KHR_materials_emissive_strength = class {
	...
	*/
	constructor(loader) {
		...
		*/
		this.order = 170;
		this._loader = loader;
		this.enabled = this._loader.isExtensionUsed(NAME$22);
	}
	/** @internal */
	dispose() {
		this._loader = null;
	...
	*/
	...
	_loadEmissiveProperties(context, properties, babylonMaterial) {
		if (!(babylonMaterial instanceof PBRMaterial)) throw new Error(`${context}: Material type not supported`);
		if (properties.emissiveStrength !== void 0) babylonMaterial.emissiveIntensity = properties.emissiveStrength;
	}
};
unregisterGLTFExtension(NAME$22);
registerGLTFExtension(NAME$22, true, (loader) => new KHR_materials_emissive_strength(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_materials_sheen.js
var NAME$21 = "KHR_materials_sheen";
/**
...
var KHR_materials_sheen = class {
	...
	*/
	constructor(loader) {
		...
		*/
		this.order = 190;
		this._loader = loader;
		this.enabled = this._loader.isExtensionUsed(NAME$21);
	}
	/** @internal */
	dispose() {
		this._loader = null;
	...
	*/
	...
	_loadSheenPropertiesAsync(context, properties, babylonMaterial) {
		...
		if (properties.sheenColorFactor != void 0) babylonMaterial.sheen.color = Color3.FromArray(properties.sheenColorFactor);
		else babylonMaterial.sheen.color = Color3.Black();
		if (properties.sheenColorTexture) promises.push(this._loader.loadTextureInfoAsync(`${context}/sheenColorTexture`, properties.sheenColorTexture, (texture) => {
			texture.name = `${babylonMaterial.name} (Sheen Color)`;
			babylonMaterial.sheen.texture = texture;
		}));
		if (properties.sheenRoughnessFactor !== void 0) babylonMaterial.sheen.roughness = properties.sheenRoughnessFactor;
		else babylonMaterial.sheen.roughness = 0;
...
unregisterGLTFExtension(NAME$21);
registerGLTFExtension(NAME$21, true, (loader) => new KHR_materials_sheen(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_materials_specular.js
var NAME$20 = "KHR_materials_specular";
/**
...
var KHR_materials_specular = class {
	...
	*/
	constructor(loader) {
		...
		*/
		this.order = 190;
		this._loader = loader;
		this.enabled = this._loader.isExtensionUsed(NAME$20);
	}
	/** @internal */
	dispose() {
		this._loader = null;
	...
	*/
	...
	_loadSpecularPropertiesAsync(context, properties, babylonMaterial) {
		if (!(babylonMaterial instanceof PBRMaterial)) throw new Error(`${context}: Material type not supported`);
		const promises = new Array();
		if (properties.specularFactor !== void 0) babylonMaterial.metallicF0Factor = properties.specularFactor;
		if (properties.specularColorFactor !== void 0) babylonMaterial.metallicReflectanceColor = Color3.FromArray(properties.specularColorFactor);
		...
		if (properties.specularColorTexture) promises.push(this._loader.loadTextureInfoAsync(`${context}/specularColorTexture`, properties.specularColorTexture, (texture) => {
...
unregisterGLTFExtension(NAME$20);
registerGLTFExtension(NAME$20, true, (loader) => new KHR_materials_specular(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_materials_ior.js
var NAME$19 = "KHR_materials_ior";
/**
* [Specification](https://github.com/KhronosGroup/glTF/blob/main/extensions/2.0/Khronos/KHR_materials_ior/README.md)
*/
var KHR_materials_ior = class KHR_materials_ior {
	/**
	* @internal
	*/
	constructor(loader) {
		...
		*/
		this.order = 180;
		this._loader = loader;
		this.enabled = this._loader.isExtensionUsed(NAME$19);
	}
	/** @internal */
	dispose() {
		this._loader = null;
	...
	*/
	...
	_loadIorPropertiesAsync(context, properties, babylonMaterial) {
		if (!(babylonMaterial instanceof PBRMaterial)) throw new Error(`${context}: Material type not supported`);
		if (properties.ior !== void 0) babylonMaterial.indexOfRefraction = properties.ior;
		else babylonMaterial.indexOfRefraction = KHR_materials_ior._DEFAULT_IOR;
...
unregisterGLTFExtension(NAME$19);
registerGLTFExtension(NAME$19, true, (loader) => new KHR_materials_ior(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_materials_variants.js
var NAME$18 = "KHR_materials_variants";
/**
* [Specification](https://github.com/KhronosGroup/glTF/blob/main/extensions/2.0/Khronos/KHR_materials_variants/README.md)
*/
var KHR_materials_variants = class KHR_materials_variants {
	/**
	* @internal
	*/
	constructor(loader) {
		...
		*/
		this.name = NAME$18;
		this._loader = loader;
		this.enabled = this._loader.isExtensionUsed(NAME$18);
	}
	/** @internal */
	dispose() {
		this._loader = null;
	...
	*/
	static SelectVariant(rootNode, variantName) {
		const extensionMetadata = this._GetExtensionMetadata(rootNode);
		if (!extensionMetadata) throw new Error(`Cannot select variant on a glTF mesh that does not have the ${NAME$18} extension`);
		const select = (variantName) => {
			const entries = extensionMetadata.variants[variantName];
			if (entries) for (const entry of entries) entry.mesh.material = entry.material;
		};
		if (variantName instanceof Array) for (const name of variantName) select(name);
		else select(variantName);
	...
	*/
	static Reset(rootNode) {
		const extensionMetadata = this._GetExtensionMetadata(rootNode);
		if (!extensionMetadata) throw new Error(`Cannot reset on a glTF mesh that does not have the ${NAME$18} extension`);
		for (const entry of extensionMetadata.original) entry.mesh.material = entry.material;
	...
	*/
	static GetLastSelectedVariant(rootNode) {
		const extensionMetadata = this._GetExtensionMetadata(rootNode);
		if (!extensionMetadata) throw new Error(`Cannot get the last selected variant on a glTF mesh that does not have the ${NAME$18} extension`);
	...
	*/
	...
	static _GetExtensionMetadata(rootNode) {
		return rootNode?._internalMetadata?.gltf?.[NAME$18] || null;
	}
	/** @internal */
	onLoading() {
		const extensions = this._loader.gltf.extensions;
		if (extensions && extensions[this.name]) this._variants = extensions[this.name].variants;
	}
	/** @internal */
	onReady() {
		const rootNode = this._loader.rootBabylonMesh;
		if (rootNode) {
			const options = this._loader.parent.extensionOptions[NAME$18];
			if (options?.defaultVariant) KHR_materials_variants.SelectVariant(rootNode, options.defaultVariant);
			options?.onLoaded?.({
				get variants() {
					return KHR_materials_variants.GetAvailableVariants(rootNode);
				},
				get selectedVariant() {
					...
					if (!lastSelectedVariant) return KHR_materials_variants.GetAvailableVariants(rootNode)[0];
					if (Array.isArray(lastSelectedVariant)) return lastSelectedVariant[0];
		...
		return GLTFLoader.LoadExtensionAsync(context, primitive, this.name, (extensionContext, extension) => {
			...
			promises.push(this._loader._loadMeshPrimitiveAsync(context, name, node, mesh, primitive, (babylonMesh) => {
				...
				if (babylonMesh instanceof Mesh) {
					...
					const gltf = metadata.gltf = metadata.gltf || {};
					const extensionMetadata = gltf[NAME$18] = gltf[NAME$18] || {
						lastSelected: null,
						original: [],
						variants: {}
					};
					...
					for (let mappingIndex = 0; mappingIndex < extension.mappings.length; ++mappingIndex) {
						...
						promises.push(this._loader._loadMaterialAsync(`#/materials/${mapping.material}`, material, babylonMesh, babylonDrawMode, (babylonMaterial) => {
							for (let mappingVariantIndex = 0; mappingVariantIndex < mapping.variants.length; ++mappingVariantIndex) {
								...
								const variant = ArrayItem.Get(`/extensions/${NAME$18}/variants/${variantIndex}`, this._variants, variantIndex);
								...
								babylonMesh.onClonedObservable.add((newOne) => {
									...
									if (root && metadata === KHR_materials_variants._GetExtensionMetadata(root)) {
										...
										for (const key in root._internalMetadata) newRoot._internalMetadata[key] = root._internalMetadata[key];
										newRoot._internalMetadata.gltf = [];
										for (const key in root._internalMetadata.gltf) newRoot._internalMetadata.gltf[key] = root._internalMetadata.gltf[key];
										newRoot._internalMetadata.gltf[NAME$18] = {
											lastSelected: null,
											original: [],
											variants: {}
										};
										for (const original of metadata.original) newRoot._internalMetadata.gltf[NAME$18].original.push({
											mesh: original.mesh,
											material: original.material
										});
										for (const key in metadata.variants) if (Object.prototype.hasOwnProperty.call(metadata.variants, key)) {
											newRoot._internalMetadata.gltf[NAME$18].variants[key] = [];
											for (const variantEntry of metadata.variants[key]) newRoot._internalMetadata.gltf[NAME$18].variants[key].push({
												mesh: variantEntry.mesh,
												material: variantEntry.material
											});
										}
										metadata = newRoot._internalMetadata.gltf[NAME$18];
									}
									for (const target of metadata.original) if (target.mesh === babylonMesh) target.mesh = newMesh;
									for (const target of metadata.variants[variant.name]) if (target.mesh === babylonMesh) target.mesh = newMesh;
								});
							}
						}));
					}
...
unregisterGLTFExtension(NAME$18);
registerGLTFExtension(NAME$18, true, (loader) => new KHR_materials_variants(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_materials_transmission.js
/**
* A class to handle setting up the rendering of opaque objects to be shown through transmissive objects.
*/
var TransmissionHelper = class TransmissionHelper {
	/**
	...
	*/
	updateOptions(options) {
		if (!Object.keys(options).filter((key) => this._options[key] !== options[key]).length) return;
		const newOptions = {
			...this._options,
			...options
		};
		...
		if (newOptions.renderSize !== oldOptions.renderSize || newOptions.renderTargetTextureType !== oldOptions.renderTargetTextureType || newOptions.generateMipmaps !== oldOptions.generateMipmaps || !this._opaqueRenderTarget) this._setupRenderTargets();
		else {
	...
	*/
	...
	_shouldRenderAsTransmission(material) {
		...
		if (material instanceof PBRMaterial && material.subSurface.isRefractionEnabled) return true;
		return false;
	}
	_addMesh(mesh) {
		...
		Tools.SetImmediate(() => {
			if (this._shouldRenderAsTransmission(mesh.material)) {
				...
				if (this._transparentMeshesCache.indexOf(mesh) === -1) this._transparentMeshesCache.push(mesh);
			} else if (this._opaqueMeshesCache.indexOf(mesh) === -1) this._opaqueMeshesCache.push(mesh);
		});
	}
	_removeMesh(mesh) {
		...
		if (idx !== -1) this._transparentMeshesCache.splice(idx, 1);
		idx = this._opaqueMeshesCache.indexOf(mesh);
		if (idx !== -1) this._opaqueMeshesCache.splice(idx, 1);
	}
	_parseScene() {
	...
	_onMeshMaterialChanged(mesh) {
		...
		if (this._shouldRenderAsTransmission(mesh.material)) {
			if (mesh.material instanceof PBRMaterial) mesh.material.subSurface.refractionTexture = this._opaqueRenderTarget;
			if (opaqueIdx !== -1) {
				this._opaqueMeshesCache.splice(opaqueIdx, 1);
				this._transparentMeshesCache.push(mesh);
			} else if (transparentIdx === -1) this._transparentMeshesCache.push(mesh);
		} else if (transparentIdx !== -1) {
			this._transparentMeshesCache.splice(transparentIdx, 1);
			this._opaqueMeshesCache.push(mesh);
		} else if (opaqueIdx === -1) this._opaqueMeshesCache.push(mesh);
	}
	/**
	* @internal
	...
	*/
	_setupRenderTargets() {
		if (this._opaqueRenderTarget) this._opaqueRenderTarget.dispose();
		...
		this._opaqueRenderTarget.onBeforeBindObservable.add((opaqueRenderTarget) => {
			...
			if (!this._options.clearColor) this._scene.clearColor.toLinearSpaceToRef(opaqueRenderTarget.clearColor, this._scene.getEngine().useExactSrgbConversions);
			else opaqueRenderTarget.clearColor.copyFrom(this._options.clearColor);
		});
		this._opaqueRenderTarget.onAfterUnbindObservable.add(() => {
			this._scene.environmentIntensity = saveSceneEnvIntensity;
		});
		this._transparentMeshesCache.forEach((mesh) => {
			if (this._shouldRenderAsTransmission(mesh.material)) mesh.material.refractionTexture = this._opaqueRenderTarget;
		});
	}
	/**
...
var NAME$17 = "KHR_materials_transmission";
/**
	...
	*/
	constructor(loader) {
		...
		*/
		this.order = 175;
		this._loader = loader;
		this.enabled = this._loader.isExtensionUsed(NAME$17);
		if (this.enabled) loader.parent.transparencyAsCoverage = true;
	}
	/** @internal */
	dispose() {
		this._loader = null;
	...
	*/
	...
	_loadTransparentPropertiesAsync(context, material, babylonMaterial, extension) {
		...
		if (extension.transmissionFactor !== void 0) {
			...
			if (pbrMaterial.subSurface.refractionIntensity && !scene._transmissionHelper) new TransmissionHelper({}, pbrMaterial.getScene());
			else if (pbrMaterial.subSurface.refractionIntensity && !scene._transmissionHelper?._isRenderTargetValid()) scene._transmissionHelper?._setupRenderTargets();
		} else {
...
unregisterGLTFExtension(NAME$17);
registerGLTFExtension(NAME$17, true, (loader) => new KHR_materials_transmission(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_materials_diffuse_transmission.js
var NAME$16 = "KHR_materials_diffuse_transmission";
/**
...
var KHR_materials_diffuse_transmission = class {
	...
	*/
	constructor(loader) {
		...
		*/
		this.order = 174;
		this._loader = loader;
		this.enabled = this._loader.isExtensionUsed(NAME$16);
		if (this.enabled) loader.parent.transparencyAsCoverage = true;
	}
	/** @internal */
	dispose() {
	...
	*/
	...
	_loadTranslucentPropertiesAsync(context, material, babylonMaterial, extension) {
		...
		if (extension.diffuseTransmissionFactor !== void 0) pbrMaterial.subSurface.translucencyIntensity = extension.diffuseTransmissionFactor;
		else {
		...
		if (extension.diffuseTransmissionColorFactor !== void 0) pbrMaterial.subSurface.translucencyColor = Color3.FromArray(extension.diffuseTransmissionColorFactor);
		else pbrMaterial.subSurface.translucencyColor = Color3.White();
		if (extension.diffuseTransmissionColorTexture) promises.push(this._loader.loadTextureInfoAsync(`${context}/diffuseTransmissionColorTexture`, extension.diffuseTransmissionColorTexture).then((texture) => {
...
unregisterGLTFExtension(NAME$16);
registerGLTFExtension(NAME$16, true, (loader) => new KHR_materials_diffuse_transmission(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_materials_volume.js
var NAME$15 = "KHR_materials_volume";
/**
...
var KHR_materials_volume = class {
	...
	*/
	constructor(loader) {
		...
		*/
		this.order = 173;
		this._loader = loader;
		this.enabled = this._loader.isExtensionUsed(NAME$15);
		if (this.enabled) this._loader._disableInstancedMesh++;
	}
	/** @internal */
	dispose() {
		if (this.enabled) this._loader._disableInstancedMesh--;
	...
	*/
	...
	_loadVolumePropertiesAsync(context, material, babylonMaterial, extension) {
		if (!(babylonMaterial instanceof PBRMaterial)) throw new Error(`${context}: Material type not supported`);
		if (!babylonMaterial.subSurface.isRefractionEnabled && !babylonMaterial.subSurface.isTranslucencyEnabled || !extension.thicknessFactor) return Promise.resolve();
		...
		if (extension.attenuationColor !== void 0 && extension.attenuationColor.length == 3) babylonMaterial.subSurface.tintColor.copyFromFloats(extension.attenuationColor[0], extension.attenuationColor[1], extension.attenuationColor[2]);
...
unregisterGLTFExtension(NAME$15);
registerGLTFExtension(NAME$15, true, (loader) => new KHR_materials_volume(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_materials_dispersion.js
var NAME$14 = "KHR_materials_dispersion";
/**
...
var KHR_materials_dispersion = class {
	...
	*/
	constructor(loader) {
		...
		*/
		this.order = 174;
		this._loader = loader;
		this.enabled = this._loader.isExtensionUsed(NAME$14);
	}
	/** @internal */
	dispose() {
		this._loader = null;
	...
	*/
	...
	_loadDispersionPropertiesAsync(context, material, babylonMaterial, extension) {
		if (!(babylonMaterial instanceof PBRMaterial)) throw new Error(`${context}: Material type not supported`);
		if (!babylonMaterial.subSurface.isRefractionEnabled || !extension.dispersion) return Promise.resolve();
		babylonMaterial.subSurface.isDispersionEnabled = true;
		babylonMaterial.subSurface.dispersion = extension.dispersion;
		return Promise.resolve();
	}
};
unregisterGLTFExtension(NAME$14);
registerGLTFExtension(NAME$14, true, (loader) => new KHR_materials_dispersion(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_mesh_quantization.js
var NAME$13 = "KHR_mesh_quantization";
/**
...
var KHR_mesh_quantization = class {
	...
	*/
	constructor(loader) {
		...
		*/
		this.name = NAME$13;
		this.enabled = loader.isExtensionUsed(NAME$13);
	}
	/** @internal */
	dispose() {}
};
unregisterGLTFExtension(NAME$13);
registerGLTFExtension(NAME$13, true, (loader) => new KHR_mesh_quantization(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_texture_basisu.js
var NAME$12 = "KHR_texture_basisu";
/**
...
var KHR_texture_basisu = class {
	...
	*/
	constructor(loader) {
		...
		this.name = NAME$12;
		this._loader = loader;
		this.enabled = loader.isExtensionUsed(NAME$12);
	}
	/** @internal */
	dispose() {
		this._loader = null;
...
unregisterGLTFExtension(NAME$12);
registerGLTFExtension(NAME$12, true, (loader) => new KHR_texture_basisu(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_texture_transform.js
var NAME$11 = "KHR_texture_transform";
/**
	...
	*/
	constructor(loader) {
		...
		*/
		this.name = NAME$11;
		this._loader = loader;
		this.enabled = this._loader.isExtensionUsed(NAME$11);
	}
	/** @internal */
	dispose() {
		this._loader = null;
	...
	loadTextureInfoAsync(context, textureInfo, assign) {
		return GLTFLoader.LoadExtensionAsync(context, textureInfo, this.name, (extensionContext, extension) => {
			return this._loader.loadTextureInfoAsync(context, textureInfo, (babylonTexture) => {
				if (!(babylonTexture instanceof Texture)) throw new Error(`${extensionContext}: Texture type not supported`);
				if (extension.offset) {
				...
				if (extension.rotation) babylonTexture.wAng = -extension.rotation;
				if (extension.scale) {
					babylonTexture.uScale = extension.scale[0];
					babylonTexture.vScale = extension.scale[1];
				}
				if (extension.texCoord != void 0) babylonTexture.coordinatesIndex = extension.texCoord;
				assign(babylonTexture);
			});
		});
	}
};
unregisterGLTFExtension(NAME$11);
registerGLTFExtension(NAME$11, true, (loader) => new KHR_texture_transform(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_xmp_json_ld.js
var NAME$10 = "KHR_xmp_json_ld";
/**
...
var KHR_xmp_json_ld = class {
	...
	*/
	constructor(loader) {
		...
		*/
		this.order = 100;
		this._loader = loader;
		this.enabled = this._loader.isExtensionUsed(NAME$10);
	}
	/** @internal */
	dispose() {
		this._loader = null;
	...
	*/
	onLoading() {
		if (this._loader.rootBabylonMesh === null) return;
...
unregisterGLTFExtension(NAME$10);
registerGLTFExtension(NAME$10, true, (loader) => new KHR_xmp_json_ld(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_animation_pointer.data.js
...
var CameraAnimationPropertyInfo = class extends AnimationPropertyInfo {
	...
	buildAnimations(target, name, fps, keys) {
		return [{
			babylonAnimatable: target._babylonCamera,
...
var MaterialAnimationPropertyInfo = class extends AnimationPropertyInfo {
	...
	buildAnimations(target, name, fps, keys) {
		...
		for (const fillMode in target._data) babylonAnimations.push({
...
var LightAnimationPropertyInfo = class extends AnimationPropertyInfo {
	...
	buildAnimations(target, name, fps, keys) {
		return [{
			babylonAnimatable: target._babylonLight,
			babylonAnimation: this._buildAnimation(name, fps, keys)
		}];
	}
};
SetInterpolationForKey("/cameras/{}/orthographic/xmag", [new CameraAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "orthoLeft", getMinusFloat, () => 1), new CameraAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "orthoRight", getNextFloat, () => 1)]);
SetInterpolationForKey("/cameras/{}/orthographic/ymag", [new CameraAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "orthoBottom", getMinusFloat, () => 1), new CameraAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "orthoTop", getNextFloat, () => 1)]);
...
SetInterpolationForKey("/materials/{}/pbrMetallicRoughness/baseColorFactor", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_COLOR3, "albedoColor", getColor3, () => 4), new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "alpha", getAlpha, () => 4)]);
...
SetInterpolationForKey("/materials/{}/extensions/KHR_materials_anisotropy/anisotropyStrength", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "anisotropy.intensity", getFloat, () => 1)]);
SetInterpolationForKey("/materials/{}/extensions/KHR_materials_anisotropy/anisotropyRotation", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "anisotropy.angle", getFloat, () => 1)]);
...
SetInterpolationForKey("/materials/{}/extensions/KHR_materials_clearcoat/clearcoatFactor", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "clearCoat.intensity", getFloat, () => 1)]);
SetInterpolationForKey("/materials/{}/extensions/KHR_materials_clearcoat/clearcoatRoughnessFactor", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "clearCoat.roughness", getFloat, () => 1)]);
...
SetInterpolationForKey("/materials/{}/extensions/KHR_materials_clearcoat/clearcoatNormalTexture/scale", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "clearCoat.bumpTexture.level", getFloat, () => 1)]);
...
SetInterpolationForKey("/materials/{}/extensions/KHR_materials_dispersion/dispersionFactor", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "subSurface.dispersion", getFloat, () => 1)]);
SetInterpolationForKey("/materials/{}/extensions/KHR_materials_emissive_strength/emissiveStrength", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "emissiveIntensity", getFloat, () => 1)]);
SetInterpolationForKey("/materials/{}/extensions/KHR_materials_ior/ior", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "indexOfRefraction", getFloat, () => 1)]);
SetInterpolationForKey("/materials/{}/extensions/KHR_materials_iridescence/iridescenceFactor", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "iridescence.intensity", getFloat, () => 1)]);
SetInterpolationForKey("/materials/{}/extensions/KHR_materials_iridescence/iridescenceIor", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "iridescence.indexOfRefraction", getFloat, () => 1)]);
SetInterpolationForKey("/materials/{}/extensions/KHR_materials_iridescence/iridescenceThicknessMinimum", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "iridescence.minimumThickness", getFloat, () => 1)]);
SetInterpolationForKey("/materials/{}/extensions/KHR_materials_iridescence/iridescenceThicknessMaximum", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "iridescence.maximumThickness", getFloat, () => 1)]);
...
SetInterpolationForKey("/materials/{}/extensions/KHR_materials_sheen/sheenColorFactor", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_COLOR3, "sheen.color", getColor3, () => 3)]);
SetInterpolationForKey("/materials/{}/extensions/KHR_materials_sheen/sheenRoughnessFactor", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "sheen.roughness", getFloat, () => 1)]);
...
SetInterpolationForKey("/materials/{}/extensions/KHR_materials_specular/specularFactor", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "metallicF0Factor", getFloat, () => 1)]);
SetInterpolationForKey("/materials/{}/extensions/KHR_materials_specular/specularColorFactor", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_COLOR3, "metallicReflectanceColor", getColor3, () => 3)]);
...
SetInterpolationForKey("/materials/{}/extensions/KHR_materials_transmission/transmissionFactor", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "subSurface.refractionIntensity", getFloat, () => 1)]);
...
SetInterpolationForKey("/materials/{}/extensions/KHR_materials_volume/attenuationColor", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_COLOR3, "subSurface.tintColor", getColor3, () => 3)]);
SetInterpolationForKey("/materials/{}/extensions/KHR_materials_volume/attenuationDistance", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "subSurface.tintColorAtDistance", getFloat, () => 1)]);
SetInterpolationForKey("/materials/{}/extensions/KHR_materials_volume/thicknessFactor", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "subSurface.maximumThickness", getFloat, () => 1)]);
...
SetInterpolationForKey("/materials/{}/extensions/KHR_materials_diffuse_transmission/diffuseTransmissionFactor", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "subSurface.translucencyIntensity", getFloat, () => 1)]);
...
SetInterpolationForKey("/materials/{}/extensions/KHR_materials_diffuse_transmission/diffuseTransmissionColorFactor", [new MaterialAnimationPropertyInfo(Animation.ANIMATIONTYPE_COLOR3, "subSurface.translucencyColor", getColor3, () => 3)]);
...
SetInterpolationForKey("/extensions/KHR_lights_punctual/lights/{}/spot/innerConeAngle", [new LightAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "innerAngle", getFloatBy2, () => 1)]);
SetInterpolationForKey("/extensions/KHR_lights_punctual/lights/{}/spot/outerConeAngle", [new LightAnimationPropertyInfo(Animation.ANIMATIONTYPE_FLOAT, "angle", getFloatBy2, () => 1)]);
...
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_animation_pointer.js
var NAME$9 = "KHR_animation_pointer";
/**
...
var KHR_animation_pointer = class {
	...
	*/
	_loadAnimationChannelAsync(context, animationContext, animation, channel, onLoad) {
		...
		if (!extension || !this._pathToObjectConverter) return null;
		if (channel.target.path !== "pointer") Logger.Warn(`${context}/target/path: Value (${channel.target.path}) must be (pointer) when using the ${this.name} extension`);
		if (channel.target.node != void 0) Logger.Warn(`${context}/target/node: Value (${channel.target.node}) must not be present when using the ${this.name} extension`);
		const extensionContext = `${context}/extensions/${this.name}`;
		const pointer = extension.pointer;
		if (!pointer) throw new Error(`${extensionContext}: Pointer is missing`);
		try {
			const obj = this._pathToObjectConverter.convert(pointer);
			if (!obj.info.interpolation) throw new Error(`${extensionContext}/pointer: Interpolation is missing`);
...
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/MSFT_audio_emitter.js
var NAME$8 = "MSFT_audio_emitter";
/**
...
var MSFT_audio_emitter = class {
	...
	loadSceneAsync(context, scene) {
		return GLTFLoader.LoadExtensionAsync(context, scene, this.name, (extensionContext, extension) => {
			...
			for (const emitterIndex of extension.emitters) {
				...
				if (emitter.refDistance != void 0 || emitter.maxDistance != void 0 || emitter.rolloffFactor != void 0 || emitter.distanceModel != void 0 || emitter.innerAngle != void 0 || emitter.outerAngle != void 0) throw new Error(`${extensionContext}: Direction or Distance properties are not allowed on emitters attached to a scene`);
	...
	*/
	loadAnimationAsync(context, animation) {
		return GLTFLoader.LoadExtensionAsync(context, animation, this.name, (extensionContext, extension) => {
			return this._loader.loadAnimationAsync(context, animation).then((babylonAnimationGroup) => {
				...
				for (const event of extension.events) promises.push(this._loadAnimationEventAsync(`${extensionContext}/events/${event.index}`, context, animation, event, babylonAnimationGroup));
				return Promise.all(promises).then(() => {
					return babylonAnimationGroup;
				});
			});
	...
	_loadClipAsync(context, clip) {
		if (clip._objectURL) return clip._objectURL;
		let promise;
		if (clip.uri) promise = this._loader.loadUriAsync(context, clip, clip.uri);
		else {
	...
	_loadEmitterAsync(context, emitter) {
		...
		if (!emitter._babylonData) {
			...
			emitter._babylonData = { loaded: Promise.all(clipPromises).then(() => {
	...
	_getEventAction(context, sound, action, time, startOffset) {
		switch (action) {
			...
			default: throw new Error(`${context}: Unsupported action ${action}`);
		}
	}
	_loadAnimationEventAsync(context, animationContext, animation, event, babylonAnimationGroup) {
		if (babylonAnimationGroup.targetedAnimations.length == 0) return Promise.resolve();
...
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/MSFT_lod.js
var NAME$7 = "MSFT_lod";
/**
...
var MSFT_lod = class {
		...
		for (let indexLOD = 0; indexLOD < this._nodePromiseLODs.length; indexLOD++) {
			const promise = Promise.all(this._nodePromiseLODs[indexLOD]).then(() => {
				...
				if (indexLOD !== this._nodePromiseLODs.length - 1) {
					...
					if (this._nodeSignalLODs[indexLOD]) this._nodeSignalLODs[indexLOD].resolve();
				}
			});
		...
		for (let indexLOD = 0; indexLOD < this._materialPromiseLODs.length; indexLOD++) {
			const promise = Promise.all(this._materialPromiseLODs[indexLOD]).then(() => {
				...
				if (indexLOD !== this._materialPromiseLODs.length - 1) {
					...
					if (this._materialSignalLODs[indexLOD]) this._materialSignalLODs[indexLOD].resolve();
				}
			});
			this._loader._completePromises.push(promise);
		}
	...
	loadNodeAsync(context, node, assign) {
		return GLTFLoader.LoadExtensionAsync(context, node, this.name, (extensionContext, extension) => {
			...
			for (let indexLOD = 0; indexLOD < nodeLODs.length; indexLOD++) {
				...
				if (indexLOD === 0) firstPromise = promise;
				else {
					this._nodeIndexLOD = null;
	...
	*/
	_loadMaterialAsync(context, material, babylonMesh, babylonDrawMode, assign) {
		if (this._nodeIndexLOD) return null;
		return GLTFLoader.LoadExtensionAsync(context, material, this.name, (extensionContext, extension) => {
			let firstPromise;
			...
			for (let indexLOD = 0; indexLOD < materialLODs.length; indexLOD++) {
				...
				if (indexLOD !== 0) this._materialIndexLOD = indexLOD;
				const promise = this._loader._loadMaterialAsync(`/materials/${materialLOD.index}`, materialLOD, babylonMesh, babylonDrawMode, (babylonMaterial) => {
					if (indexLOD === 0) assign(babylonMaterial);
	...
	*/
	loadBufferAsync(context, buffer, byteOffset, byteLength) {
		if (this._loader.parent.useRangeRequests && !buffer.uri) {
			if (!this._loader.bin) throw new Error(`${context}: Uri is missing or the binary glTF is missing its binary chunk`);
			...
			this._loader.log(`deferred`);
			if (this._nodeIndexLOD !== null) return loadAsync(this._nodeBufferLODs, this._nodeIndexLOD);
			else if (this._materialIndexLOD !== null) return loadAsync(this._materialBufferLODs, this._materialIndexLOD);
			else return loadAsync(this._bufferLODs, 0);
		}
		return null;
	}
	...
	*/
	_getLODs(context, property, array, ids) {
		...
		for (let i = ids.length - 1; i >= 0; i--) {
			...
			if (properties.length === this.maxLODsToLoad) return properties;
		}
	...
	_disposeTransformNode(babylonTransformNode) {
		...
		if (babylonMaterial) babylonMaterials.push(babylonMaterial);
		for (const babylonMesh of babylonTransformNode.getChildMeshes()) if (babylonMesh.material) babylonMaterials.push(babylonMesh.material);
	...
	_disposeMaterials(babylonMaterials) {
		...
		for (const babylonMaterial of babylonMaterials) {
			for (const babylonTexture of babylonMaterial.getActiveTextures()) babylonTextures[babylonTexture.uniqueId] = babylonTexture;
			babylonMaterial.dispose();
		}
		for (const uniqueId in babylonTextures) for (const babylonMaterial of this._loader.babylonScene.materials) if (babylonMaterial.hasTexture(babylonTextures[uniqueId])) delete babylonTextures[uniqueId];
		for (const uniqueId in babylonTextures) babylonTextures[uniqueId].dispose();
	}
};
unregisterGLTFExtension(NAME$7);
registerGLTFExtension(NAME$7, true, (loader) => new MSFT_lod(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/MSFT_minecraftMesh.js
var NAME$6 = "MSFT_minecraftMesh";
/** @internal */
var MSFT_minecraftMesh = class {
	...
	loadMaterialPropertiesAsync(context, material, babylonMaterial) {
		return GLTFLoader.LoadExtraAsync(context, material, this.name, (extraContext, extra) => {
			if (extra) {
				if (!(babylonMaterial instanceof PBRMaterial)) throw new Error(`${extraContext}: Material type not supported`);
...
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/MSFT_sRGBFactors.js
var NAME$5 = "MSFT_sRGBFactors";
/** @internal */
	...
	loadMaterialPropertiesAsync(context, material, babylonMaterial) {
		return GLTFLoader.LoadExtraAsync(context, material, this.name, (extraContext, extra) => {
			if (extra) {
				...
				if (!babylonMaterial.albedoTexture) babylonMaterial.albedoColor.toLinearSpaceToRef(babylonMaterial.albedoColor, useExactSrgbConversions);
				if (!babylonMaterial.reflectivityTexture) babylonMaterial.reflectivityColor.toLinearSpaceToRef(babylonMaterial.reflectivityColor, useExactSrgbConversions);
				return promise;
			}
			return null;
		});
...
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_interactivity/declarationMapper.js
function getMappingForFullOperationName(fullOperationName) {
	const [op, extension] = fullOperationName.split(":");
	return getMappingForDeclaration({
		op,
		extension
	});
}
	...
	if (!mapping) {
		...
		if (returnNoOpIfNotAvailable) {
			...
			if (declaration.inputValueSockets) {
				inputs.values = {};
				for (const key in declaration.inputValueSockets) inputs.values[key] = { name: key };
			}
			if (declaration.outputValueSockets) {
				outputs.values = {};
				Object.keys(declaration.outputValueSockets).forEach((key) => {
					outputs.values[key] = { name: key };
				});
			}
			return {
				blocks: [],
...
var gltfExtensionsToFlowGraphMapping = { BABYLON: { "flow/log": {
...
var gltfToFlowGraphMapping = {
	...
	"event/send": {
		...
		extraProcessor(gltfBlock, declaration, _mapping, parser, serializedObjects) {
			if (declaration.op !== "event/send" || !gltfBlock.configuration || Object.keys(gltfBlock.configuration).length !== 1) throw new Error("Receive event should have a single configuration object, the event itself");
			const eventId = gltfBlock.configuration["event"].value[0];
			if (typeof eventId !== "number") throw new Error("Event id should be a number");
	...
	"event/receive": {
		...
		validation(gltfBlock, interactivityGraph) {
			...
			if (!interactivityGraph.events?.[eventId]) {
		...
		extraProcessor(gltfBlock, declaration, _mapping, parser, serializedObjects) {
			if (declaration.op !== "event/receive" || !gltfBlock.configuration || Object.keys(gltfBlock.configuration).length !== 1) throw new Error("Receive event should have a single configuration object, the event itself");
			const eventId = gltfBlock.configuration["event"].value[0];
			if (typeof eventId !== "number") throw new Error("Event id should be a number");
			const event = parser.arrays.events[eventId];
			const serializedObject = serializedObjects[0];
	...
	"math/e": getSimpleInputMapping("FlowGraphEBlock"),
	"math/pi": getSimpleInputMapping("FlowGraphPIBlock"),
	"math/inf": getSimpleInputMapping("FlowGraphInfBlock"),
	"math/nan": getSimpleInputMapping("FlowGraphNaNBlock"),
	"math/abs": getSimpleInputMapping("FlowGraphAbsBlock"),
	"math/sign": getSimpleInputMapping("FlowGraphSignBlock"),
	"math/trunc": getSimpleInputMapping("FlowGraphTruncBlock"),
	"math/floor": getSimpleInputMapping("FlowGraphFloorBlock"),
	"math/ceil": getSimpleInputMapping("FlowGraphCeilBlock"),
	"math/round": {
	...
	"math/fract": getSimpleInputMapping("FlowGraphFractBlock"),
	"math/neg": getSimpleInputMapping("FlowGraphNegationBlock"),
	...
	"math/mul": {
		...
		extraProcessor(_gltfBlock, _declaration, _mapping, _parser, serializedObjects) {
			...
			if (type !== -1) serializedObjects[0].config.type = _parser.arrays.types[type].flowGraphType;
			return serializedObjects;
		}
	},
	...
	"math/clamp": getSimpleInputMapping("FlowGraphClampBlock", [
		"a",
		"b",
		"c"
	]),
	"math/saturate": getSimpleInputMapping("FlowGraphSaturateBlock"),
	"math/mix": getSimpleInputMapping("FlowGraphMathInterpolationBlock", [
		"a",
		"b",
		"c"
	]),
	...
	"math/isnan": getSimpleInputMapping("FlowGraphIsNaNBlock"),
	"math/isinf": getSimpleInputMapping("FlowGraphIsInfBlock"),
	"math/select": {
	...
	"math/sin": getSimpleInputMapping("FlowGraphSinBlock"),
	"math/cos": getSimpleInputMapping("FlowGraphCosBlock"),
	"math/tan": getSimpleInputMapping("FlowGraphTanBlock"),
	"math/asin": getSimpleInputMapping("FlowGraphASinBlock"),
	"math/acos": getSimpleInputMapping("FlowGraphACosBlock"),
	"math/atan": getSimpleInputMapping("FlowGraphATanBlock"),
	"math/atan2": getSimpleInputMapping("FlowGraphATan2Block", ["a", "b"]),
	"math/sinh": getSimpleInputMapping("FlowGraphSinhBlock"),
	"math/cosh": getSimpleInputMapping("FlowGraphCoshBlock"),
	"math/tanh": getSimpleInputMapping("FlowGraphTanhBlock"),
	"math/asinh": getSimpleInputMapping("FlowGraphASinhBlock"),
	"math/acosh": getSimpleInputMapping("FlowGraphACoshBlock"),
	"math/atanh": getSimpleInputMapping("FlowGraphATanhBlock"),
	"math/exp": getSimpleInputMapping("FlowGraphExponentialBlock"),
	"math/log": getSimpleInputMapping("FlowGraphLogBlock"),
	"math/log2": getSimpleInputMapping("FlowGraphLog2Block"),
	"math/log10": getSimpleInputMapping("FlowGraphLog10Block"),
	"math/sqrt": getSimpleInputMapping("FlowGraphSquareRootBlock"),
	"math/cbrt": getSimpleInputMapping("FlowGraphCubeRootBlock"),
	"math/pow": getSimpleInputMapping("FlowGraphPowerBlock", ["a", "b"]),
	"math/length": getSimpleInputMapping("FlowGraphLengthBlock"),
	"math/normalize": getSimpleInputMapping("FlowGraphNormalizeBlock"),
	...
	"math/rotate3d": getSimpleInputMapping("FlowGraphRotate3DBlock", [
		"a",
		"b",
		"c"
	]),
	...
	"math/transpose": getSimpleInputMapping("FlowGraphTransposeBlock"),
	"math/determinant": getSimpleInputMapping("FlowGraphDeterminantBlock"),
	"math/inverse": getSimpleInputMapping("FlowGraphInvertMatrixBlock"),
	...
	"math/matCompose": {
		...
		extraProcessor(_gltfBlock, _declaration, _mapping, _parser, serializedObjects, context) {
			...
			if (!d) throw new Error("Rotation quaternion input not found");
			if (context._connectionValues[d.uniqueId]) context._connectionValues[d.uniqueId].type = "Quaternion";
			return serializedObjects;
		}
	},
	"math/matDecompose": {
	...
	"math/clz": getSimpleInputMapping("FlowGraphLeadingZerosBlock"),
	"math/ctz": getSimpleInputMapping("FlowGraphTrailingZerosBlock"),
	"math/popcnt": getSimpleInputMapping("FlowGraphOneBitsCounterBlock"),
	"math/rad": getSimpleInputMapping("FlowGraphDegToRadBlock"),
	"math/deg": getSimpleInputMapping("FlowGraphRadToDegBlock"),
	"type/boolToInt": getSimpleInputMapping("FlowGraphBooleanToInt"),
	"type/boolToFloat": getSimpleInputMapping("FlowGraphBooleanToFloat"),
	"type/intToBool": getSimpleInputMapping("FlowGraphIntToBoolean"),
	"type/intToFloat": getSimpleInputMapping("FlowGraphIntToFloat"),
	"type/floatToInt": getSimpleInputMapping("FlowGraphFloatToInt"),
	"type/floatToBool": getSimpleInputMapping("FlowGraphFloatToBoolean"),
	"flow/sequence": {
	...
	"flow/switch": {
		...
		extraProcessor(gltfBlock, declaration, _mapping, _arrays, serializedObjects) {
			if (declaration.op !== "flow/switch" || !gltfBlock.flows || Object.keys(gltfBlock.flows).length === 0) throw new Error("Switch should have a single configuration object, the cases array");
			serializedObjects[0].signalOutputs.forEach((output) => {
				if (output.name !== "default") output.name = "out_" + output.name;
			});
			return serializedObjects;
		}
	},
	...
	"flow/multiGate": {
		...
		extraProcessor(gltfBlock, declaration, _mapping, _arrays, serializedObjects) {
			if (declaration.op !== "flow/multiGate" || !gltfBlock.flows || Object.keys(gltfBlock.flows).length === 0) throw new Error("MultiGate should have a single configuration object, the number of output flows");
			const serializedObject = serializedObjects[0];
			serializedObject.config || (serializedObject.config = {});
			serializedObject.config.outputSignalCount = Object.keys(gltfBlock.flows).length;
			serializedObject.signalOutputs.forEach((output, index) => {
	...
	"flow/waitAll": {
		...
		inputs: { flows: { "[segment]": { name: "in_$1" } } },
		validation(gltfBlock) {
			if (typeof gltfBlock.configuration?.inputFlows?.value[0] !== "number") {
				gltfBlock.configuration = gltfBlock.configuration || { inputFlows: { value: [0] } };
	...
	"variable/setMultiple": {
		...
		extraProcessor(_gltfBlock, _declaration, _mapping, parser, serializedObjects) {
			serializedObjects[0].dataInputs.forEach((input) => {
		...
		configuration: {
			...
			useSlerp: {
				...
				dataTransformer: (value) => {
					if (value[0] === true) return ["Quaternion"];
					else return [void 0];
				}
			}
		},
				...
				if (serializedObject.className === "FlowGraphJsonPointerParserBlock") {
					...
					Object.keys(gltfBlock.values || []).forEach((key) => {
						...
						if (key === "value" && value) {
							...
							if (type !== void 0) serializedObject.config.animationType = parser.arrays.types[type].flowGraphType;
						}
					});
				}
			});
	...
	"animation/start": {
		...
		inputs: { values: {
			...
			startTime: {
				name: "from",
				gltfType: "number",
				dataTransformer: (time, parser) => [time[0] * parser._loader.parent.targetFps]
			},
			endTime: {
				name: "to",
				gltfType: "number",
	...
	"math/switch": {
		...
		extraProcessor(_gltfBlock, _declaration, _mapping, _arrays, serializedObjects) {
			serializedObjects[0].dataInputs.forEach((input) => {
				if (input.name !== "default" && input.name !== "case") input.name = "in_" + input.name;
			});
			return serializedObjects;
		}
	},
...
*/
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_interactivity/interactivityGraphParser.js
...
var InteractivityGraphToFlowGraphParser = class {
	...
	_parseTypes() {
		if (!this._interactivityGraph.types) return;
		for (const type of this._interactivityGraph.types) this._types.push(gltfTypeToBabylonType[type.signature]);
	}
	_parseDeclarations() {
		if (!this._interactivityGraph.declarations) return;
	...
	_parseVariables() {
		if (!this._interactivityGraph.variables) return;
	...
	_parseVariable(variable, dataTransform) {
		...
		if (!value.length) switch (type.flowGraphType) {
			case "boolean":
				value.push(false);
				break;
		...
		return {
			...
			value: dataTransform ? dataTransform(value, this) : value
		};
	}
	_parseEvents() {
		if (!this._interactivityGraph.events) return;
		for (const event of this._interactivityGraph.events) {
			const converted = { eventId: event.id || "internalEvent_" + this._internalEventsCounter++ };
			if (event.values) converted.eventData = Object.keys(event.values).map((key) => {
	...
	_parseNodes() {
		if (!this._interactivityGraph.nodes) return;
		for (const node of this._interactivityGraph.nodes) {
			...
			if (mapping.flowGraphMapping.validation) {
				if (!mapping.flowGraphMapping.validation(node, this._interactivityGraph, this._gltf)) throw new Error(`Error validating interactivity node ${node}`);
			}
			const blocks = [];
			...
			this._nodes.push({
				blocks,
				fullOperationName: mapping.fullOperationName
			});
		}
	}
	...
	_parseNodeConfiguration(node, block, nodeMapping, blockType) {
		...
		if (node.configuration) Object.keys(node.configuration).forEach((key) => {
			...
			if (configMapping && configMapping.toBlock ? configMapping.toBlock === blockType : nodeMapping.blocks.indexOf(blockType) === 0) {
				const configKey = configMapping?.name || key;
				if ((!value || typeof value.value === "undefined") && typeof configMapping?.defaultValue !== "undefined") configuration[configKey] = { value: configMapping.defaultValue };
				else if (value.value.length >= 1) configuration[configKey] = { value: value.value.length === 1 ? value.value[0] : value.value };
				else Logger.Warn(["Invalid value for node configuration", value]);
				if (configMapping && configMapping.dataTransformer) configuration[configKey].value = configMapping.dataTransformer([configuration[configKey].value], this)[0];
			}
		});
	}
	_parseNodeConnections(context) {
		for (let i = 0; i < this._nodes.length; i++) {
			...
			for (const flowKey of flowsKeys) {
				...
				(flowMapping && flowMapping.toBlock && flowGraphBlocks.blocks.find((b) => b.className === flowMapping.toBlock) || flowGraphBlocks.blocks[0]).signalOutputs.push(socketOut);
				...
				if (!flowInMapping) {
					for (const key in inputMapper.inputs?.flows) if (key.startsWith("[") && key.endsWith("]")) {
						arrayMapping = true;
			...
			for (const valueKey of valuesKeys) {
				...
				let arrayMapping = false;
				if (!valueMapping) {
					for (const key in outputMapper.flowGraphMapping.inputs?.values) if (key.startsWith("[") && key.endsWith("]")) {
						arrayMapping = true;
				...
				(valueMapping && valueMapping.toBlock && flowGraphBlocks.blocks.find((b) => b.className === valueMapping.toBlock) || flowGraphBlocks.blocks[0]).dataInputs.push(socketIn);
				if (value.value !== void 0) {
					...
					let arrayMapping = false;
					if (!valueMapping) {
						for (const key in outputMapper.outputs?.values) if (key.startsWith("[") && key.endsWith("]")) {
							arrayMapping = true;
			...
			if (outputMapper.flowGraphMapping.interBlockConnectors) for (const connector of outputMapper.flowGraphMapping.interBlockConnectors) {
	...
	_connectFlowGraphNodes(input, output, serializedInput, serializedOutput, isVariable) {
		...
		if (!inputArray.find((s) => s.name === input)) inputArray.push(inputConnection);
		if (!outputArray.find((s) => s.name === output)) outputArray.push(outputConnection);
	...
	serializeToFlowGraph() {
		...
		return {
			rightHanded: true,
			allBlocks: this._nodes.reduce((acc, val) => acc.concat(val.blocks), []),
			executionContexts: [context]
		};
	}
};
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_interactivity.js
var NAME$4 = "KHR_interactivity";
/**
	...
	*/
	constructor(_loader) {
		...
		*/
		...
		if (scene) _AddInteractivityObjectModel(scene);
	}
	dispose() {
		this._loader = null;
	...
	async onReady() {
		...
		if (!interactivityDefinition) return;
		const coordinator = new FlowGraphCoordinator({ scene });
		const graphs = interactivityDefinition.graphs.map((graph) => {
			return new InteractivityGraphToFlowGraphParser(graph, this._loader.gltf, this._loader).serializeToFlowGraph();
		});
		await Promise.all(graphs.map((graph) => ParseFlowGraphAsync(graph, {
			coordinator,
...
function _AddInteractivityObjectModel(scene) {
	AddObjectAccessorToKey("/extensions/KHR_interactivity/?/activeCamera/rotation", {
		get: () => {
			if (!scene.activeCamera) return new Quaternion(NaN, NaN, NaN, NaN);
	...
	AddObjectAccessorToKey("/extensions/KHR_interactivity/?/activeCamera/position", {
		get: () => {
			if (!scene.activeCamera) return new Vector3(NaN, NaN, NaN);
...
addToBlockFactory(NAME$4, "FlowGraphGLTFDataProvider", async () => {
	return (await import("./flowGraphGLTFDataProvider-BHEBrXWl.js")).FlowGraphGLTFDataProvider;
});
unregisterGLTFExtension(NAME$4);
registerGLTFExtension(NAME$4, true, (loader) => new KHR_interactivity(loader));
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_node_visibility.js
var NAME$3 = "KHR_node_visibility";
AddObjectAccessorToKey("/nodes/{}/extensions/KHR_node_visibility/visible", {
	get: (node) => {
		const tn = node._babylonTransformNode;
		if (tn && tn.isVisible !== void 0) return tn.isVisible;
		return true;
	},
	set: (value, node) => {
		...
		if (node._babylonTransformNode) node._babylonTransformNode.isVisible = value;
		node._primitiveBabylonMeshes?.forEach((mesh) => {
		...
		this._loader.gltf.nodes?.forEach((node) => {
			...
			if (node.extensions?.KHR_node_visibility) {
				if (node.extensions?.KHR_node_visibility.visible === false) {
					if (node._babylonTransformNode) node._babylonTransformNode.isVisible = false;
					node._primitiveBabylonMeshes?.forEach((mesh) => {
...
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_node_selectability.js
var NAME$2 = "KHR_node_selectability";
addNewInteractivityFlowGraphMapping("event/onSelect", NAME$2, {
	...
	extraProcessor(gltfBlock, _declaration, _mapping, _arrays, serializedObjects, context, globalGLTF) {
		...
		if (nodeIndex === void 0 || typeof nodeIndex !== "number") throw new Error("nodeIndex not found in configuration");
...
AddObjectAccessorToKey("/nodes/{}/extensions/KHR_node_selectability/selectable", {
	get: (node) => {
		const tn = node._babylonTransformNode;
		if (tn && tn.isPickable !== void 0) return tn.isPickable;
		return true;
	},
	set: (value, node) => {
		node._primitiveBabylonMeshes?.forEach((mesh) => {
...
var KHR_node_selectability = class {
	...
	*/
	...
	async onReady() {
		this._loader.gltf.nodes?.forEach((node) => {
			if (node.extensions?.KHR_node_selectability && node.extensions?.KHR_node_selectability.selectable === false) node._babylonTransformNode?.getChildMeshes().forEach((mesh) => {
...
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/KHR_node_hoverability.js
var NAME$1 = "KHR_node_hoverability";
...
AddObjectAccessorToKey("/nodes/{}/extensions/KHR_node_hoverability/hoverable", {
	get: (node) => {
		const tn = node._babylonTransformNode;
		if (tn && tn.pointerOverDisableMeshTesting !== void 0) return tn.pointerOverDisableMeshTesting;
		return true;
	},
	set: (value, node) => {
		node._primitiveBabylonMeshes?.forEach((mesh) => {
...
var KHR_node_hoverability = class {
	...
	*/
	...
	async onReady() {
		this._loader.gltf.nodes?.forEach((node) => {
			if (node.extensions?.KHR_node_hoverability && node.extensions?.KHR_node_hoverability.hoverable === false) node._babylonTransformNode?.getChildMeshes().forEach((mesh) => {
...
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/index.js
var _2_0_exports = /* @__PURE__ */ __exportAll({
...
//#endregion
export { _1_0_exports as GLTF1, _2_0_exports as GLTF2, GLTFFileLoader, GLTFLoaderAnimationStartMode, GLTFLoaderCoordinateSystemMode, GLTFLoaderState, GLTFValidation };
