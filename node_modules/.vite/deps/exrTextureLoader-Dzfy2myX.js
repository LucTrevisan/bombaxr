import { f as __exportAll, t as Clamp } from "./math.scalar.functions-7SKaVf9_.js";
import { t as Logger } from "./logger-BsklRjyg.js";
import { n as Tools } from "./tools-B3Kj9lR8.js";
//#region node_modules/@babylonjs/core/Materials/Textures/Loaders/EXR/exrLoader.interfaces.js
var USHORT_RANGE = 65536;
var BITMAP_SIZE = USHORT_RANGE >> 3;
var HUF_ENCSIZE = 65537;
var HUF_DECSIZE = 16384;
var HUF_DECMASK = HUF_DECSIZE - 1;
//#endregion
//#region node_modules/@babylonjs/core/Materials/Textures/Loaders/EXR/exrLoader.core.js
/**
...
function ToHalfFloat(value) {
	if (Math.abs(value) > 65504) throw new Error("Value out of range.Consider using float instead of half-float.");
...
function ParseValue(dataView, offset, type, size) {
	switch (type) {
		case "string":
		case "stringvector":
		case "iccProfile": return ParseFixedLengthString(dataView.buffer, offset, size);
		case "chlist": return ParseChlist(dataView, offset, size);
		case "chromaticities": return ParseChromaticities(dataView, offset);
		case "compression": return ParseCompression(dataView, offset);
		case "box2i": return ParseBox2i(dataView, offset);
		case "lineOrder": return ParseLineOrder(dataView, offset);
		case "float": return ParseFloat32(dataView, offset);
		case "v2f": return ParseV2f(dataView, offset);
		case "v3f": return ParseV3f(dataView, offset);
		case "int": return ParseInt32(dataView, offset);
		case "rational": return ParseRational(dataView, offset);
		case "timecode": return ParseTimecode(dataView, offset);
		case "preview":
			offset.value += size;
			return "skipped";
		default:
...
//#endregion
//#region node_modules/@babylonjs/core/Materials/Textures/Loaders/EXR/exrLoader.header.js
/**
* Inspired by https://github.com/sciecode/three.js/blob/dev/examples/jsm/loaders/EXRLoader.js
* Referred to the original Industrial Light & Magic OpenEXR implementation and the TinyEXR / Syoyo Fujita
* implementation.
...
function GetExrHeader(dataView, offset) {
	if (dataView.getUint32(0, true) != EXR_MAGIC) throw new Error("Incorrect OpenEXR format");
	...
	while (keepReading) {
		...
		if (!attributeName) keepReading = false;
			...
			const attributeValue = ParseValue(dataView, offset, attributeType, ParseUint32(dataView, offset));
			if (attributeValue === void 0) Logger.Warn(`Unknown header attribute type ${attributeType}'.`);
			else headerData[attributeName] = attributeValue;
		}
	}
...
//#endregion
//#region node_modules/@babylonjs/core/Materials/Textures/Loaders/EXR/exrLoader.compression.huf.js
/**
* Inspired by https://github.com/sciecode/three.js/blob/dev/examples/jsm/loaders/EXRLoader.js
* Referred to the original Industrial Light & Magic OpenEXR implementation and the TinyEXR / Syoyo Fujita
* implementation.
...
function GetCode(po, rlc, c, lc, array, offset, outBuffer, outBufferOffset, outBufferEndOffset) {
	if (po == rlc) {
		...
		if (outBufferOffset.value + cs > outBufferEndOffset) return null;
...
function HufUnpackEncTable(array, offset, ni, im, iM, hcode) {
	...
	for (; im <= iM; im++) {
		...
		hcode[im] = l;
		if (l == 63) {
			if (p.value - offset.value > ni) throw new Error("Error in HufUnpackEncTable");
...
function HufUncompress(array, dataView, offset, nCompressed, outBuffer, nRaw) {
	...
	offset.value += 4;
	if (im < 0 || im >= 65537 || iM < 0 || iM >= 65537) throw new Error("Wrong HUF_ENCSIZE");
	const freq = new Array(HUF_ENCSIZE);
	const hdec = new Array(HUF_DECSIZE);
	HufClearDecTable(hdec);
	HufUnpackEncTable(array, offset, nCompressed - (offset.value - initialInOffset), im, iM, freq);
	if (nBits > 8 * (nCompressed - (offset.value - initialInOffset))) throw new Error("Wrong hufUncompress");
	...
	while (p >= 1) {
		...
		for (; py <= ey; py += oy2) {
			...
			if (nx & p) {
				const p10 = px + oy1;
				let result;
				if (w14) result = Wdec14(buffer[px + j], buffer[p10 + j]);
				else result = Wdec16(buffer[px + j], buffer[p10 + j]);
				i00 = result.a;
		...
		if (ny & p) {
			...
			for (; px <= ex; px += ox2) {
				const p01 = px + ox1;
				let result;
				if (w14) result = Wdec14(buffer[px + j], buffer[p01 + j]);
				else result = Wdec16(buffer[px + j], buffer[p01 + j]);
				i00 = result.a;
				buffer[p01 + j] = result.b;
				buffer[px + j] = i00;
			}
...
//#endregion
//#region node_modules/@babylonjs/core/Materials/Textures/Loaders/EXR/exrLoader.compression.rle.js
/**
* Inspired by https://github.com/sciecode/three.js/blob/dev/examples/jsm/loaders/EXRLoader.js
* Referred to the original Industrial Light & Magic OpenEXR implementation and the TinyEXR / Syoyo Fujita
* implementation.
...
//#endregion
//#region node_modules/@babylonjs/core/Materials/Textures/Loaders/EXR/exrLoader.compression.js
/**
* No compression
...
function UncompressPXR(decoder) {
	...
	for (let y = 0; y < decoder.lines; y++) for (let c = 0; c < decoder.channels; c++) {
		let pixel = 0;
		switch (decoder.type) {
			case 1:
				ptr[0] = tmpBufferEnd;
...
function UncompressPIZ(decoder) {
	...
	for (let y = 0; y < decoder.lines; y++) for (let c = 0; c < decoder.channels; c++) {
		...
		const cp = new Uint8Array(outBuffer.buffer, cd.end * 2, n * 2);
...
//#endregion
//#region node_modules/@babylonjs/core/Materials/Textures/Loaders/EXR/exrLoader.configuration.js
var EXROutputType;
...
//#endregion
//#region node_modules/@babylonjs/core/Materials/Textures/Loaders/EXR/exrLoader.decoder.js
/**
* Inspired by https://github.com/sciecode/three.js/blob/dev/examples/jsm/loaders/EXRLoader.js
* Referred to the original Industrial Light & Magic OpenEXR implementation and the TinyEXR / Syoyo Fujita
* implementation.
...
async function CreateDecoderAsync(header, dataView, offset, outputType) {
	...
	switch (header.compression) {
		...
		case CompressionCodes.PXR24_COMPRESSION:
			...
			await Tools.LoadScriptAsync(ExrLoaderGlobalConfiguration.FFLATEUrl);
			break;
		default: throw new Error(CompressionCodes[header.compression] + " is unsupported");
	}
	decoder.scanlineBlockSize = decoder.lines;
	const channels = {};
	for (const channel of header.channels) switch (channel.name) {
		case "Y":
		case "R":
		case "G":
		case "B":
	...
	if (decoder.type === 1) switch (outputType) {
	...
	else if (decoder.type === 2) switch (outputType) {
		case EXROutputType.Float:
	...
	else throw new Error("Unsupported pixelType " + decoder.type + " for " + header.compression);
	...
	switch (outputType) {
		case EXROutputType.Float:
			...
			if (fillAlpha) decoder.byteArray.fill(1, 0, size);
			break;
		case EXROutputType.HalfFloat:
			decoder.byteArray = new Uint16Array(size);
			decoder.textureType = 2;
			if (fillAlpha) decoder.byteArray.fill(15360, 0, size);
			break;
		default: throw new Error("Unsupported type: " + outputType);
	}
	let byteOffset = 0;
	for (const channel of header.channels) {
		if (decoder.decodeChannels[channel.name] !== void 0) decoder.channelLineOffsets[channel.name] = byteOffset * decoder.width;
		byteOffset += channel.pixelType * 2;
	}
	decoder.bytesPerLine = decoder.width * byteOffset;
	decoder.outLineWidth = decoder.width * decoder.outputChannels;
	if (header.lineOrder === "INCREASING_Y") decoder.scanOrder = (y) => y;
	else decoder.scanOrder = (y) => decoder.height - 1 - y;
...
function ScanData(decoder, header, dataView, offset) {
	...
	for (let scanlineBlockIdx = 0; scanlineBlockIdx < decoder.height / decoder.scanlineBlockSize; scanlineBlockIdx++) {
		...
		const viewer = decoder.size < decoder.lines * decoder.bytesPerLine && decoder.uncompress ? decoder.uncompress(decoder) : UncompressRAW(decoder);
		...
		for (let line_y = 0; line_y < decoder.scanlineBlockSize; line_y++) {
			...
			if (true_y >= decoder.height) continue;
			...
			for (let channelID = 0; channelID < decoder.channels; channelID++) {
				...
				for (let x = 0; x < decoder.width; x++) {
					...
					if (decoder.byteArray) decoder.byteArray[outIndex] = decoder.getter(viewer, tmpOffset);
				}
			}
		}
	}
...
//#endregion
//#region node_modules/@babylonjs/core/Materials/Textures/Loaders/exrTextureLoader.js
var exrTextureLoader_exports = /* @__PURE__ */ __exportAll({ _ExrTextureLoader: () => _ExrTextureLoader });
/**
* Inspired by https://github.com/sciecode/three.js/blob/dev/examples/jsm/loaders/EXRLoader.js
* Referred to the original Industrial Light & Magic OpenEXR implementation and the TinyEXR / Syoyo Fujita
* implementation.
...
var _ExrTextureLoader = class {
	...
	*/
	async loadData(data, texture, callback) {
		...
		callback(header.dataWindow.xMax - header.dataWindow.xMin + 1, header.dataWindow.yMax - header.dataWindow.yMin + 1, texture.generateMipMaps, false, () => {
			const engine = texture.getEngine();
			texture.format = header.format;
			texture.type = decoder.textureType;
			texture.invertY = false;
			texture._gammaSpace = !header.linearSpace;
			if (decoder.byteArray) engine._uploadDataToTextureDirectly(texture, decoder.byteArray, 0, 0, void 0, true);
		});
	}
};
//#endregion
export { ParseValue as A, ParseInt32 as C, ParseUint32 as D, ParseUint16 as E, ParseUint8 as O, ParseFloat32 as S, ParseNullTerminatedString as T, GetExrHeader as _, EXROutputType as a, InterleaveScalar as b, UncompressPXR as c, UncompressZIP as d, DecodeRunLength as f, Wav2Decode as g, ReverseLutFromBitmap as h, ScanData as i, Predictor as j, ParseUint8Array as k, UncompressRAW as l, HufUncompress as m, exrTextureLoader_exports as n, ExrLoaderGlobalConfiguration as o, ApplyLut as p, CreateDecoderAsync as r, UncompressPIZ as s, _ExrTextureLoader as t, UncompressRLE as u, CompressionCodes as v, ParseInt64 as w, ParseFloat16 as x, DecodeFloat32 as y };

//# sourceMappingURL=exrTextureLoader-Dzfy2myX.js.map