...
import { E as getRichTypeByFlowGraphType, T as getRichTypeByAnimationType, t as FlowGraphBlock, u as RichTypeAny, v as RichTypeNumber } from "./flowGraphBlock-Cu7z1xvN.js";
...
var FlowGraphInterpolationBlock = class extends FlowGraphBlock {
	constructor(config = {}) {
		...
		*/
		...
		for (let i = 1; i < numberOfKeyFrames + 1; i++) {
			const duration = this.registerDataInput(`duration_${i}`, RichTypeNumber, i === numberOfKeyFrames ? config.duration : void 0);
	...
	_updateOutputs(context) {
		...
		if (Array.isArray(animation)) for (const anim of animation) interpolationAnimations.push(anim.uniqueId);
		else interpolationAnimations.push(animation.uniqueId);
	...
	_createAnimation(context, propertyName, easingFunction) {
		...
		for (let i = 1; i < numberOfKeyFrames + 1; i++) {
			...
			if (i === numberOfKeyFrames - 1) value = value || type.defaultValue;
			if (duration !== void 0 && value) keys.push({
				frame: duration * 60,
				value
			});
		}
		const customBuildAnimation = this.customBuildAnimation.getValue(context);
		if (customBuildAnimation) return customBuildAnimation()(keys, 60, type.animationType, easingFunction);
		if (typeof propertyName === "string") {
			const animation = Animation.CreateAnimation(propertyName, type.animationType, 60, easingFunction);
			animation.setKeys(keys);
			return [animation];
		} else return propertyName.map((name) => {
...
//#endregion
export { FlowGraphInterpolationBlock as t };

//# sourceMappingURL=flowGraphInterpolationBlock-H8pHzkps.js.map