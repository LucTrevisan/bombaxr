import { i as ILog2 } from "./math.scalar.functions-7SKaVf9_.js";
import { t as Scene } from "./scene-DhWwr7VO.js";
import { a as Vector3 } from "./math.vector-Clv1Vglo.js";
import { t as Logger } from "./logger-BsklRjyg.js";
import { n as Tools } from "./tools-B3Kj9lR8.js";
import { d as InternalTexture } from "./tools.functions-Bl8BhFJU.js";
import { o as BaseTexture } from "./texture-BC46D7N_.js";
import { T as SphericalPolynomial, k as PostProcess, n as ApplyPostProcess, t as CubeMapToSphericalPolynomialTools } from "./cubemapToSphericalPolynomial-CYDqs8Y8.js";
import { n as DumpDataAsync } from "./dumpTools-kxWv909Y.js";
...
var RGBDTextureTools = class {
	...
	*/
	static ExpandRGBDTexture(texture) {
		...
		if (!internalTexture || !texture.isRGBD) return;
		...
		const expandRGBDTexture = async () => {
			...
			internalTexture.isReady = false;
			if (isWebGPU) await import("./rgbdDecode.fragment-DGMGCFma.js").then((n) => n.n);
			else await import("./rgbdDecode.fragment-CCnir1gr.js").then((n) => n.n);
			...
			rgbdPostProcess.onEffectCreatedObservable.addOnce((e) => {
				e.executeWhenCompiled(() => {
					...
					if (rgbdPostProcess) rgbdPostProcess.dispose();
					expandedTexture._swapAndDie(internalTexture);
					internalTexture.isReady = true;
				});
			});
		};
		if (expandTexture) if (isReady) expandRGBDTexture();
		else texture.onLoadObservable.addOnce(expandRGBDTexture);
	}
	/**
	...
	*/
	static async EncodeTextureToRGBD(internalTexture, scene, outputTextureType = 0) {
		if (!scene.getEngine().isWebGPU) await import("./rgbdEncode.fragment-BV5uaP22.js").then((n) => n.n);
		else await import("./rgbdEncode.fragment-DSjpsTsE.js").then((n) => n.n);
		return ApplyPostProcess("rgbdEncode", internalTexture, scene, outputTextureType, 1, 5);
	}
};
//#endregion
//#region node_modules/@babylonjs/core/Materials/Textures/baseTexture.polynomial.js
...
Object.defineProperty(BaseTexture.prototype, "sphericalPolynomial", {
		...
		if (this._texture) {
			...
			if (this._texture.isReady) {
				if (!this._texture._sphericalPolynomialPromise) {
					...
					if (this._texture._sphericalPolynomialPromise === null) this._texture._sphericalPolynomialComputed = true;
					else this._texture._sphericalPolynomialPromise.then((sphericalPolynomial) => {
	...
	set: function(value) {
		if (this._texture) this._texture._sphericalPolynomial = value;
	},
	enumerable: true,
	configurable: true
});
...
function GetEnvInfo(data) {
	...
	let pos = 0;
	for (let i = 0; i < MagicBytes.length; i++) if (dataView.getUint8(pos++) !== MagicBytes[i]) {
		Logger.Error("Not a babylon environment map");
		return null;
	}
	let manifestString = "";
	let charCode = 0;
	while (charCode = dataView.getUint8(pos++)) manifestString += String.fromCharCode(charCode);
	let manifest = JSON.parse(manifestString);
	manifest = normalizeEnvInfo(manifest);
	manifest.binaryDataPosition = pos;
	if (manifest.specular) manifest.specular.lodGenerationScale = manifest.specular.lodGenerationScale || .8;
	return manifest;
}
/**
...
function normalizeEnvInfo(info) {
	if (info.version > CurrentVersion) throw new Error(`Unsupported babylon environment map version "${info.version}". Latest supported version is "${CurrentVersion}".`);
...
async function CreateEnvTextureAsync(texture, options = {}) {
	...
	if (!internalTexture) return Promise.reject("The cube texture is invalid.");
	const engine = internalTexture.getEngine();
	if (texture.textureType !== 2 && texture.textureType !== 1 && texture.textureType !== 0 && texture.textureType !== 0 && texture.textureType !== 7 && texture.textureType !== -1) return Promise.reject("The cube texture should allow HDR (Full Float or Half Float).");
	let textureType = 1;
	if (!engine.getCaps().textureFloatRender) {
		textureType = 2;
		if (!engine.getCaps().textureHalfFloatRender) return Promise.reject("Env texture can only be created when the browser supports half float or full float rendering.");
	}
	...
	for (let i = 0; i <= mipmapsCount; i++) {
		...
		for (let face = 0; face < 6; face++) specularTextures[i * 6 + face] = await _getTextureEncodedData(hostingScene, texture, textureType, face, i, faceWidth, imageType, options.imageQuality);
	}
	...
	if (irradianceTexture) {
		...
		for (let face = 0; face < 6; face++) diffuseTextures[face] = await _getTextureEncodedData(hostingScene, irradianceTexture, textureType, face, 0, faceWidth, imageType, options.imageQuality);
	}
	hostingScene.dispose();
	if (sphericalPolynomialPromise) await sphericalPolynomialPromise;
	const info = {
	...
	let position = 0;
	for (let i = 0; i <= mipmapsCount; i++) for (let face = 0; face < 6; face++) {
	...
	for (let i = 0, strLen = infoString.length; i < strLen; i++) infoView[i] = infoString.charCodeAt(i);
	...
	let pos = 0;
	for (let i = 0; i < MagicBytes.length; i++) dataView.setUint8(pos++, MagicBytes[i]);
	...
	if (irradianceTexture) for (let face = 0; face < 6; face++) {
...
async function _getTextureEncodedData(hostingScene, texture, textureType, face, i, size, imageType, imageQuality) {
	...
	if (faceData && faceData.byteLength === faceData.length) {
		...
		for (let i = 0; i < floatData.length; i++) floatData[i] = Math.pow(floatData[i], 2.2);
	}
	const engine = hostingScene.getEngine();
	const tempTexture = engine.createRawTexture(faceData, size, size, 5, false, true, 1, null, textureType);
	await RGBDTextureTools.EncodeTextureToRGBD(tempTexture, hostingScene, textureType);
	const imageEncodedData = await DumpDataAsync(size, size, await engine._readTexturePixels(tempTexture, size, size), imageType, void 0, false, true, imageQuality);
	tempTexture.dispose();
...
function _CreateEnvTextureIrradiance(texture) {
	...
	if (polynmials == null) return null;
	return {
		x: [
			polynmials.x.x,
			polynmials.x.y,
...
function CreateRadianceImageDataArrayBufferViews(data, info) {
	...
	if (specularInfo.mipmaps.length !== 6 * mipmapsCount) throw new Error(`Unsupported specular mipmaps number "${specularInfo.mipmaps.length}"`);
...
function CreateIrradianceImageDataArrayBufferViews(data, info) {
	...
	if (irradianceTexture) {
		if (irradianceTexture.faces.length !== 6) throw new Error(`Incorrect irradiance texture faces number "${irradianceTexture.faces.length}"`);
		for (let face = 0; face < 6; face++) {
...
function UploadEnvLevelsAsync(texture, data, info) {
	info = normalizeEnvInfo(info);
	const specularInfo = info.specular;
	if (!specularInfo) return Promise.resolve([]);
	...
	return new Promise((resolve, reject) => {
		if (expandTexture) {
			...
			if (generateNonLODTextures) {
				...
				if (lodTexture) engine._uploadImageToTexture(lodTexture._texture, image, face, 0);
			}
			resolve();
		}
	});
...
async function UploadIrradianceLevelsAsync(mainTexture, imageData, size, imageType = DefaultEnvironmentTextureImageType) {
	...
	mainTexture._irradianceTexture = new BaseTexture(engine, texture);
	texture.isCube = true;
	texture.format = 5;
	texture.type = 0;
	texture.generateMipMaps = true;
...
async function _UploadLevelsAsync(texture, imageData, canGenerateNonLODTextures, imageType = DefaultEnvironmentTextureImageType) {
	if (!Tools.IsExponentOfTwo(texture.width)) throw new Error("Texture size must be a power of two");
	...
	} else if (!engine._features.supportRenderAndCopyToLodForFloatTextures) expandTexture = false;
	...
	if (expandTexture) {
		if (engine.isWebGPU) {
			shaderLanguage = 1;
			await import("./rgbdDecode.fragment-DGMGCFma.js").then((n) => n.n);
		} else await import("./rgbdDecode.fragment-CCnir1gr.js").then((n) => n.n);
	...
	const promises = [];
	for (let i = 0; i < imageData.length; i++) for (let face = 0; face < 6; face++) {
		const bytes = imageData[i][face];
		const blob = new Blob([bytes], { type: imageType });
		const url = URL.createObjectURL(blob);
		let promise;
		if (engine._features.forceBitmapOverHTMLImageElement) promise = engine.createImageBitmap(blob, { premultiplyAlpha: "none" }).then((img) => {
	...
	if (imageData.length < mipmapsCount) {
		...
		for (let i = imageData.length; i < mipmapsCount; i++) for (let face = 0; face < 6; face++) engine._uploadArrayBufferViewToTexture(cubeRtt?.texture || texture, data, face, i);
	}
	if (cubeRtt) {
	...
	if (rgbdPostProcess) rgbdPostProcess.dispose();
	if (generateNonLODTextures) {
		if (texture._lodTextureHigh && texture._lodTextureHigh._texture) texture._lodTextureHigh._texture.isReady = true;
		if (texture._lodTextureMid && texture._lodTextureMid._texture) texture._lodTextureMid._texture.isReady = true;
		if (texture._lodTextureLow && texture._lodTextureLow._texture) texture._lodTextureLow._texture.isReady = true;
	}
}
/**
...
function _UpdateRGBDAsync(internalTexture, data, sphericalPolynomial, lodScale, lodOffset) {
	const proxyPromise = UploadRadianceLevelsAsync(internalTexture.getEngine().createRawCubeTexture(null, internalTexture.width, internalTexture.format, internalTexture.type, internalTexture.generateMipMaps, internalTexture.invertY, internalTexture.samplingMode, internalTexture._compression), data).then(() => internalTexture);
...
//#endregion
export { GetEnvInfo as a, UploadIrradianceLevelsAsync as c, normalizeEnvInfo as d, RGBDTextureTools as f, EnvironmentTextureTools as i, UploadRadianceLevelsAsync as l, CreateIrradianceImageDataArrayBufferViews as n, UploadEnvLevelsAsync as o, CreateRadianceImageDataArrayBufferViews as r, UploadEnvSpherical as s, CreateEnvTextureAsync as t, _UpdateRGBDAsync as u };

//# sourceMappingURL=environmentTextureTools-BZsJxEYg.js.map