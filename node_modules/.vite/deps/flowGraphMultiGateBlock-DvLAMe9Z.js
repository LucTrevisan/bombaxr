...
import { A as FlowGraphInteger, m as RichTypeFlowGraphInteger } from "./flowGraphBlock-Cu7z1xvN.js";
...
var FlowGraphMultiGateBlock = class extends FlowGraphExecutionBlock {
	...
	_getNextIndex(indexesUsed) {
		if (!indexesUsed.includes(false)) {
			if (this.config.isLoop) indexesUsed.fill(false);
		}
		if (!this.config.isRandom) return indexesUsed.indexOf(false);
		else {
	...
	*/
	setNumberOfOutputSignals(numberOutputSignals = 1) {
		...
		while (this.outputSignals.length < numberOutputSignals) this.outputSignals.push(this._registerSignalOutput(`out_${this.outputSignals.length}`));
	}
	_execute(context, callingSignal) {
		if (!context._hasExecutionVariable(this, "indexesUsed")) context._setExecutionVariable(this, "indexesUsed", this.outputSignals.map(() => false));
...
//#endregion
export { FlowGraphMultiGateBlock as t };

//# sourceMappingURL=flowGraphMultiGateBlock-DvLAMe9Z.js.map