...
import { _ as __decorate, h as serializeAsVector3, p as serializeAsTexture, r as serialize } from "./decorators-C2fLRD2d.js";
...
var ShadowLight = class extends Light {
	...
	*/
	computeTransformedInformation() {
		if (this.parent && this.parent.getWorldMatrix) {
			if (!this.transformedPosition) this.transformedPosition = Vector3.Zero();
			Vector3.TransformCoordinatesToRef(this.position, this.parent.getWorldMatrix(), this.transformedPosition);
			if (this.direction) {
				if (!this.transformedDirection) this.transformedDirection = Vector3.Zero();
	...
	*/
	setShadowProjectionMatrix(matrix, viewMatrix, renderList) {
		if (this.customProjectionMatrixBuilder) this.customProjectionMatrixBuilder(viewMatrix, renderList, matrix);
		else this._setDefaultShadowProjectionMatrix(matrix, viewMatrix, renderList);
		return this;
	}
	/** @internal */
	...
	*/
	getViewMatrix(faceIndex) {
		...
		if (this.computeTransformedInformation()) lightPosition = this.transformedPosition;
		Vector3.NormalizeToRef(this.getShadowDirection(faceIndex), lightDirection);
		if (Math.abs(Vector3.Dot(lightDirection, Vector3.Up())) === 1) lightDirection.z = 1e-13;
...
*/
var SpotLight = class SpotLight extends ShadowLight {
	/**
	...
	*/
	...
	set iesProfileTexture(value) {
		if (this._iesProfileTexture === value) return;
		this._iesProfileTexture = value;
		if (this._iesProfileTexture && SpotLight._IsTexture(this._iesProfileTexture)) this._iesProfileTexture.onLoadObservable.addOnce(() => {
	...
	*/
	set angle(value) {
		this._angle = value;
		this._cosHalfAngle = Math.cos(value * .5);
	...
	*/
	set projectionTexture(value) {
		if (this._projectionTexture === value) return;
		this._projectionTexture = value;
		this._projectionTextureDirty = true;
		if (this._projectionTexture && !this._projectionTexture.isReady()) {
			if (SpotLight._IsProceduralTexture(this._projectionTexture)) this._projectionTexture.getEffect().executeWhenCompiled(() => {
				this._markMeshesAsLightDirty();
			});
			else if (SpotLight._IsTexture(this._projectionTexture)) this._projectionTexture.onLoadObservable.addOnce(() => {
				this._markMeshesAsLightDirty();
			});
		}
	}
	...
	*/
	constructor(name, position, direction, angle, exponent, scene) {
		...
		this._projectionTextureScalingMatrix = Matrix.FromValues(.5, 0, 0, 0, 0, .5, 0, 0, 0, 0, .5, 0, .5, .5, .5, 1);
	...
	*/
	...
	_computeProjectionTextureProjectionLightMatrix() {
		...
		Matrix.FromValuesToRef(S / 1, 0, 0, 0, 0, S, 0, 0, 0, 0, P, 1, 0, 0, Q, 0, this._projectionTextureProjectionLightMatrix);
	}
	/**
	...
	*/
	_computeProjectionTextureMatrix() {
		...
		if (this._projectionTexture instanceof Texture) {
			...
			Matrix.FromValuesToRef(u, 0, 0, 0, 0, v, 0, 0, 0, 0, .5, 0, .5, .5, .5, 1, this._projectionTextureScalingMatrix);
		}
	...
	*/
	transferTexturesToEffect(effect, lightIndex) {
		if (this.projectionTexture && this.projectionTexture.isReady()) {
			if (this._projectionTextureViewLightDirty) this._computeProjectionTextureViewLightMatrix();
			if (this._projectionTextureProjectionLightDirty) this._computeProjectionTextureProjectionLightMatrix();
			if (this._projectionTextureDirty) this._computeProjectionTextureMatrix();
			effect.setMatrix("textureProjectionMatrix" + lightIndex, this._projectionTextureMatrix);
			effect.setTexture("projectionLightTexture" + lightIndex, this.projectionTexture);
		}
		if (this._iesProfileTexture && this._iesProfileTexture.isReady()) effect.setTexture("iesLightTexture" + lightIndex, this._iesProfileTexture);
		return this;
	}
	/**
	...
	*/
	...
	transferToNodeMaterialEffect(effect, lightDataUniformName) {
		let normalizeDirection;
		if (this.computeTransformedInformation()) normalizeDirection = Vector3.Normalize(this.transformedDirection);
		else normalizeDirection = Vector3.Normalize(this.direction);
		if (this.getScene().useRightHandedSystem) effect.setFloat3(lightDataUniformName, -normalizeDirection.x, -normalizeDirection.y, -normalizeDirection.z);
		else effect.setFloat3(lightDataUniformName, normalizeDirection.x, normalizeDirection.y, normalizeDirection.z);
		return this;
	}
	/**
...
//#endregion
export { ShadowLight as n, SpotLight as t };

//# sourceMappingURL=spotLight-B4p4OWrG.js.map