import { a as Vector3, i as Vector2, n as Quaternion, t as Matrix } from "./math.vector-Clv1Vglo.js";
import { n as Color4 } from "./math.color-CXYOFsIw.js";
	...
	*/
	convert(path) {
		...
		if (parts[parts.length - 1].includes(".length")) {
			const split = parts[parts.length - 1].split(".");
			parts.pop();
			parts.push(...split);
		}
		...
		for (const part of parts) {
			...
			if (isLength && !infoTree.__array__) throw new Error(`Path ${path} is invalid`);
			if (infoTree.__ignoreObjectTree__) ignoreObjectTree = true;
			if (infoTree.__array__ && !isLength) infoTree = infoTree.__array__;
			else {
				infoTree = infoTree[part];
				if (!infoTree) throw new Error(`Path ${path} is invalid`);
			}
			if (!ignoreObjectTree) {
				if (objectTree === void 0) throw new Error(`Path ${path} is invalid`);
				if (!isLength) objectTree = objectTree?.[part];
			}
			if (infoTree.__target__ || isLength) target = objectTree;
		}
		return {
			object: target,
			info: infoTree
...
//#endregion
//#region node_modules/@babylonjs/loaders/glTF/2.0/Extensions/objectModelMapping.js
var nodesTree = {
	length: {
		type: "number",
	...
	__array__: {
		...
		globalMatrix: {
			...
			get: (node) => {
				...
				while (rootNode && rootNode.parent) rootNode = rootNode.parent;
				...
				if (rootNode) {
					...
					if (rootMatrix) node._babylonTransformNode?.computeWorldMatrix(forceUpdate)?.multiplyToRef(rootMatrix, matrix);
				} else if (node._babylonTransformNode) matrix.copyFrom(node._babylonTransformNode.computeWorldMatrix(forceUpdate));
				return matrix;
			},
			getTarget: (node) => node._babylonTransformNode,
			isReadOnly: true
		},
		extensions: { EXT_lights_ies: {
			multiplier: {
				type: "number",
				get: (node) => {
...
var camerasTree = { __array__: {
	...
	orthographic: {
		...
		zfar: {
			...
			set: (value, camera) => {
				if (camera._babylonCamera) camera._babylonCamera.maxZ = value;
			},
			getTarget: (camera) => camera,
		...
		znear: {
			...
			set: (value, camera) => {
				if (camera._babylonCamera) camera._babylonCamera.minZ = value;
			},
			getTarget: (camera) => camera,
	...
	perspective: {
		...
		yfov: {
			...
			set: (value, camera) => {
				if (camera._babylonCamera) camera._babylonCamera.fov = value;
			},
			getTarget: (camera) => camera,
			getPropertyName: [() => "fov"]
		},
...
var materialsTree = { __array__: {
	...
	emissiveTexture: { extensions: { KHR_texture_transform: _GenerateTextureMap("emissiveTexture") } },
	normalTexture: {
		scale: {
			type: "number",
			...
			set: (value, material, index, payload) => {
				...
				if (texture) texture.level = value;
			},
			getTarget: (material, index, payload) => _GetMaterial(material, index, payload),
			getPropertyName: [() => "level"]
		},
		extensions: { KHR_texture_transform: _GenerateTextureMap("bumpTexture") }
	},
	occlusionTexture: {
		strength: {
			type: "number",
			...
			set: (value, material, index, payload) => {
				...
				if (mat) mat.ambientTextureStrength = value;
			},
			getTarget: (material, index, payload) => _GetMaterial(material, index, payload),
			getPropertyName: [() => "ambientTextureStrength"]
		},
		extensions: { KHR_texture_transform: _GenerateTextureMap("ambientTexture") }
	},
	pbrMetallicRoughness: {
		...
		baseColorTexture: { extensions: { KHR_texture_transform: _GenerateTextureMap("albedoTexture") } },
		metallicFactor: {
			type: "number",
			get: (material, index, payload) => _GetMaterial(material, index, payload).metallic,
			set: (value, material, index, payload) => {
				const mat = _GetMaterial(material, index, payload);
				if (mat) mat.metallic = value;
			},
			getTarget: (material, index, payload) => _GetMaterial(material, index, payload),
		...
		metallicRoughnessTexture: { extensions: { KHR_texture_transform: _GenerateTextureMap("metallicTexture") } }
	},
	extensions: {
		KHR_materials_anisotropy: {
			...
			anisotropyTexture: { extensions: { KHR_texture_transform: _GenerateTextureMap("anisotropy", "texture") } }
		},
		KHR_materials_clearcoat: {
			...
			clearcoatTexture: { extensions: { KHR_texture_transform: _GenerateTextureMap("clearCoat", "texture") } },
			clearcoatNormalTexture: {
				...
				extensions: { KHR_texture_transform: _GenerateTextureMap("clearCoat", "bumpTexture") }
			},
			clearcoatRoughnessTexture: { extensions: { KHR_texture_transform: _GenerateTextureMap("clearCoat", "textureRoughness") } }
		},
		...
		KHR_materials_iridescence: {
			...
			iridescenceTexture: { extensions: { KHR_texture_transform: _GenerateTextureMap("iridescence", "texture") } },
			...
			iridescenceThicknessTexture: { extensions: { KHR_texture_transform: _GenerateTextureMap("iridescence", "thicknessTexture") } }
		},
		KHR_materials_sheen: {
			sheenColorFactor: {
				type: "Color3",
			...
			sheenColorTexture: { extensions: { KHR_texture_transform: _GenerateTextureMap("sheen", "texture") } },
			...
			sheenRoughnessTexture: { extensions: { KHR_texture_transform: _GenerateTextureMap("sheen", "thicknessTexture") } }
		},
		KHR_materials_specular: {
			...
			specularTexture: { extensions: { KHR_texture_transform: _GenerateTextureMap("metallicReflectanceTexture") } },
			specularColorTexture: { extensions: { KHR_texture_transform: _GenerateTextureMap("reflectanceTexture") } }
		},
		KHR_materials_transmission: {
			...
			transmissionTexture: { extensions: { KHR_texture_transform: _GenerateTextureMap("subSurface", "refractionIntensityTexture") } }
		},
		KHR_materials_diffuse_transmission: {
			...
			diffuseTransmissionTexture: { extensions: { KHR_texture_transform: _GenerateTextureMap("subSurface", "translucencyIntensityTexture") } },
			...
			diffuseTransmissionColorTexture: { extensions: { KHR_texture_transform: _GenerateTextureMap("subSurface", "translucencyColorTexture") } }
		},
		KHR_materials_volume: {
			...
			thicknessTexture: { extensions: { KHR_texture_transform: _GenerateTextureMap("subSurface", "thicknessTexture") } }
		}
	}
} };
var extensionsTree = {
		...
		__array__: {
			...
			rotation: {
				...
				set: (value, light) => {
					...
					if (!light._babylonTexture.getScene()?.useRightHandedSystem) value = Quaternion.Inverse(value);
...
function _GenerateTextureMap(textureType, textureInObject) {
	return {
		offset: {
			...
			getPropertyName: [() => `${textureType}${textureInObject ? "." + textureInObject : ""}.uOffset`, () => `${textureType}${textureInObject ? "." + textureInObject : ""}.vOffset`]
		},
		rotation: {
			type: "number",
		...
		scale: {
			...
			getPropertyName: [() => `${textureType}${textureInObject ? "." + textureInObject : ""}.uScale`, () => `${textureType}${textureInObject ? "." + textureInObject : ""}.vScale`]
		}
	};
}
...
function GetMappingForKey(key) {
	...
	if (current && current.type && current.get) return current;
}
/**
...
function SetInterpolationForKey(key, interpolation) {
	...
	if (current && current.type && current.get) current.interpolation = interpolation;
}
/**
...
function AddObjectAccessorToKey(key, accessor) {
	...
	for (const part of keyParts) {
		...
		if (!current[part]) {
			...
			current[part] = {};
			if (part === "__array__") current[part].__target__ = true;
		}
		current = current[part];
	}
...
function getWeights(target, source, offset, scale) {
	...
	for (let i = 0; i < value.length; i++) value[i] = source[offset++] * scale;
	return value;
}
/** @internal */
...
var TransformNodeAnimationPropertyInfo = class extends AnimationPropertyInfo {
	...
	buildAnimations(target, name, fps, keys) {
		...
		babylonAnimations.push({
			babylonAnimatable: target._babylonTransformNode,
...
var WeightAnimationPropertyInfo = class extends AnimationPropertyInfo {
	buildAnimations(target, name, fps, keys) {
		const babylonAnimations = [];
		if (target._numMorphTargets) for (let targetIndex = 0; targetIndex < target._numMorphTargets; targetIndex++) {
			...
			if (target._primitiveBabylonMeshes) {
				for (const babylonMesh of target._primitiveBabylonMeshes) if (babylonMesh.morphTargetManager) {
					const morphTarget = babylonMesh.morphTargetManager.getTarget(targetIndex);
					const babylonAnimationClone = babylonAnimation.clone();
					morphTarget.animations.push(babylonAnimationClone);
					babylonAnimations.push({
						babylonAnimatable: morphTarget,
						babylonAnimation: babylonAnimationClone
					});
				}
			}
		}
...
//#endregion
export { getVector3 as a, GetMappingForKey as c, getQuaternion as i, GetPathToObjectConverter as l, TransformNodeAnimationPropertyInfo as n, getWeights as o, WeightAnimationPropertyInfo as r, AddObjectAccessorToKey as s, AnimationPropertyInfo as t, SetInterpolationForKey as u };

//# sourceMappingURL=glTFLoaderAnimation-WufOmVm2.js.map