...
import { v as RichTypeNumber } from "./flowGraphBlock-Cu7z1xvN.js";
import { t as FlowGraphAsyncExecutionBlock } from "./flowGraphAsyncExecutionBlock-B2FuxKqh.js";
...
var AdvancedTimer = class {
	...
	constructor(options) {
		...
		*/
		...
		this._tick = (payload) => {
			...
			if (shouldBreak || this._timer >= this._timeToEnd) this._stop(data, shouldBreak);
			else this.onEachCountObservable.notifyObservers(data);
		};
		this._setState(0);
		...
		if (options.onEnded) this.onTimerEndedObservable.add(options.onEnded);
		if (options.onTick) this.onEachCountObservable.add(options.onTick);
		if (options.onAborted) this.onTimerAbortedObservable.add(options.onAborted);
	}
	/**
	...
	*/
	start(timeToEnd = this._timeToEnd) {
		if (this._state === 1) throw new Error("Timer already started. Please stop it before starting again");
	...
	*/
	dispose() {
		if (this._observer) this._contextObservable.remove(this._observer);
	...
	_stop(data, aborted = false) {
		...
		this._setState(2);
		if (aborted) this.onTimerAbortedObservable.notifyObservers(data);
		else this.onTimerEndedObservable.notifyObservers(data);
	}
};
//#endregion
...
*/
var FlowGraphSetDelayBlock = class FlowGraphSetDelayBlock extends FlowGraphAsyncExecutionBlock {
	constructor(config) {
		super(config);
	...
	_preparePendingTasks(context) {
		...
		if (duration < 0 || isNaN(duration) || !isFinite(duration)) return this._reportError(context, "Invalid duration in SetDelay block");
		if (context._getGlobalContextVariable("activeDelays", 0) >= FlowGraphSetDelayBlock.MaxParallelDelayCount) return this._reportError(context, "Max parallel delays reached");
	...
	_onEnded(timer, context) {
		...
		else Logger.Warn("FlowGraphTimerBlock: Timer ended but was not found in the running timers list");
...
//#endregion
export { setAndStartTimer as i, AdvancedTimer as n, TimerState as r, FlowGraphSetDelayBlock as t };

//# sourceMappingURL=flowGraphSetDelayBlock-CnhrtsuE.js.map