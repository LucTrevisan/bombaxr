...
import { N as _retryWithInterval, p as TextureSampler } from "./tools.functions-Bl8BhFJU.js";
...
//#region node_modules/@babylonjs/core/Buffers/storageBuffer.js
/**
...
//#endregion
//#region node_modules/@babylonjs/core/Engines/WebGPU/webgpuPerfCounter.js
/**
...
var WebGPUPerfCounter = class {
	...
	*/
	_addDuration(currentFrameId, duration) {
		if (currentFrameId < this._gpuTimeInFrameId) return;
		...
		} else this.counter.addCount(duration, false);
	}
};
//#endregion
//#region node_modules/@babylonjs/core/Compute/computeShader.js
/**
* The ComputeShader object lets you execute a compute shader on your GPU (if supported by the engine)
*/
var ComputeShader = class ComputeShader {
	/**
	...
	*/
	constructor(name, engine, shaderPath, options = {}) {
		...
		*/
		...
		if (engine.enableGPUTimingMeasurements) this.gpuTimeInFrame = new WebGPUPerfCounter();
	...
	*/
	setUniformBuffer(name, buffer) {
		...
		this._bindings[name] = {
			type: ComputeShader._BufferIsDataBuffer(buffer) ? 7 : 2,
			object: buffer,
			indexInGroupEntries: current?.indexInGroupEntries
		};
	}
	...
	*/
	setStorageBuffer(name, buffer) {
		...
		this._contextIsDirty || (this._contextIsDirty = !current || current.object !== buffer);
		this._bindings[name] = {
			type: ComputeShader._BufferIsDataBuffer(buffer) ? 7 : 3,
			object: buffer,
			indexInGroupEntries: current?.indexInGroupEntries
		};
	}
	...
	*/
	isReady() {
		...
		if (this._options.defines) for (let index = 0; index < this._options.defines.length; index++) defines.push(this._options.defines[index]);
	...
	*/
	dispatch(x, y, z) {
		if (!this.fastMode && !this._checkContext()) return false;
	...
	*/
	...
	_checkContext() {
		...
		for (const key in this._bindings) {
			...
			if (!this._options.bindingsMapping[key]) throw new Error("ComputeShader ('" + this.name + "'): No binding mapping has been provided for the property '" + key + "'");
			switch (binding.type) {
				case 0: {
			...
			switch (binding.type) {
				...
				case 1: {
					...
					if (serializedData) {
						...
						serializationObject.bindings[key] = { type: binding.type };
					}
					break;
				}
				case 2: break;
	...
	*/
	static Parse(source, scene, rootUrl) {
		...
		for (const key in source.textures) {
			...
			if (binding.type === 0) compute.setTexture(key, texture);
			else if (binding.type === 4) compute.setTexture(key, texture, false);
			else compute.setStorageTexture(key, texture);
		}
		return compute;
	}
...
//#endregion
//#region node_modules/@babylonjs/core/ShadersWGSL/boundingInfo.compute.js
...
//#endregion
//#region node_modules/@babylonjs/core/Culling/Helper/computeShaderBoundingHelper.js
/** @internal */
var ComputeShaderBoundingHelper = class {
	...
	*/
	...
	_getComputeShader(defines, hasBones, hasMorphs) {
		...
		} else computeShader = this._computeShadersCache[join];
		return computeShader;
	}
	_getUBO() {
	...
	_extractDataAndLink(computeShader, mesh, kind, stride, name, storageUnit) {
		...
		} else buffer = storageUnit[mesh.uniqueId];
	...
	registerMeshListAsync(meshes) {
		...
		for (let i = 0; i < meshes.length; i++) {
			const mesh = meshes[i];
			if (mesh.getTotalVertices() === 0 || !mesh.getVertexBuffer || !mesh.getVertexBuffer(VertexBuffer.PositionKind)) continue;
			this._processedMeshes.push(mesh);
			const manager = mesh.morphTargetManager;
			if (manager && manager.supportsPositions) maxNumInfluencers = Math.max(maxNumInfluencers, manager.numTargets);
		}
		for (let i = 0; i < this._processedMeshes.length; i++) {
			...
			} else this._computeShaders.push([computeShaderWithoutMorph, computeShaderWithoutMorph]);
		...
		return new Promise((resolve) => {
			_retryWithInterval(() => {
				...
				for (let key = iterator.next(); key.done !== true; key = iterator.next()) if (!key.value.isReady()) return false;
				return true;
			}, resolve);
		});
	}
	/** @internal */
	processMeshList() {
		if (this._processedMeshes.length === 0) return;
		this._uboIndex = 0;
	...
	fetchResultsForMeshListAsync() {
		return new Promise((resolve) => {
			...
			this._engine.readFromMultipleStorageBuffers(buffers, 0, void 0, resultData, true).then(() => {
				...
				for (const resultBuffer of this._resultBuffers) resultBuffer.dispose();
				this._resultBuffers = [];
				this._uboIndex = 0;
				resolve();
			});
	...
	_disposeCache(storageUnit) {
		for (const key in storageUnit) storageUnit[key].dispose();
	}
	_disposeForMeshList() {
		for (const resultBuffer of this._resultBuffers) resultBuffer.dispose();
		this._resultBuffers = [];
	...
	dispose() {
		...
		this._morphTargetTextureIndexBuffers = {};
		for (const ubo of this._ubos) ubo.dispose();
		this._ubos = [];
		this._computeShadersCache = {};
		this._engine = void 0;
		this._disposeForMeshList();
...
//#endregion
export { StorageBuffer as i, ComputeShader as n, WebGPUPerfCounter as r, ComputeShaderBoundingHelper as t };

//# sourceMappingURL=computeShaderBoundingHelper-CjtVYZ2m.js.map