...
import { o as defaultValueSerializationFunction } from "./flowGraphBlock-Cu7z1xvN.js";
import { n as GetFlowGraphAssetWithType } from "./flowGraphAssetsContext-XewpN2aF.js";
import { t as FlowGraphExecutionBlock } from "./flowGraphExecutionBlock-CTQRJ-er.js";
...
var FlowGraphLogger = class {
	...
	addLogItem(item) {
		...
		if (this.logToConsole) {
			...
			if (typeof value === "object" && value.getClassName) Logger.Log(`[FGLog] ${item.className}:${item.uniqueId.split("-")[0]} ${item.action} - ${JSON.stringify(value.getClassName())}: ${value.toString()}`);
			else Logger.Log(`[FGLog] ${item.className}:${item.uniqueId.split("-")[0]} ${item.action} - ${JSON.stringify(item.payload)}`);
		}
	}
...
//#endregion
//#region node_modules/@babylonjs/core/FlowGraph/flowGraphContext.js
/**
...
var FlowGraphContext = class {
	...
	*/
	...
	set enableLogging(value) {
		if (this._enableLogging === value) return;
	...
	*/
	_getGlobalContextVariable(name, defaultValue) {
		...
		if (this._hasGlobalContextVariable(name)) return this._globalContextVariables[name];
	...
	*/
	_getExecutionVariable(block, name, defaultValue) {
		if (this._hasExecutionVariable(block, name)) return this._executionVariables[this._getUniqueIdPrefixedName(block, name)];
		else return defaultValue;
	}
	/**
	...
	*/
	_addPendingBlock(block) {
		if (this._pendingBlocks.includes(block)) return;
	...
	*/
	_removePendingBlock(block) {
		...
		if (index !== -1) this._pendingBlocks.splice(index, 1);
	}
	/**
	...
	*/
	_clearPendingBlocks() {
		for (const block of this._pendingBlocks) block._cancelPendingTasks(this);
	...
	*/
	...
	_notifyOnTick(framePayload) {
		...
		for (const block of this._pendingBlocks) block._executeOnTick?.(this);
	}
	/**
	* @internal
	*/
	...
	*/
	serialize(serializationObject = {}, valueSerializationFunction = defaultValueSerializationFunction) {
		...
		for (const key in this._userVariables) valueSerializationFunction(key, this._userVariables[key], serializationObject._userVariables);
		serializationObject._connectionValues = {};
		for (const key in this._connectionValues) valueSerializationFunction(key, this._connectionValues[key], serializationObject._connectionValues);
		if (this.assetsContext !== this.getScene()) serializationObject._assetsContext = {
	...
	_initialize() {
		this._sceneReadyObserver = this._scene.onReadyObservable.add(() => {
			if (!this.sceneReadyTriggered) {
				this.onEventTriggeredObservable.notifyObservers({ type: "SceneReady" });
		...
		this._meshUnderPointerObserver = this._scene.onMeshUnderPointerUpdatedObservable.add((data) => {
			...
			if (!previousState && mesh) this.onEventTriggeredObservable.notifyObservers({
				type: "PointerOver",
				payload: {
					pointerId,
					mesh
			...
			else if (previousState && !mesh) this.onEventTriggeredObservable.notifyObservers({
				type: "PointerOut",
				payload: {
					pointerId,
					mesh: previousState
...
var FlowGraph = class {
		...
		*/
		...
		this._eventObserver = this._sceneEventCoordinator.onEventTriggeredObservable.add((event) => {
			for (const context of this._executionContexts) {
				...
				for (const block of order) if (!block._executeEvent(context, event.payload)) break;
			}
			switch (event.type) {
				case "SceneReady":
				...
				case "SceneBeforeRender":
					for (const context of this._executionContexts) context._notifyOnTick(event.payload);
					break;
				case "SceneDispose":
					this.dispose();
					break;
	...
	*/
	addEventBlock(block) {
		if (block.type === "PointerOver" || block.type === "PointerOut") this._scene.constantlyUpdateMeshUnderPointer = true;
		if (block.type !== "NoTrigger") this._eventBlocks[block.type].push(block);
		if (this.state === 1) for (const context of this._executionContexts) block._startPendingTasks(context);
		else this.onStateChangedObservable.addOnce((state) => {
			if (state === 1) for (const context of this._executionContexts) block._startPendingTasks(context);
		});
	}
	/**
	...
	*/
	start() {
		...
		if (this._executionContexts.length === 0) this.createContext();
		this.onStateChangedObservable.add((state) => {
			if (state === 1) {
				this._startPendingEvents();
				if (this._scene.isReady(true)) this._sceneEventCoordinator.onEventTriggeredObservable.notifyObservers({ type: "SceneReady" });
			}
		});
		this.state = 1;
	}
	_startPendingEvents() {
		for (const context of this._executionContexts) for (const type in this._eventBlocks) {
			...
			for (const block of order) block._startPendingTasks(context);
		}
	}
		...
		if (type === "MeshPick") {
			...
			for (const block1 of order) {
				...
				for (; i < order.length; i++) {
					const mesh2 = order[i].asset.getValue(context);
					if (mesh1 && mesh2 && _isADescendantOf(mesh1, mesh2)) break;
				}
	...
	*/
	dispose() {
		...
		this.state = 0;
		for (const context of this._executionContexts) context._clearPendingBlocks();
		this._executionContexts.length = 0;
		for (const type in this._eventBlocks) this._eventBlocks[type].length = 0;
	...
	*/
	visitAllBlocks(visitor) {
		...
		for (const type in this._eventBlocks) for (const block of this._eventBlocks[type]) {
		...
		while (visitList.length > 0) {
			...
			visitor(block);
			for (const dataIn of block.dataInputs) for (const connection of dataIn._connectedPoint) if (!idsAddedToVisitList.has(connection._ownerBlock.uniqueId)) {
				visitList.push(connection._ownerBlock);
				idsAddedToVisitList.add(connection._ownerBlock.uniqueId);
			}
			if (block instanceof FlowGraphExecutionBlock) {
				for (const signalOut of block.signalOutputs) for (const connection of signalOut._connectedPoint) if (!idsAddedToVisitList.has(connection._ownerBlock.uniqueId)) {
					visitList.push(connection._ownerBlock);
					idsAddedToVisitList.add(connection._ownerBlock.uniqueId);
				}
			}
...
//#endregion
//#region node_modules/@babylonjs/core/FlowGraph/flowGraphCoordinator.js
/**
...
*/
var FlowGraphCoordinator = class FlowGraphCoordinator {
	constructor(config) {
		this.config = config;
		/**
		...
		*/
		...
		(FlowGraphCoordinator.SceneCoordinators.get(this.config.scene) ?? []).push(this);
	}
	/**
	...
	*/
	dispose() {
		...
		if (index !== -1) coordinators.splice(index, 1);
	}
	/**
	...
	*/
	notifyCustomEvent(id, data, async = !this.dispatchEventsSynchronously) {
		...
		} else this._eventExecutionCounter.set(id, 1);
		const observable = this._customEventsMap.get(id);
		if (observable) observable.notifyObservers(data);
	}
};
/**
...
//#endregion
export { FlowGraphAction as a, FlowGraphContext as i, FlowGraph as n, FlowGraphLogger as o, FlowGraphState as r, FlowGraphCoordinator as t };

//# sourceMappingURL=flowGraphCoordinator-C6XHDW8I.js.map