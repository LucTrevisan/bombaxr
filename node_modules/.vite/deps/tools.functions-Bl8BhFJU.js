...
import { n as PerformanceConfigurator, t as EngineStore } from "./engineStore-Hebxz09y.js";
import { t as Logger } from "./logger-BsklRjyg.js";
import { t as _WarnImport } from "./devTools-OD6ut7ms.js";
import { a as IsNavigatorAvailable, i as IsDocumentAvailable, o as IsWindowObjectExist, r as GetDOMTextContent, t as PrecisionDate } from "./precisionDate-CuAZ8wH-.js";
...
var ShaderCodeNode = class {
	...
	process(preprocessors, options) {
		...
		if (this.additionalDefineKey) preprocessors[this.additionalDefineKey] = this.additionalDefineValue || "true";
		return result;
	}
};
//#endregion
...
var ShaderCodeConditionNode = class extends ShaderCodeNode {
	process(preprocessors, options) {
		for (let index = 0; index < this.children.length; index++) {
			...
			if (node.isValid(preprocessors)) return node.process(preprocessors, options);
		}
		return "";
	}
};
...
var ShaderDefineArithmeticOperator = class extends ShaderDefineExpression {
	...
	isTrue(preprocessors) {
		...
		if (value === void 0) value = this.define;
		let condition = false;
...
function _loadFile(url, onSuccess, onProgress, offlineProvider, useArrayBuffer, onError, injectedLoadFile) {
	...
	if (loadFile) return loadFile(url, onSuccess, onProgress, offlineProvider, useArrayBuffer, onError);
...
function Process(sourceCode, options, callback, engine) {
	...
	_ProcessIncludes(sourceCode, options, (codeWithIncludes) => {
		...
		callback(_ProcessShaderConversion(codeWithIncludes, options, engine), codeWithIncludes);
	});
}
/** @internal */
function PreProcess(sourceCode, options, callback, engine) {
	...
	_ProcessIncludes(sourceCode, options, (codeWithIncludes) => {
		if (options.processCodeAfterIncludes) codeWithIncludes = options.processCodeAfterIncludes(options.isFragment ? "fragment" : "vertex", codeWithIncludes, options.defines);
		callback(_ApplyPreProcessing(codeWithIncludes, options, engine), codeWithIncludes);
	});
}
/** @internal */
...
function _BuildExpression(line, start) {
	...
	else node.testExpression = _BuildSubExpression(expression);
	return node;
}
...
function _ProcessShaderConversion(sourceCode, options, engine) {
	...
	if (options.processor.postProcessor) preparedSourceCode = options.processor.postProcessor(preparedSourceCode, defines, options.isFragment, options.processingContext, engine ? { drawBuffersExtensionDisabled: engine.getCaps().drawBuffersExtension ? false : true } : {});
...
function _ProcessIncludes(sourceCode, options, callback) {
	...
	if (keepProcessing) _ProcessIncludes(returnValue.toString(), options, callback);
...
var TimingTools = class {
	...
	*/
	static SetImmediate(action) {
		if (_immediateQueue.length === 0) setTimeout(() => {
...
var _retryWithInterval = (condition, onSuccess, onError, step = 16, maxTimeout = 3e4, checkConditionOnCall = true, additionalStringOnTimeout) => {
	if (checkConditionOnCall) {
		if (_runWithCondition(condition, onSuccess, onError)) return null;
	}
	const int = setInterval(() => {
		if (_runWithCondition(condition, onSuccess, onError)) clearInterval(int);
		else {
			maxTimeout -= step;
			if (maxTimeout < 0) {
				clearInterval(int);
...
function _processShaderCode(processorOptions, baseName, processFinalCode, onFinalCodeReady, shaderLanguage, engine, effectContext) {
	...
	else if (baseName.vertexSource) vertexSource = "source:" + baseName.vertexSource;
	else if (baseName.vertexElement) vertexSource = hostDocument?.getElementById(baseName.vertexElement) || baseName.vertexElement;
	else vertexSource = baseName.vertex || baseName;
	if (typeof baseName === "string") fragmentSource = baseName;
	else if (baseName.fragmentSource) fragmentSource = "source:" + baseName.fragmentSource;
	else if (baseName.fragmentElement) fragmentSource = hostDocument?.getElementById(baseName.fragmentElement) || baseName.fragmentElement;
...
//#endregion
//#region node_modules/@babylonjs/core/Materials/effect.js
/**
	...
	*/
	constructor(baseName, attributesNamesOrOptions, uniformsNamesOrEngine, samplers = null, engine, defines = null, fallbacks = null, onCompiled = null, onError = null, indexParameters, key = "", shaderLanguage = 0, extraInitializationsAsync) {
		...
		*/
		...
		if (!cachedPipeline) this._processShaderCodeAsync(null, false, null, extraInitializationsAsync);
		else {
...
var AlphaState = class {
	...
	*/
	...
	set alphaBlend(value) {
		if (this._alphaBlend === value) return;
	...
	setAlphaEquationParameters(rgb, alpha) {
		if (this._blendEquationParameters[0] === rgb && this._blendEquationParameters[1] === alpha) return;
	...
	apply(gl) {
		...
		if (this._isAlphaBlendDirty) {
			if (this._alphaBlend) gl.enable(gl.BLEND);
...
//#endregion
//#region node_modules/@babylonjs/core/Materials/Textures/internalTexture.js
/**
...
*/
var InternalTexture = class InternalTexture extends TextureSampler {
	/**
		...
		switch (this.source) {
			...
			case 9:
				proxy = this._engine.createPrefilteredCubeTexture(this.url, null, this._lodGenerationScale, this._lodGenerationOffset, (proxy) => {
					if (proxy) proxy._swapAndDie(this, false);
					this.isReady = true;
...
function registerTextureLoader(extension, loaderFactory) {
	if (unregisterTextureLoader(extension)) Logger.Warn(`Extension with the name '${name}' already exists`);
...
function _GetCompatibleTextureLoader(extension, mimeType) {
	...
	if (!_registeredTextureLoaders.has(extension)) {
		if (extension.endsWith(".ies")) registerTextureLoader(".ies", () => import("./iesTextureLoader-juvIgqWe.js").then((n) => n.n).then((module) => new module._IESTextureLoader()));
		if (extension.endsWith(".dds")) registerTextureLoader(".dds", () => import("./ddsTextureLoader-dgCKz9aR.js").then((module) => new module._DDSTextureLoader()));
		if (extension.endsWith(".basis")) registerTextureLoader(".basis", () => import("./basisTextureLoader-CvJ0p9Fs.js").then((module) => new module._BasisTextureLoader()));
		if (extension.endsWith(".env")) registerTextureLoader(".env", () => import("./envTextureLoader-CNOcrP_9.js").then((module) => new module._ENVTextureLoader()));
		if (extension.endsWith(".hdr")) registerTextureLoader(".hdr", () => import("./hdrTextureLoader-BjANRtOQ.js").then((module) => new module._HDRTextureLoader()));
		if (extension.endsWith(".ktx") || extension.endsWith(".ktx2")) {
			registerTextureLoader(".ktx", () => import("./ktxTextureLoader-C_Ed--9k.js").then((n) => n.n).then((module) => new module._KTXTextureLoader()));
			registerTextureLoader(".ktx2", () => import("./ktxTextureLoader-C_Ed--9k.js").then((n) => n.n).then((module) => new module._KTXTextureLoader()));
		}
		if (extension.endsWith(".tga")) registerTextureLoader(".tga", () => import("./tgaTextureLoader-DcahcqF-.js").then((n) => n.n).then((module) => new module._TGATextureLoader()));
		if (extension.endsWith(".exr")) registerTextureLoader(".exr", () => import("./exrTextureLoader-Dzfy2myX.js").then((n) => n.n).then((module) => new module._ExrTextureLoader()));
	}
	const registered = _registeredTextureLoaders.get(extension);
	return registered ? Promise.resolve(registered(mimeType)) : null;
}
//#endregion
//#region node_modules/@babylonjs/core/Engines/abstractEngine.js
/**
...
var AbstractEngine = class AbstractEngine {
	...
	*/
	...
	_rebuildRenderTargetWrappers() {
		...
		for (const renderTargetWrapper of currentState) renderTargetWrapper._rebuild();
	}
	_rebuildEffects() {
	...
	*/
	get emptyTexture() {
		if (!this._emptyTexture) this._emptyTexture = this.createRawTexture(new Uint8Array(4), 1, 1, 5, false, false, 1);
	...
	*/
	get emptyTexture3D() {
		if (!this._emptyTexture3D) this._emptyTexture3D = this.createRawTexture3D(new Uint8Array(4), 1, 1, 1, 5, false, false, 1);
	...
	*/
	get emptyTexture2DArray() {
		if (!this._emptyTexture2DArray) this._emptyTexture2DArray = this.createRawTexture2DArray(new Uint8Array(4), 1, 1, 1, 5, false, false, 1);
	...
	*/
	...
	_renderLoop(timestamp) {
		...
		if (this._activeRenderLoops.length > 0 && this._frameHandler === 0) this._frameHandler = this._queueNewFrame(this._boundRenderFunction, this.getHostWindow());
	}
	/** @internal */
	_renderFrame() {
	...
	*/
	runRenderLoop(renderFunction) {
		...
		if (this._activeRenderLoops.length === 1 && this._frameHandler === 0) this._frameHandler = this._queueNewFrame(this._boundRenderFunction, this.getHostWindow());
	}
	/**
	...
	*/
	...
	_createTextureBase(url, noMipmap, invertY, scene, samplingMode = 3, onLoad = null, onError = null, prepareTexture, prepareTextureProcess, buffer = null, fallback = null, format = null, forcedExtension = null, mimeType, loaderOptions, useSRGBBuffer) {
		...
		if (loaderPromise) {
			...
			if (!buffer) this._loadFile(url, (data) => callback(new Uint8Array(data)), void 0, scene ? scene.offlineProvider : void 0, true, (request, exception) => {
				onInternalError("Unable to load " + (request && request.responseURL, exception));
			});
			else if (buffer instanceof ArrayBuffer) callback(new Uint8Array(buffer));
			else if (ArrayBuffer.isView(buffer)) callback(buffer);
...
//#endregion
export { getStateObject as A, EngineFunctionContext as B, _isRenderingStateCompiled as C, createRawShaderProgram as D, createPipelineContext as E, Initialize as F, _getGlobalDefines as H, PreProcess as I, Process as L, TimingTools as M, _retryWithInterval as N, createShaderProgram as O, Finalize as P, _ProcessIncludes as R, _finalizePipelineContext as S, _setProgram as T, allocateAndCopyTypedBuffer as U, _ConcatenateShader as V, DepthCullingState as _, NearestPOT as a, _createShaderProgram as b, _GetCompatibleTextureLoader as c, InternalTexture as d, InternalTextureSource as f, StencilStateComposer as g, StencilState as h, Mix as i, WebGLPipelineContext as j, deleteStateObject as k, registerTextureLoader as l, AlphaState as m, GetExponentOfTwo as n, AbstractEngine as o, TextureSampler as p, IsExponentOfTwo as r, QueueNewFrame as s, FloorPOT as t, unregisterTextureLoader as u, Effect as v, _preparePipelineContext as w, _executeWhenRenderingStateIsCompiled as x, resetCachedPipeline as y, _functionContainer as z };

//# sourceMappingURL=tools.functions-Bl8BhFJU.js.map