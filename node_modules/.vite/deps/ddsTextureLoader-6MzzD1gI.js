import { T as SphericalPolynomial } from "./cubemapToSphericalPolynomial-CYDqs8Y8.js";
import { t as DDSTools } from "./dds-D8JVWYLp.js";
//#region node_modules/@babylonjs/core/Materials/Textures/Loaders/ddsTextureLoader.js
/**
	...
	*/
	loadCubeData(imgs, texture, createPolynomials, onLoad) {
		...
		if (Array.isArray(imgs)) for (let index = 0; index < imgs.length; index++) {
			...
			if (!info.isFourCC && info.mipmapCount === 1) engine.generateMipMapsForCubemap(texture);
			else maxLevel = info.mipmapCount - 1;
		}
		else {
			const data = imgs;
			info = DDSTools.GetDDSInfo(data);
			texture.width = info.width;
			texture.height = info.height;
			if (createPolynomials) info.sphericalPolynomial = new SphericalPolynomial();
			loadMipmap = (info.isRGB || info.isLuminance || info.mipmapCount > 1) && texture.generateMipMaps;
			engine._unpackFlipY(info.isCompressed);
			DDSTools.UploadDDSLevels(engine, texture, data, info, loadMipmap, 6);
			if (!info.isFourCC && info.mipmapCount === 1) engine.generateMipMapsForCubemap(texture, false);
			else maxLevel = info.mipmapCount - 1;
		}
...
//#endregion
export { _DDSTextureLoader as t };

//# sourceMappingURL=ddsTextureLoader-6MzzD1gI.js.map