...
var FlowGraphStopAnimationBlock = class extends FlowGraphAsyncExecutionBlock {
	...
	_cancelPendingTasks(context) {
		const animationToStopValue = this.animationGroup.getValue(context);
		const pendingStopAnimations = context._getGlobalContextVariable("pendingStopAnimations", []);
		for (let i = 0; i < pendingStopAnimations.length; i++) if (pendingStopAnimations[i].uniqueId === animationToStopValue.uniqueId) {
	...
	_execute(context) {
		...
		if (isNaN(stopTime)) return this._reportError(context, "Invalid stop time.");
		if (stopTime > 0) this._startPendingTasks(context);
		else this._stopAnimation(animationToStopValue, context);
...
//#endregion
export { FlowGraphStopAnimationBlock as t };

//# sourceMappingURL=flowGraphStopAnimationBlock-DtX3FPB0.js.map