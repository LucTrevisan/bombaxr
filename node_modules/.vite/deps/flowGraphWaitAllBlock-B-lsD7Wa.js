...
import { t as FlowGraphExecutionBlockWithOutSignal } from "./flowGraphExecutionBlockWithOutSignal-xSLd1rxK.js";
...
var FlowGraphWaitAllBlock = class extends FlowGraphExecutionBlockWithOutSignal {
	constructor(config) {
		...
		*/
		...
		for (let i = 0; i < this.config.inputSignalCount; i++) this.inFlows.push(this._registerSignalInput(`in_${i}`));
	...
	_getCurrentActivationState(context) {
		...
		if (!context._hasExecutionVariable(this, "activationState")) for (let i = 0; i < this.config.inputSignalCount; i++) activationState.push(false);
		else {
			const contextActivationState = context._getExecutionVariable(this, "activationState", []);
			for (let i = 0; i < contextActivationState.length; i++) activationState.push(contextActivationState[i]);
		}
		return activationState;
	}
	_execute(context, callingSignal) {
		const activationState = this._getCurrentActivationState(context);
		if (callingSignal === this.reset) for (let i = 0; i < this.config.inputSignalCount; i++) activationState[i] = false;
		else {
			const index = this.inFlows.indexOf(callingSignal);
			if (index >= 0) activationState[index] = true;
		}
		...
		if (!activationState.includes(false)) {
			...
			for (let i = 0; i < this.config.inputSignalCount; i++) activationState[i] = false;
		} else callingSignal !== this.reset && this.out._activateSignal(context);
	}
	/**
...
//#endregion
export { FlowGraphWaitAllBlock as t };

//# sourceMappingURL=flowGraphWaitAllBlock-B-lsD7Wa.js.map